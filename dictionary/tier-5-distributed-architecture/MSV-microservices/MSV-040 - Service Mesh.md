...
CONTROL PLANE (Istio's istiod):
  - Receives mesh configuration (VirtualService, DestinationRule,
...
DATA PLANE (Envoy Sidecar):
  - One Envoy pod per application pod (injected automatically
...
REQUEST FLOW:
  ...
  Envoy A: open mTLS connection to Envoy B
           (verify B's SPIFFE cert: payment-service identity)
