...
TRADITIONAL CRUD (loses history):
  ...
  +----------+-------------+------------+
  | 001      | cust-1      | CONFIRMED  |  <- only current state
...
EVENT SOURCING (complete history):
  ...
  +----------+----------+------------------+------------------+
  | 1        | 001      | OrderCreated     | {cust-1, items}  |
  | 2        | 001      | PaymentProcessed | {amount: 99.99}  |
  | 3        | 001      | OrderConfirmed   | {}               |
  | 4        | 002      | OrderCreated     | {cust-2, items}  |
  | 5        | 002      | PaymentFailed    | {reason: no_funds}|
  | 6        | 002      | OrderCancelled   | {reason: payment}|
  +----------+----------+------------------+------------------+
