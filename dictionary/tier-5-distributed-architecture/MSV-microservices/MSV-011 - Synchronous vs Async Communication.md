...
THREAD:  [RUNNING] →[request]→ BLOCKED(on I/O) →[response]→ RUNNING
...
Kafka Sender Thread:   flushes to broker every linger.ms (5ms default)
Broker:                stores in partition, acknowledges

Thread is NEVER blocked. Resources freed immediately.
1000 concurrent publishes = 1000 fast buffer writes (each ~0.1ms)
vs 1000 synchronous calls = 1000 threads blocked for 200ms each
```
