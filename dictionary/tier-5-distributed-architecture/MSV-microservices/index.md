---
layout: default
title: "Microservices"
parent: "Technical Dictionary"
nav_order: 22
has_children: true
permalink: /microservices/
---

# Microservices

Service decomposition, inter-service communication, resilience patterns, distributed data management, deployment strategies, and microservices architecture at scale.

**Keywords:** MSV-001–MSV-090 (90 terms)

| ID      | Keyword                                    | Difficulty |
| ------- | ------------------------------------------ | ---------- |
| MSV-001 | Monolith vs Microservices                  | ★☆☆        |
| MSV-002 | Microservices Architecture                 | ★☆☆        |
| MSV-003 | Stateless Services                         | ★☆☆        |
| MSV-004 | Modular Monolith                           | ★★☆        |
| MSV-005 | Service Decomposition                      | ★★☆        |
| MSV-006 | Service Registry                           | ★★☆        |
| MSV-007 | Service Discovery                          | ★★☆        |
| MSV-008 | Health Check Patterns                      | ★★☆        |
| MSV-009 | Readiness and Liveness Probes              | ★★☆        |
| MSV-010 | Inter-Service Communication                | ★★☆        |
| MSV-011 | Synchronous vs Async Communication         | ★★☆        |
| MSV-012 | API Gateway                                | ★★☆        |
| MSV-013 | Backend for Frontend (BFF)                 | ★★☆        |
| MSV-014 | Load Balancing in Microservices            | ★★☆        |
| MSV-015 | Rate Limiting                              | ★★☆        |
| MSV-016 | Timeout Strategy                           | ★★☆        |
| MSV-017 | Retry Strategy                             | ★★☆        |
| MSV-018 | Fallback Strategy                          | ★★☆        |
| MSV-019 | API Composition Pattern                    | ★★☆        |
| MSV-020 | Polyglot Persistence                       | ★★☆        |
| MSV-021 | Correlation ID                             | ★★☆        |
| MSV-022 | Distributed Tracing                        | ★★☆        |
| MSV-023 | Blue-Green Deployment                      | ★★☆        |
| MSV-024 | Feature Flags                              | ★★☆        |
| MSV-025 | Twelve-Factor App                          | ★★☆        |
| MSV-026 | Backward Compatibility                     | ★★☆        |
| MSV-027 | Versioning Strategy                        | ★★☆        |
| MSV-028 | Microservices Testing Strategy             | ★★☆        |
| MSV-029 | Contract-First API Design                  | ★★☆        |
| MSV-030 | SLA and SLO in Microservices              | ★★☆        |
| MSV-031 | Domain-Driven Design (DDD)                 | ★★★        |
| MSV-032 | Bounded Context                            | ★★★        |
| MSV-033 | Aggregate                                  | ★★★        |
| MSV-034 | Ubiquitous Language                        | ★★★        |
| MSV-035 | Anti-Corruption Layer                      | ★★★        |
| MSV-036 | Strangler Fig Pattern                      | ★★★        |
| MSV-037 | Decomposition by Business Capability       | ★★★        |
| MSV-038 | Decomposition by Subdomain                 | ★★★        |
| MSV-039 | Client-Side vs Server-Side Discovery       | ★★★        |
| MSV-040 | Service Mesh                               | ★★★        |
| MSV-041 | Istio                                      | ★★★        |
| MSV-042 | Envoy Proxy                                | ★★★        |
| MSV-043 | Resilience4j                               | ★★★        |
| MSV-044 | Circuit Breaker                            | ★★★        |
| MSV-045 | Bulkhead Pattern                           | ★★★        |
| MSV-046 | Saga Pattern                               | ★★★        |
| MSV-047 | Distributed Transaction                    | ★★★        |
| MSV-048 | Event-Driven Microservices                 | ★★★        |
| MSV-049 | Eventual Consistency in Microservices      | ★★★        |
| MSV-050 | CQRS in Microservices                      | ★★★        |
| MSV-051 | Event Sourcing in Microservices            | ★★★        |
| MSV-052 | Shared Database Anti-Pattern               | ★★★        |
| MSV-053 | Database per Service                       | ★★★        |
| MSV-054 | Outbox Pattern                             | ★★★        |
| MSV-055 | Change Data Capture (CDC)                  | ★★★        |
| MSV-056 | Two-Phase Commit                           | ★★★        |
| MSV-057 | Compensating Transaction                   | ★★★        |
| MSV-058 | Idempotency in Microservices               | ★★★        |
| MSV-059 | Event-Carried State Transfer               | ★★★        |
| MSV-060 | Data Isolation per Service                 | ★★★        |
| MSV-061 | Consumer-Driven Contract Testing           | ★★★        |
| MSV-062 | Pact (Contract Testing)                    | ★★★        |
| MSV-063 | Cross-Cutting Concerns                     | ★★★        |
| MSV-064 | Distributed Logging                        | ★★★        |
| MSV-065 | OpenTelemetry in Microservices             | ★★★        |
| MSV-066 | Chaos Engineering                          | ★★★        |
| MSV-067 | Canary Deployment                          | ★★★        |
| MSV-068 | Zero-Downtime Deployment                   | ★★★        |
| MSV-069 | Graceful Shutdown                          | ★★★        |
| MSV-070 | Service Contract                           | ★★★        |
| MSV-071 | API Evolution Strategy                     | ★★★        |
| MSV-072 | Sidecar Pattern                            | ★★★        |
| MSV-073 | Ambassador Pattern                         | ★★★        |
| MSV-074 | Adapter Pattern in Microservices           | ★★★        |
| MSV-075 | mTLS in Microservices                      | ★★★        |
| MSV-076 | Zero Trust Security in Microservices       | ★★★        |
| MSV-077 | Microservices Security Patterns            | ★★★        |
| MSV-078 | Service Mesh Traffic Management            | ★★★        |
| MSV-079 | Spring Cloud Ecosystem                     | ★★★        |
| MSV-080 | Conway's Law in Microservices              | ★★★        |
| MSV-081 | Team Topologies                            | ★★★        |
| MSV-082 | Service Ownership Model                    | ★★★        |
| MSV-083 | Distributed Caching in Microservices       | ★★★        |
| MSV-084 | Data Mesh                                  | ★★★        |
| MSV-085 | Monolith to Microservices Migration        | ★★★        |
| MSV-086 | On-Premises to Cloud Migration             | ★★★        |
| MSV-087 | Technology Migration Strategy              | ★★★        |
| MSV-088 | Re-platforming vs Re-architecting          | ★★★        |
| MSV-089 | Proof of Concept (POC) in Architecture     | ★★★        |
| MSV-090 | Anti-Patterns in Microservices             | ★★★        |
