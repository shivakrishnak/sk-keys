...
LAYER 1: DATABASE CREDENTIALS
  ...
  order-service DB user: order_svc_user
    GRANT SELECT, INSERT, UPDATE ON orders.* TO order_svc_user
    GRANT SELECT, INSERT ON orders.outbox_events TO order_svc_user
    REVOKE ALL ON customers.* FROM order_svc_user
  
  customer-service DB user: customer_svc_user
    GRANT SELECT, INSERT, UPDATE ON customers.* TO customer_svc_user
...
│              │  credentials + network + policy = 3 layers"│
└──────────────────────────────────────────────────────────┘
```
