...
- SAP-044 - SOLID Principles (specifically the Open-Closed Principle: new knowledge sources extend the system without modifying the controller or other knowledge sources)

**Builds On This (learn these next):**

- SAP-076 - Plugin Architecture (knowledge sources are typically implemented as plugins; the blackboard framework defines the knowledge source interface)
- SAP-077 - Pipe and Filter (contrasting pattern: Pipe and Filter uses fixed order with predetermined transformations; Blackboard uses opportunistic, order-independent contributions; knowing both clarifies when each applies)

**Alternatives / Comparisons:**

- SAP-077 - Pipe and Filter (for sequential processing where the order of transformations is known and fixed; simpler and more predictable than Blackboard)
