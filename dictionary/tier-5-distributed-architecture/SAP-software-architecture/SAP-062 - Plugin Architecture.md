...
difficulty: ★★★
depends_on: SAP-044, SAP-012, SAP-048
used_by:
related: SAP-075, SAP-077
tags:
  - architecture
  - pattern
  - deep-dive
...
| Field          | Value                     |
| -------------- | ------------------------- |
| **Depends on** | SAP-044, SAP-012, SAP-048 |
| **Used by**    | -                         |
| **Related**    | SAP-075, SAP-077          |
...
- SAP-044 - SOLID Principles (specifically the Open-Closed Principle: open for extension via plugins, closed for modification; and Dependency Inversion: depend on abstractions = depend on plugin interfaces)
- SAP-012 - Cohesion and SAP-048 - Coupling (plugin architecture reduces coupling between the core and implementations; understanding these principles explains why the pattern works)

**Builds On This (learn these next):**

- SAP-075 - Modular Monolith Patterns (plugins are a way to extend a modular monolith without modifying its core modules)
- SAP-077 - Pipe and Filter (filters in a pipeline are often implemented as plugins; the pipeline framework defines the filter interface, implementations are plugins)

**Alternatives / Comparisons:**

- SAP-063 - Hexagonal Architecture (closely related; hexagonal ports are plugin interfaces; adapters are plugin implementations; hexagonal is the DDD-influenced cousin of Plugin Architecture)
