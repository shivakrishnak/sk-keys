...
Parameters:
  ...
  refill_rate: float  # tokens added per second (e.g., 10/sec)
...
refill_rate = sustained throughput
  ...
  If client sends above refill_rate: bucket drains over time
  Example: refill_rate=10/sec, capacity=100
    → Can burst 100 in 1 second
    → Then limited to 10/sec
    → After 10 seconds of silence: full bucket again (burst ready)
```
