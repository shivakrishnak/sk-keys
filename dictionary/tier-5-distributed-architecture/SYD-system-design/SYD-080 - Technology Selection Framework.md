...
nav_order: 80
permalink: /syd/technology-selection-framework/
---
...
### RELATIONAL (ACID, complex queries)
  PostgreSQL:
    ...
    Cons: single-primary write bottleneck (~10K writes/s),
          vertical scale limit (~5-10TB with good indexing).
...
### WIDE-COLUMN (high write throughput, horizontal scale)
  Cassandra:
    ...
    Cons: no joins, no aggregation, no ACID cross-partition,
    ...
    Choose when: 100K+ writes/sec, key-based access only,
                 eventual consistency OK, team has ops expertise.
