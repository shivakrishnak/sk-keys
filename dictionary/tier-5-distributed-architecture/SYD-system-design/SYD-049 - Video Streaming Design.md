...
Quality selection heuristics:
  buffer_level > 20 sec: upgrade quality if bandwidth allows
...
CDN cache key: full URL path
  ...
  Per region: segment cached at each PoP (Point of Presence)

First viewer per region: CDN miss → fetch from origin → cache
