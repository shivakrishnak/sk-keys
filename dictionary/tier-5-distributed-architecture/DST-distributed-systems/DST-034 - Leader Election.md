...
difficulty: ★★★
depends_on: DST-011, DST-051, DST-012
used_by: DST-046, DST-047, DST-050
related: DST-046, DST-047, DST-051, DST-012, DST-013
tags:
  - distributed
  - reliability
  - algorithm
...
| Metadata        |                                             |     |
| :-------------- | :------------------------------------------ | :-- |
| **Depends on:** | DST-011, DST-051, DST-012                   |     |
...
| :------------------------------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Leader election prevents all split-brain"              | Leader election prevents two leaders in the SAME term. But a stale leader from an old term may still believe it's leader until it receives a higher-term message. Fencing tokens (DST-013) are required to prevent stale leaders from corrupting storage. |
...
- DST-011 - Total Order / Partial Order (leader election enables total order broadcast)
- DST-051 - Quorum (majority quorum is the mathematical basis for election safety)
- DST-012 - Split Brain (leader election is the primary mechanism to prevent it)

**Builds On This (learn these next):**

- DST-046 - Raft (specific consensus protocol built on leader election)
- DST-047 - Paxos (alternative consensus protocol with implicit leader election)
- DST-050 - State Machine Replication (requires leader election as its foundation)

**Alternatives / Comparisons:**

- DST-013 - Fencing / Epoch (prevents stale leaders from acting after election)
- DST-051 - Quorum (mathematical basis for election safety guarantees)
