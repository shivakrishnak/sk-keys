...
related: DST-027, DST-019, DST-009, DST-028, DST-010
tags:
  - distributed
  - reliability
  - pattern
...
- DST-027 - Circuit Breaker (fallback is typically triggered when circuit breaker opens — understand circuit breaker first)
- DST-010 - Timeout (timeout triggers the exception that triggers the fallback — timeout before fallback)

**Builds On This (learn these next):**

- DST-019 - Graceful Degradation (broader pattern of which fallback is a mechanism)

**Alternatives / Comparisons:**

- DST-027 - Circuit Breaker (complementary — circuit breaker trips the fallback)
- DST-028 - Retry with Backoff (retry first, then fallback if retries exhausted)
- DST-009 - Bulkhead (bulkhead prevents resource exhaustion; fallback handles the rejected calls)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-019 Graceful Degradation,  |
