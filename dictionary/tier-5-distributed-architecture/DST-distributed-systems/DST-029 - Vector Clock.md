...
difficulty: ★★★
depends_on: DST-040, DST-035, DST-037
used_by: DST-023, DST-037, DST-039
related: DST-040, DST-037, DST-011, DST-023, DST-024
tags:
  - distributed
  - algorithm
  - deep-dive
...
| Metadata        |                                    |     |
| :-------------- | :--------------------------------- | :-- |
| **Depends on:** | DST-040, DST-035, DST-037          |     |
| **Used by:**    | DST-023, DST-037, DST-039          |     |
| **Related:**    | DST-040, DST-037, DST-011, DST-023 |     |
...
- DST-040 - Lamport Clock (the simpler logical clock that vector clocks extend)
- DST-035 - Consistency Models (why concurrent event detection matters for system design)
- DST-037 - Causal Consistency (uses vector clocks as its implementation mechanism)

**Builds On This (learn these next):**

- DST-011 - Total Order / Partial Order (the formal order theory behind vector clocks)
- DST-023 - Eventual Consistency (conflict detection enables safe eventual convergence)
- DST-039 - Serializability (contrast: how databases avoid concurrent writes entirely)

**Alternatives / Comparisons:**

- DST-040 - Lamport Clock (simpler, O(1), but cannot detect concurrent events)
- DST-020 - CRDT (builds on vector clock principles with automatic merge functions)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-011 Total/Partial Order,   |
|                  | DST-020 CRDT                   |
+------------------+--------------------------------+
```
