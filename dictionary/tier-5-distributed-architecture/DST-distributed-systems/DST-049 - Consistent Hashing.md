...
4. **Load balance via distribution:** if node positions are uniformly distributed on the ring: each node holds approximately K/N keys. If positions are non-uniform (some nodes clump together): some nodes are overloaded. Virtual nodes (DST-054) solve this.
...
- "Cassandra has uneven load on nodes" → Consistent hashing with single tokens can produce large arc lengths (some nodes handle large ranges). Solution: virtual nodes (vnodes) — each physical node has 256 virtual tokens, distributing the arc lengths evenly. See DST-054.
...
public class ConsistentHashRouter {
    ...
    public void addServer(String server) {
        ...
        // (virtual nodes = DST-054, improves load balance)
...
2. **Ring propagated:** gossip protocol (DST-055) propagates the new ring topology to all existing nodes.
...
| :-------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Consistent hashing means equal load distribution"              | Without virtual nodes: arc lengths between nodes depend on hash values — inherently unequal. A node with a large arc handles proportionally more keys. Equal load requires virtual nodes (DST-054) — many positions per physical node, averaging out the arc lengths.                                                                                                |
...
- DST-022 - CAP Theorem (consistent hashing is a technique for distributed data placement; understanding CAP provides context for the trade-offs in distributed key-value stores that use it)

**Builds On This (learn these next):**

- DST-054 - Virtual Nodes (vnodes extend consistent hashing to improve load balance)

**Alternatives / Comparisons:**

- DST-054 - Virtual Nodes (the standard extension for consistent hashing)
- DST-055 - Gossip Protocol (how ring topology changes are propagated between nodes)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-054 Virtual Nodes,         |
...
3. Without virtual nodes (DST-054): load distribution is uneven (arc length variance). With virtual nodes (256 positions per node): load is approximately equal across all physical nodes.
