...
difficulty: ★★☆
depends_on: DST-026, DST-010
used_by: DST-009, DST-030, DST-019
related: DST-028, DST-009, DST-030, DST-010
tags:
  - distributed
  - reliability
  - pattern
...
| Metadata        |                                    |     |
| :-------------- | :--------------------------------- | :-- |
| **Depends on:** | DST-026, DST-010                   |     |
| **Used by:**    | DST-009, DST-030, DST-019          |     |
| **Related:**    | DST-028, DST-009, DST-030, DST-010 |     |
...
| Retry (DST-028)    | Retry transient failures     | Increased latency    | Per-attempt                 |
| Timeout (DST-010)  | Bound wait time              | Fail after T_timeout | Per-call                    |
| Bulkhead (DST-009) | Isolate resource pools       | Reject excess        | Capacity-based              |
| Fallback (DST-030) | Alternative response         | Degraded response    | N/A (always active)         |
...
GOOD: (1) Add a separate circuit breaker for the fallback path (circuit breaker for D). (2) Rate-limit the fallback path (Bulkhead DST-009 on D's caller). (3) Make fallback truly local (cached data, static response) — no additional remote call.
...
**Prevention:** Rate limiting (Bulkhead DST-009) + input validation at the API gateway before requests reach circuit-breaker-protected paths.
...
- DST-026 - Heartbeat (circuit breaker HALF-OPEN probe is conceptually a heartbeat to the dependency)
- DST-010 - Timeout (timeouts are the primary trigger for circuit breaker failures)

**Builds On This (learn these next):**

- DST-009 - Bulkhead (bulkhead + circuit breaker = standard resilience combination)
- DST-030 - Fallback (circuit breaker in OPEN state needs a fallback strategy)
- DST-019 - Graceful Degradation (circuit breaker is the mechanism; graceful degradation is the strategy)

**Alternatives / Comparisons:**

- DST-028 - Retry with Backoff (complementary — retry handles transient, circuit breaker handles sustained failures)
- DST-010 - Timeout (simpler primitive; circuit breaker adds state machine on top of timeouts)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-009 Bulkhead,              |
|                  | DST-028 Retry, DST-030 Fallback|
+------------------+--------------------------------+
```
