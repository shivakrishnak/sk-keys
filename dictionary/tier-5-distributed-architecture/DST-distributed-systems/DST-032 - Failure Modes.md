...
difficulty: ★★★
depends_on: DST-006, DST-023
used_by: DST-033, DST-035
related: DST-023, DST-024, DST-033, DST-006
tags:
  - distributed
  - reliability
  - foundational
...
| Metadata        |                                    |     |
| :-------------- | :--------------------------------- | :-- |
| **Depends on:** | DST-006, DST-023                   |     |
| **Used by:**    | DST-033, DST-035                   |     |
| **Related:**    | DST-023, DST-024, DST-033, DST-006 |     |
...
- DST-006 - CAP Theorem (failure models and the CAP theorem are tightly coupled — partitions are a specific failure mode)
- DST-023 - Raft (Raft is the most widely deployed crash-recovery protocol — understand what it handles before what it doesn't)

**Builds On This (learn these next):**

- DST-033 - Two-Phase Commit (2PC assumes crash-recovery model — coordinator crash is the critical failure case)
- DST-035 - Three-Phase Commit (3PC improves 2PC's crash handling in crash-recovery model)

**Alternatives / Comparisons:**

- DST-023 - Raft (crash-recovery solution)
- DST-024 - Paxos (crash-recovery solution, same failure model as Raft)
- DST-033 - Two-Phase Commit (crash-recovery atomic commitment protocol)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-033 Two-Phase Commit,      |
|                  | DST-023 Raft                   |
+------------------+--------------------------------+
```
