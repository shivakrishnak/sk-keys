...
difficulty: ★★★
depends_on: DST-029, DST-057
related: DST-017, DST-015, DST-029, DST-062, DST-061
tags:
  - distributed
  - pattern
  - architecture
...
3. **Compensating transactions must be idempotent (DST-029).** Compensation may be triggered multiple times (message retry). `CancelOrder(orderId)` called twice must produce the same result as calling once.
...
**Accidental:** Choreography vs orchestration (DST-057), saga state persistence mechanism (Axon, Temporal, custom), event sourcing (DST-062) vs CRUD for saga state.
...
GOOD: Saga T3 sends `{orderId: 123, amount: 100, idempotencyKey: "saga-123-step-3"}`. Payment service checks idempotency key first (DST-029). Duplicate request → return stored result (no second charge).
...
- DST-029 - Idempotency (compensating transactions must be idempotent; saga steps must be idempotent for safe retry)
- DST-057 - Choreography vs Orchestration (saga is implemented with either choreography or orchestration — understand both before choosing)

**Builds On This (learn these next):**

- DST-015 - Outbox Pattern (reliable event publication for saga steps — required for production sagas)
- DST-062 - Event Sourcing (saga state stored as event sequence; natural pairing with sagas)

**Alternatives / Comparisons:**

- DST-017 - Two-Phase Commit (alternative for distributed transactions — strong consistency vs saga's eventual consistency)
- DST-061 - CQRS (often combined with saga for eventual consistency in read models)
...
|                  | "eventual atomicity"           |
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-015 Outbox Pattern,        |
|                  | DST-062 Event Sourcing,        |
|                  | DST-057 Choreography vs Orch   |
+------------------+--------------------------------+
```
...
2. Every saga step must be idempotent (DST-029). The saga orchestrator will retry failed or timed-out steps. If a step charges a payment: charging twice because of a retry is a bug, not expected behavior. Use idempotency keys for all external side effects.
3. Use the outbox pattern (DST-015) for publishing saga events. Direct Kafka publish after DB commit has a failure window: DB commits, publish fails, saga never advances. Outbox atomically stores events in the same DB transaction, then reliably publishes them.
...
- **CQRS + Saga for order processing:** In CQRS (DST-061): commands (CreateOrder, ReserveInventory) are handled by saga steps. Events (OrderCreated, InventoryReserved) update read models asynchronously. The saga coordinates the command side while CQRS handles the read side. This pairing — saga for write-side consistency, CQRS for read-side eventual consistency — is a common pattern in DDD-based microservices (Axon Framework uses exactly this).
