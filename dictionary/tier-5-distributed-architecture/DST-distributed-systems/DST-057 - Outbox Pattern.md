...
difficulty: ★★★
depends_on: DST-056, DST-055, DST-045
related: DST-056, DST-049, DST-055
tags:
  - distributed
  - architecture
  - pattern
...
| Metadata        |                           |     |
| :-------------- | :------------------------ | :-- |
| **Depends on:** | DST-056, DST-055, DST-045 |     |
| **Related:**    | DST-056, DST-049, DST-055 |     |
...
| :-------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Outbox Pattern provides exactly-once delivery"     | Outbox Pattern provides AT-LEAST-ONCE delivery. If the relay publishes to Kafka and then crashes before marking the outbox row as `DONE`: on restart, it will re-read the same row and publish again → duplicate message. Consumers must be idempotent (DST-045). If you need exactly-once: use Kafka's transactional producer + consumer, coordinated with the database transaction — but this is significantly more complex.                                                                               |
...
- DST-056 - Event Sourcing (Outbox Pattern solves reliable publishing from Event Sourcing stores)
- DST-055 - CQRS (Outbox enables reliable event delivery for CQRS projections)
- DST-045 - Idempotency (at-least-once delivery requires idempotent consumers)

**Builds On This (learn these next):**

- DST-049 - Saga Pattern (Sagas use reliable event publishing — Outbox Pattern enables this)
- DST-056 - Event Sourcing (ES + Outbox = reliable event sourcing pipeline)

**Alternatives / Comparisons:**

- DST-055 - CQRS (CQRS projections need reliable event delivery — Outbox Pattern is the solution)
