...
difficulty: ★★★
depends_on: DST-023, DST-025, DST-027
used_by: DST-027
related: DST-023, DST-025, DST-027, DST-022
tags:
  - distributed
  - replication
  - algorithm
...
| Metadata        |                                    |     |
| :-------------- | :--------------------------------- | :-- |
| **Depends on:** | DST-023, DST-025, DST-027          |     |
| **Used by:**    | DST-027                            |     |
| **Related:**    | DST-023, DST-025, DST-027, DST-022 |     |
...
- DST-023 - Raft (the consensus protocol that uses log replication as its core mechanism)
- DST-025 - Replication Strategies (log replication is the mechanism behind single-leader strategy)
- DST-022 - Leader Election (leader must be elected before log replication begins)

**Builds On This (learn these next):**

- DST-027 - State Machine Replication (log replication is the transport layer for SMR)

**Alternatives / Comparisons:**

- DST-025 - Replication Strategies (log replication vs. leaderless quorum vs. multi-leader)
- DST-027 - State Machine Replication (log replication as the mechanism enabling SMR)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-027 State Machine          |
|                  | Replication                    |
+------------------+--------------------------------+
```
