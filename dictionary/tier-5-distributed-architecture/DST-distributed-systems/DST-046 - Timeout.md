...
difficulty: ★★☆
depends_on: DST-041
related: DST-042, DST-044, DST-041, DST-043
tags:
  - distributed
  - reliability
  - pattern
...
GOOD: timeout = P99_peak × 1.5. For catalog: P99_peak = 1.8s → timeout = 2.7s. Also: add rate limiting (DST-043 bulkhead) to prevent catalog from being overloaded in the first place.
...
- DST-041 - Heartbeat (heartbeat and timeout both address failure detection — heartbeat is proactive, timeout is reactive)

**Builds On This (learn these next):**

- DST-042 - Circuit Breaker (circuit breaker aggregates timeout-based failures into state transitions)
- DST-044 - Retry with Backoff (retry triggers on timeout; timeout must be set before retry makes sense)
- DST-043 - Bulkhead (bulkhead limits concurrent calls, reduces timeout load)

**Alternatives / Comparisons:**

- DST-041 - Heartbeat (proactive liveness detection vs reactive timeout)
- DST-042 - Circuit Breaker (aggregates many timeouts into a circuit-level decision)
