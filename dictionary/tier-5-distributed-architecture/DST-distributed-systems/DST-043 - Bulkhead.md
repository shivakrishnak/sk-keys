...
used_by: DST-048
related: DST-042, DST-044, DST-046, DST-047, DST-048
tags:
  - distributed
  - reliability
  - pattern
...
GOOD: (1) Rate limiting per authenticated user BEFORE bulkhead: max N Payment calls per user per minute. (2) API gateway rate limiting by IP/user. (3) Combine rate limit (DST-044 retry limit = N per minute) + bulkhead (N concurrent).
...
- DST-042 - Circuit Breaker (circuit breaker complements bulkhead — together they form the core resilience pair)
- DST-046 - Timeout (timeout triggers bulkhead saturation — understand timeouts before bulkheads)

**Builds On This (learn these next):**

- DST-048 - Graceful Degradation (bulkhead enables graceful degradation by containing blast radius)

**Alternatives / Comparisons:**

- DST-042 - Circuit Breaker (stops calling failing service; bulkhead limits concurrent calls — use together)
- DST-047 - Fallback (what to do when bulkhead full — return cached/default response)
- DST-046 - Timeout (bound wait time per call; bulkhead bounds concurrent calls)
