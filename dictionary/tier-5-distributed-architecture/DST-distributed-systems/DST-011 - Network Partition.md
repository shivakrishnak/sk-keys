...
difficulty: ★★☆
depends_on: DST-022, DST-004, DST-013
used_by: DST-014, DST-012, DST-022
related: DST-022, DST-012, DST-013
tags:
  - distributed
  - networking
  - reliability
...
| Relationship    | IDs                                     |         |
| --------------- | --------------------------------------- | ------- |
| **Depends on:** | DST-022, DST-004, DST-013               |         |
| **Used by:**    | DST-014, DST-012, DST-022               |         |
| **Related:**    | DST-022, DST-012, DST-013               |         |
...
healing; risk of "split brain" (DST-012).
...
- Use circuit breakers (DST-027) to fail fast when peers
  are unreachable.
- Use idempotent operations (DST-029) so retries on healing
  are safe.
- Decide CP vs AP upfront and encode it in your SLA/README.

**Level 3 - How it works (mid-level engineer):**
Partitions are detected via heartbeat (DST-026) timeouts. When a
...
tokens (DST-013) prevent the isolated minority from writing to
...
```
**Fix:** Implement fencing tokens (DST-013): each new leader
...
- DST-022 - CAP Theorem (formal framework for partition trade-offs)
- DST-004 - Fallacies of Distributed Computing (network is reliable)
- DST-026 - Heartbeat (partition detection mechanism)

**Builds On This (learn these next):**
- DST-012 - Split Brain (what happens when partition goes unhandled)
- DST-013 - Fencing / Epoch (how to safely handle partition healing)
- DST-014 - Failure Modes (broader taxonomy of distributed failures)
...
| NEXT EXPLORE  | DST-012 Split Brain, DST-022 CAP  |
+-------------------------------------------------+
```
