...
difficulty: ★★★
depends_on: DST-048, DST-022
used_by: DST-045, DST-046, DST-047, DST-012
related: DST-045, DST-048, DST-012, DST-046
tags:
  - distributed
  - consensus
  - reliability
...
| Metadata        |                                    |     |
| :-------------- | :--------------------------------- | :-- |
| **Depends on:** | DST-048, DST-022                   |     |
| **Used by:**    | DST-045, DST-046, DST-047, DST-012 |     |
| **Related:**    | DST-045, DST-048, DST-012, DST-046 |     |
...
- DST-048 - Replication Strategies (quorum is the consistency mechanism for leaderless replication)
- DST-022 - CAP Theorem (quorum is how distributed systems choose between C and A)

**Builds On This (learn these next):**

- DST-045 - Leader Election (quorum is the safety mechanism for leader election)
- DST-046 - Raft (Raft uses majority quorum for both leader election and log commit)
- DST-047 - Paxos (Paxos uses quorum intersection as its core safety invariant)
- DST-012 - Split Brain (quorum is the primary prevention mechanism for split-brain)

**Alternatives / Comparisons:**

- DST-048 - Replication Strategies (sync/async replication as alternatives to quorum-based)
- DST-012 - Split Brain (the failure mode quorums are designed to prevent)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-012 Split Brain,           |
|                  | DST-046 Raft                   |
+------------------+--------------------------------+
```
