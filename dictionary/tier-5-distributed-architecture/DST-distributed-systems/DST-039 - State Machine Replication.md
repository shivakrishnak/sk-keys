...
difficulty: ★★★
depends_on: DST-049, DST-046, DST-011
used_by: DST-046, DST-047
related: DST-049, DST-046, DST-047, DST-011
tags:
  - distributed
  - consensus
  - algorithm
...
# DST-050 - State Machine Replication
...
| Metadata        |                                    |     |
| :-------------- | :--------------------------------- | :-- |
| **Depends on:** | DST-049, DST-046, DST-011          |     |
| **Used by:**    | DST-046, DST-047                   |     |
| **Related:**    | DST-049, DST-046, DST-047, DST-011 |     |
...
- DST-049 - Log Replication (the mechanism that delivers total-ordered commands to all replicas)
- DST-046 - Raft (the most widely-deployed consensus protocol providing total order for SMR)
- DST-011 - Total Order / Partial Order (total order broadcast is the mathematical foundation of SMR)

**Builds On This (learn these next):**

- DST-046 - Raft (Raft IS an SMR implementation — leader election + log replication = SMR)
- DST-047 - Paxos (Paxos provides the same total order broadcast as Raft for SMR)

**Alternatives / Comparisons:**

- DST-049 - Log Replication (the transport mechanism for SMR commands)
- DST-011 - Total Order / Partial Order (total order broadcast equivalence to SMR)
...
+------------------+--------------------------------+
| NEXT EXPLORE     | DST-049 Log Replication,       |
|                  | DST-046 Raft                   |
+------------------+--------------------------------+
```
