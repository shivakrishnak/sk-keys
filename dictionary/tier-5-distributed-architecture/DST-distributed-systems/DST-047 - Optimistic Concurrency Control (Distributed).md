...
difficulty: ★★★
depends_on: DST-015, DST-016, DST-039
used_by: DST-052, DST-061
related: DST-052, DST-015, DST-039
tags:
  - distributed
  - concurrency
  - transactions
...
# DST-018 - Optimistic Concurrency Control (Distributed)
...
| Relationship    | IDs                                     |         |
| --------------- | --------------------------------------- | ------- |
| **Depends on:** | DST-015, DST-016, DST-039               |         |
| **Used by:**    | DST-052, DST-061                        |         |
| **Related:**    | DST-052, DST-015, DST-039               |         |
...
CockroachDB uses HLC timestamps (DST-042) to approximate this
...
```
This is 2PC (DST-015) applied to the validation + commit step.
...
- DST-039 - Serializability (isolation level OCC can provide)
- DST-015 - Two-Phase Commit (OCC validation phase uses 2PC)
- DST-016 - XA Transactions (pessimistic alternative for comparison)

**Builds On This (learn these next):**
- DST-052 - Distributed Locking (compare with pessimistic approach)
- DST-061 - CQRS (OCC enables high-throughput write paths)
- DST-062 - Event Sourcing (append-only writes avoid OCC conflicts)
...
- CRDTs (DST-020): avoid conflict entirely via commutative ops
...
| NEXT EXPLORE  | DST-052 Distributed Locking      |
+-------------------------------------------------+
```
...
OCC and Saga (DST-056) for a multi-step order process that spans
