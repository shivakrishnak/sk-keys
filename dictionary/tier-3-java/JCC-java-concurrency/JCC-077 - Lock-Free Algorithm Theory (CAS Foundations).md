...
difficulty: ★★★
depends_on: JCC-068, JCC-072, JCC-026
used_by:
related: JCC-043, JCC-067, JCC-042
tags:
  - java
  - concurrency
  - advanced
...
nav_order: 77
permalink: /java-concurrency/lock-free-algorithm-theory-cas-foundations/
---
...
| Field      | Value                                              |
|------------|----------------------------------------------------|
| Depends on | JCC-068 Lock-Free Data Structures, JCC-072 JSR 133, JCC-026 CAS (Compare-And-Swap) |
| Related    | JCC-043 VarHandle, JCC-067 JMM Happens-Before, JCC-042 Atomic Classes |
...
Proof sketch (by contradiction):
  Suppose R/W protocol agrees on one value for any 2-thread start.
  Consider a bivalent configuration (both 0 and 1 still possible).
  - Thread 1 reads R: reading never changes state -> still bivalent
  - Thread 2 writes R: writes don't communicate read values
  - After any sequence of R/W: still bivalent
  ... [formal argument: cannot reach univalent state with only R/W]
...
Thread 1 starts enqueue: CAS tail.next from null to newNode
  -> succeeds: partial state (tail.next != null but tail != newNode)

Thread 2 starts enqueue: observes tail.next != null
  -> "helps" Thread 1: CAS this.tail from old to tail.next (advance tail)
  -> now state is consistent
  -> Thread 2 can proceed with its own enqueue

Result: both Thread 1's partial work AND Thread 2's work progress.
System advances even when threads interleave at worst possible points.
```
...
(* Invariant: at every state, stack contains exactly pushed - popped items *)
```

**Using JCStress (concurrent testing):**
```java
// JCStress tests exhaustively find interleavings conventional tests miss
@JCStressTest
...
- [[JCC-068 - Lock-Free Data Structures]] - practical Java implementations
- [[JCC-072 - JSR 133 - Java Memory Model Specification]] - the
...
- [[JCC-043 - VarHandle]] - Java 9 access mode API implementing
