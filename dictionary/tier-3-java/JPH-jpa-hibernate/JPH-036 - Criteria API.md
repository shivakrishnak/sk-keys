...
nav_order: 36
permalink: /jpa-hibernate/criteria-api/
---

# JPH-036 - Criteria API
...
Criteria API: Java objects, compile-time type-checkable
  ...
  -> Type mismatch (comparing price to String) = compile error
```
...
public abstract class Product_ {
    ...
    public static volatile ManyToOneAttribute<Product, Category> category;
    // ... one field per entity field
}

// Usage (compile-time safe):
p.get(Product_.name)     // returns Path<String>; compile error if "name" renamed
p.get(Product_.price)    // returns Path<BigDecimal>
cb.gt(p.get(Product_.price), 100) // type-safe: price is BigDecimal; 100 is Number
...
```
┌──────────────────────────────────────────────────────────┐
│ BUILDER      │ CriteriaBuilder cb = em.getCriteriaBuilder│
