...
nav_order: 35
permalink: /jpa-hibernate/query-cache/
---
...
```
Cache key:
  QueryKey = (SQL string, parameter values, first, max, sort)
...
Without query cache:
  1. SELECT id, name, price FROM products WHERE featured=true
...
│              │ invalidates ALL related query cache entries│
├──────────────┼───────────────────────────────────────────┤
