...
# SPR-071 - Spring Boot Startup Lifecycle
...
│
├─5. refreshContext()  ←── THIS IS THE BIG PHASE (80% of time)
│     │
...
│     │         (constructor → @Autowired inject → @PostConstruct)
│     │
...
BeanCreationException: Error creating bean 'orderService'
  Caused by: BeanCreationException: Error creating bean 'dataSource'
    Caused by: HikariPool$PoolInitializationException:
      Failed to initialize pool: Connection refused
        Caused by: PSQLException: Connection to db:5432 refused.
```
