...
nav_order: 33
permalink: /react/react-quick-recall-card/
---
...
COMPONENT TYPES:
  ...
  Class component: extends React.Component [legacy, needed for Error Boundaries]
...
REF HOOKS:
  useRef        - DOM element access, mutable value (no re-render)
...
FORMS:
  ...
  Libraries: React Hook Form (uncontrolled, fast), Formik (controlled)
...
ERROR HANDLING:
  Error Boundaries: class component, getDerivedStateFromError
...
LIFECYCLE:
  ...
  Update: function re-runs + useEffect([dep]) if dep changed
...
CONCURRENT (React 18):
  ...
  useDeferredValue: defer a value, show stale during transition
...
PROBLEM: Component not re-rendering when expected
  ...
  → Is React.memo blocking it?
    (Parent re-renders but child doesn't because React.memo)

PROBLEM: Component re-rendering too often
  → Is state too high up? (Lifting too far causes all children to re-render)
...
Cross-tree state                    createContext + Provider
...
├──────────────────────────────────────────────────────────┤
│ DATA FLOW   │ Down: props | Up: callbacks | Wide: Context│
├──────────────────────────────────────────────────────────┤
