...
nav_order: 47
permalink: /react/class-components-to-hooks-migration/
---
...
constructor(props)     → useState(initialValue)
                         (initialValue can be a function: useState(() => ...)
...
componentDidUpdate     → useEffect(() => { ... }, [dep1, dep2])
                         ...
                         Note: useEffect ALWAYS runs after first render too
                         (no equivalent to "only on update, skip mount")

componentWillUnmount   → useEffect cleanup: return () => { ... }

this.state.x           → const [x, setX] = useState(initialX)
...
getDerivedStateFromProps → not a hook; compute during render:
                           const derivedValue = derive(props, state)

shouldComponentUpdate  → React.memo (for the whole component)
                         or useMemo for specific expensive values

componentDidCatch      → NO HOOK EQUIVALENT (still requires class)
getSnapshotBeforeUpdate→ NO HOOK EQUIVALENT (still requires class)
```
