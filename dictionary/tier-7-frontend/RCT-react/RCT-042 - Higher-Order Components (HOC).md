...
nav_order: 42
permalink: /react/higher-order-components/
---
...
Higher-order component (React):
  ...
  const LoggedButton = withLogging(Button);  // Component → Component
```
...
HOC contract:
  ...
  - Rule:   MUST spread all original props through to wrapped component
  - Rule:   MUST forward refs if needed (using forwardRef)
  - Rule:   MUST not mutate the input component
  - Rule:   Should copy static methods (hoistNonReactStatics)
```
...
```jsx
const EnhancedPage = withAuth(withAnalytics(withLogger(withData(Page))));
```
