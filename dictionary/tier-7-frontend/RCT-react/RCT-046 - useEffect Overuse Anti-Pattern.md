...
nav_order: 46
permalink: /react/useeffect-overuse-anti-pattern/
---
...
useEffect is for SYNCHRONISING with external systems:
  ...
  ✅ Starting/stopping subscriptions (WebSocket, EventSource)
  ✅ Timer setup (setInterval, setTimeout)
  ✅ Third-party library initialization
  ✅ Network requests (but prefer React Query / data libraries)
  ✅ Reading from external APIs (localStorage, sensors)

useEffect is NOT for:
  ❌ Computing derived values from props/state (use inline or useMemo)
  ❌ Syncing state between two state variables (single source of truth)
  ❌ Responding to user events (put logic in event handlers)
  ❌ Notifying parent components of state changes (call in handler)
  ❌ Resetting state when props change (derive from props directly)
...
```
User clicks a button → the response goes in the CLICK HANDLER
...
3. useEffect fires → setFilteredItems(items.filter...)  → re-render #2
4. User types 'app'
...
1. User types 'ap'
2. setQuery('ap')  → re-render #1
3. const filteredItems = items.filter(...)  → 0 extra renders
```
...
Without effect (inline computation):
  ...
  Render reads: const derivedValue = computeFromState(state)
...
With effect (setState inside useEffect):
  ...
  If multiple effects chain: render #1, effect 1 → render #2,
    effect 2 → render #3, ...
...
function ProductPage({ productId, onAddToCart }) {
  // Use React Query: handles fetch, loading, error, cache, race conditions
