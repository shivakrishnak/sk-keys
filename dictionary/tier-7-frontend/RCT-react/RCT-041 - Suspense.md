...
4. Promise resolves:
   - React re-renders the subtree (from the suspended component)
   - Data is available (cached by the suspense-aware library)
...
Suspense (declarative):
  ...
  - Requires library support (cannot use raw fetch with useState)
```
