...
```
Exact nearest neighbor in 1536 dimensions: O(N × 1536) for N vectors
With 1 million vectors: 1.5 billion float comparisons per query → too slow
...
HNSW: approximate, but sub-linear:
  ...
  Search algorithm:
    ...
    Descend to layer 0: final precision search among neighbors

  Result: O(log N) hops to find approximate nearest neighbors
  Accuracy vs. speed: ef_construction, M parameters
    High ef: more accurate, slower
    Low ef: less accurate, faster

  Memory: each node stores edges → more memory than flat index
          1M vectors × 1536 dims = 6GB; HNSW index overhead: +2GB typical
```
...
vectors = [
    PointStruct(
        ...
        payload={"title": "Microservices Latency", "url": "https://example.com/1"}
    ),
    PointStruct(
        id=2,
        vector=embed("High-latency sensor data collection systems"),
        payload={"title": "IoT Sensors", "url": "https://example.com/2"}
    )
]
...
results = client.search(
    ...
    query_filter={"must": [{"key": "type", "match": {"value": "technical"}}]}  # optional filter
)
for r in results:
...
→ Return chunks: [HNSW params explanation, graph search theory, ...]
→ LLM prompt: "Based on [chunks], answer [query]"
→ GPT-4: "M controls neighborhood size; higher M = better recall but more memory..."
