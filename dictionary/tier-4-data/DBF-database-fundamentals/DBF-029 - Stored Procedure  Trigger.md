...
# DBF-044 - Stored Procedure  Trigger
...
BEGIN
    -- Lock both accounts (prevent deadlock: always lock lower ID first)
    ...
    INSERT INTO transfer_audit (from_acct, to_acct, amount, executed_at)
...
→ INSERT INTO products_audit (old_price=12.99, new_price=9.99, changed_by=app_user)
...
Fix: Change to FOR EACH STATEMENT trigger for bulk logging
     OR: handle bulk operations with a separate audit approach
     OR: disable trigger during bulk maintenance, audit separately
```
