...
Let: Y = true label, Y_hat = predicted label
     A = protected attribute (e.g., gender: 0=male, 1=female)

DEMOGRAPHIC PARITY (statistical parity):
  P(Y_hat=1 | A=0) = P(Y_hat=1 | A=1)
  "Both groups receive positive predictions at equal rates"
  USE: when base rates are equal or shouldn't affect outcome
...
EQUALIZED ODDS (Hardt et al. 2016):
  ...
  "Both true positive rate AND false positive rate are equal
   across groups"
  USE: hiring, lending - want equal opportunity + equal error rates
  PROBLEM: reduces overall accuracy when base rates differ

PREDICTIVE PARITY (calibration):
  P(Y=1 | Y_hat=1, A=0) = P(Y=1 | Y_hat=1, A=1)
  "When the model says positive, it's equally right for all groups"
  USE: medical risk scoring, recidivism - want calibrated probs
...
Training
    ...
    → Or: adversarial debiasing (separate debiasing network)
...
│              │ protected attributes while leaving proxies│
├──────────────┼───────────────────────────────────────────┤
