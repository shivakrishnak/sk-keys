...
2. FILE SYSTEM ORDERING:
   ...
   - Two builds on different filesystems → different JAR entry order
...
5. SNAPSHOT DEPENDENCIES:
   - Dependency version resolves to different artifact each run
...
# Verify JAR entries:
jar tf build/libs/my-app.jar | sort  # sorted = reproducible entry order
unzip -p build/libs/my-app.jar META-INF/MANIFEST.MF  # check for timestamps
```
...
tasks.withType<AbstractArchiveTask>().configureEach {
    isPreserveFileTimestamps = false    // no timestamps in ZIP entries
...
# Shows exactly what changed: file entries, timestamps, bytecode differences
```
...
├──────────────┼───────────────────────────────────────────┤
│ KEY RULES    │ No SNAPSHOTs, no timestamps, pin toolchain│
├──────────────┼───────────────────────────────────────────┤
