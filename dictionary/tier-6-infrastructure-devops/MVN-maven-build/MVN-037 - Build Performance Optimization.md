...
module-A [===20s===]
module-B [===20s===]  (all run simultaneously if no dependency between them)
module-C [===20s===]
                                                                     = 20s
```
...
Developer (same inputs): → lookup remote cache → restore  = 1–2s download
```
...
./gradlew build \
  ...
  --configuration-cache \    # reuse task graph
  --no-daemon \              # CI: disable daemon (containers exit after build)
