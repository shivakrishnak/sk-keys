...
Scaling policies:
  Target Tracking:  maintain metric (CPU=70%)  - recommended
  Step Scaling:     add N instances when CPU > 80%
  Scheduled:        8am add 5 instances, 6pm remove 5

Health checks:
  EC2:  instance responds to status check
  ALB:  target group health check (more thorough; recommended)

Launch Template: defines instance config (AMI, type, SG, user data)
```
...
resource "aws_autoscaling_group" "app" {
  ...
  health_check_grace_period = 300     # wait 5min before checking health
