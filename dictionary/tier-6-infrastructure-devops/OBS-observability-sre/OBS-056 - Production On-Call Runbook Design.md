...
nav_order: 56
permalink: /observability-sre/production-on-call-runbook-design/
---

# OBS-056 - Production On-Call Runbook Design
...
Anti-pattern 1: Missing prerequisite context
  ...
  GOOD: "Open the checkout-db Grafana dashboard
         ...
         Look at panel 'Connection Pool Active Connections'.
         Alert threshold: > 80 active connections (pool max: 100).
         If > 80: proceed to step 4 (pool exhaustion procedure)."
...
Anti-pattern 2: Ambiguous commands
  ...
  GOOD: "If all other checks are inconclusive,
         ...
         Monitor rollout: kubectl rollout status deployment/checkout
         Expected: completion in < 3 minutes with 0 errors."
...
Anti-pattern 3: Stale information
  ...
  Fix: Quarterly runbook exercise - new engineer runs
       through the runbook in staging, identifies broken steps.
