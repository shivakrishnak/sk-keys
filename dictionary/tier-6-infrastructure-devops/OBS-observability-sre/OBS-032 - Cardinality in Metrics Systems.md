...
nav_order: 32
permalink: /obs/cardinality-in-metrics-systems/
---

# OBS-032 - Cardinality in Metrics Systems
...
Now with user_id label:
  ...
  ...
  http_requests_total{user_id="user-10000000",...}→ series 10M
...
Pattern 2: Label multiplication
  ...
  CAUSE: adding a new label to an existing high-series metric
...
Pattern 3: Missing label value validation
  ...
  CAUSE: label values include raw user input, error messages,
...
[Incident: 18 minutes]
  ...
  Rollback: revert deployment, drain high-cardinality metrics
  Resolution: Prometheus restarts with low-cardinality metrics
  ↓
[Post-mortem]
  Root cause: unbounded label (user_id) in metric definition
  Action items:
    ...
    4. Add per-metric series limit enforcement (Cortex/Thanos)
```
