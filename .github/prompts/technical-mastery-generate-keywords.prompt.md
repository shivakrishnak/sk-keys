---
agent: agent
description: "Generate keyword entries for a category or tier using Keyword Generator v6.0"
tools:
  - run_in_terminal
  - read_file
...
The full keyword generation specification is in `technical-mastery/_config/MASTERY_OS_PROMPT.md`
(Category Keyword Generator - Master Prompt v6.0). Apply it exactly.

> **CRITICAL RULE - NON-NEGOTIABLE:**
> `TECHNICAL_MASTERY_LIST.md` MAY be read to understand the topics that
> the target category covers and to check for existing ID conflicts.
> It MUST NOT be used as the source to generate new keywords.
> All new keywords are generated using `MASTERY_OS_PROMPT.md`
> against the category's own `index.md`. `TECHNICAL_MASTERY_LIST.md`
> is updated in Phase 6 ONLY AFTER keyword generation is complete.

Keep the workflow linear and scoped: validate the target, scan the current
category state, generate only missing keywords, sync `index.md`, create
stub files, then update TECHNICAL_MASTERY_LIST.md last.
...
**Path B — Entire tier:** List all category folders under `technical-mastery/{tier}/`,
...
from the Category Code Registry in `.github/instructions/technical-mastery.instructions.md`:
...
| INDEX_PATH    | technical-mastery/TIER/FOLDER/index.md  |
...
```
Read: technical-mastery/{TIER}/{FOLDER}/index.md
```
...
```
List: technical-mastery/{TIER}/{FOLDER}/*.md (excluding index.md)
```
...
Using `technical-mastery/_config/MASTERY_OS_PROMPT.md` v6.0 specification:
...
3. Apply ALL 33 rules from Section 2
4. Use ALL 14 output components from Section 3
5. Run ALL 28 quality checks from Section 4
...
> **NON-NEGOTIABLE SAFETY RULES** (from Section 3.10 of v6.0 spec).
...
- `parent:` MUST be exactly `"Technical Mastery"`
...
- `grand_parent:` MUST be exactly `"Technical Mastery"`
...
1. Find the category's section (e.g. `#### 📐 CS Fundamentals - Paradigms & Theory`)
2. Append new rows to the bottom of that section's table, preserving format:
   `| NNN | Keyword Name | ★☆☆ |`
3. Update the section header ID range: `(001–NNN)` → new highest NNN
4. Do NOT modify any existing rows - only append
...
FILES MODIFIED:
  - technical-mastery/[tier]/[FOLDER]/index.md  (updated)
  - technical-mastery/[tier]/[FOLDER]/CODE-NNN - Name.md  (created)
  ...
...
Full content quality spec: `technical-mastery/_config/ENTRY_GENERATOR_PROMPT.md` Section 7.
...
1. List all category folders in `technical-mastery/{tier}/`
...
Scans JVM-001 through JVM-066, identifies gaps per v6.0 rules,
