---
mode: agent
description: Upgrade sk-keys dictionary entries to v3.0 — scaffold + fill content
---

# Upgrade Batch — sk-keys Technical Dictionary

You are upgrading sk-keys Technical Dictionary entries to **v3.0 standard**.
...
This shows which files in the range are not yet v3.0 complete.
...
This writes each file with correct YAML + all 22 section stubs.
Only files that are NOT already v3.0 complete need content filled in.
...
2. Replace ALL `[FILL:...]` stubs with full v3.0 content per spec
3. Follow copilot-instructions.md rules exactly:
   ...
   - 3 Think questions (types A/B/C/D/E/F), each with `*Hint:*`
...
All files in the batch should now show `v3.0 / complete`.
...
git commit -m "upgrade: ->v3.0 {{CODE}}-{{START}}-{{CODE}}-{{END}} {{CATEGORY_NAME}} - batch {{BATCH_N}}"
```
