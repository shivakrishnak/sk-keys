...
# 1171 â€” Postman / REST Assured

âš¡ TL;DR â€” Postman is a GUI tool for manual and automated API testing via collections; REST Assured is a Java DSL for fluent HTTP API assertions in code â€” both are the primary tools for API test automation at different levels of the stack.

| #1171           | Category: Testing                                             | Difficulty: â˜…â˜…â˜† |
| :-------------- | :------------------------------------------------------------ | :-------------- |
...
> Postman is the **workshop for building and testing API requests** â€” you tinker interactively until the request is right, then save it as a recipe. REST Assured is the **factory floor** â€” you codify the recipes as automated tests that run at scale, every build, without a human.
...
COLLECTION STRUCTURE:
  ...
  â””â”€â”€ Folder: "Orders"
      ...
      â”œâ”€â”€ GET Get Order by ID
      â”œâ”€â”€ PATCH Update Order Status
...
POSTMAN vs REST ASSURED â€” WHEN TO USE WHICH:

```
POSTMAN:
  âœ“ API exploration and development
  âœ“ QA team (non-Java developers)
  âœ“ Manual API verification
  âœ“ Sharing API tests across teams (collection export)
  âœ“ When tests should run against any environment
  âœ— Version control integration is clunky (JSON files)
  âœ— Less expressive than code for complex scenarios

REST ASSURED:
  âœ“ Java projects (integrates natively with Maven/Gradle)
  âœ“ Complex test scenarios requiring Java logic
  âœ“ Integrated with JUnit test suite
  âœ“ Better version control (code, not JSON)
  âœ“ Can use Java test utilities (Testcontainers, Spring Test)
  âœ— Requires Java knowledge
  âœ— Less visual than Postman
```
...
QA Engineer workflow:
  ...
  7. Newman output: JUnit XML â†’ CI picks up test results

Result: Manual Postman testing converted to automated CI pipeline
         with minimal developer overhead.

Limitation: Collection JSON in git â†’ merge conflicts are painful
             â†’ large teams often migrate to REST Assured as codebase matures
```
...
**Level 1:** Postman: send requests visually, add test scripts, save as collection, export for CI. REST Assured: `given().when().then()` fluent Java API â€” matches natural language ("given these headers, when I call this endpoint, then expect this response").
...
**Level 4:** Postman at enterprise: API Governance â€” shared team collections, centralized test environments, API versioning in collections, integration with CI/CD via Newman. REST Assured in a microservices context: shared test utility library containing `RequestSpecBuilder` configurations, authentication helpers, and common response validators â€” used across all service test suites for consistency. Integration with contract testing: REST Assured tests serve as the consumer-side tests in Pact, generating pact files from REST Assured requests.

### ðŸ’» Code Example

```java
// REST Assured â€” complete integration test example
...
Cause: Multiple developers editing the same collection â€” JSON format creates large diffs.
...
â”‚ POSTMAN      â”‚ GUI exploration â†’ Collections â†’          â”‚
â”‚              â”‚ Newman CLI â†’ CI pipeline                 â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ REST ASSURED â”‚ given().when().then() Java DSL           â”‚
â”‚              â”‚ â†’ integrated with JUnit, Maven           â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ CHOOSE       â”‚ Postman: QA/exploration; REST Assured:   â”‚
â”‚              â”‚ Java developer integration tests         â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ ONE-LINER    â”‚ "Postman for humans exploring APIs;      â”‚
â”‚              â”‚  REST Assured for code automating them"  â”‚
...
**Q1.** Postman's JavaScript test sandbox runs in a Node.js-like environment with access to `pm.*` APIs. Describe: (1) the `pm.sendRequest()` function â€” used within pre-request scripts to obtain an auth token and store it as an environment variable before each request runs, enabling fully automated OAuth 2.0 flows in Postman collections, (2) Postman's data-driven testing with CSV/JSON files â€” running the same request multiple times with different input values (like JUnit's `@ParameterizedTest`), (3) the Postman Flows visual builder (newer feature) for orchestrating complex API sequences with conditional logic, and (4) the corporate governance concern: Postman collections contain API credentials and base URLs â€” describe the security risk and how to mitigate it (never commit collections with real credentials; use environment variables; use Postman Vault for secrets).

**Q2.** REST Assured's JSON path is based on Groovy's GPath syntax. Describe: (1) the basic path syntax â€” `body("user.address.city", equalTo("London"))`, `body("items[0].price", greaterThan(9.99f))`, `body("items.size()", equalTo(3))`, (2) advanced GPath â€” `body("items.findAll { it.status == 'ACTIVE' }.size()", equalTo(2))` (filter a list), `body("items.collect { it.price }.sum()", equalTo(29.97f))` (aggregate), (3) why these Groovy closures work in a Java library (REST Assured delegates JSON path evaluation to the Groovy runtime embedded as a dependency), (4) the XML path alternative for SOAP/XML APIs using Hamcrest matchers, and (5) the `extract().as(MyClass.class)` pattern â€” deserializing the response body directly to a Java POJO using Jackson for more expressive assertions with `assertThat(order.getStatus()).isEqualTo(PENDING)`.
