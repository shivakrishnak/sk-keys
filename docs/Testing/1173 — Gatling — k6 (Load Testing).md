...
# 1173 â€” Gatling / k6 (Load Testing)

âš¡ TL;DR â€” Gatling and k6 are load testing tools that simulate thousands of concurrent users against your application, measuring response times, throughput, and error rates to validate performance requirements and find breaking points.

| #1173           | Category: Testing                                                    | Difficulty: â˜…â˜…â˜… |
| :-------------- | :------------------------------------------------------------------- | :-------------- |
...
STRESS TEST (beyond peak):
  ...
  E.g.: 5k â†’ 10k â†’ 20k â†’ 50k users. At what point do errors appear?
...
SPIKE TEST (sudden surge):
  ...
  E.g.: Marketing email sends â€” instant traffic spike
```
...
export const options = {
  stages: [
    { duration: "2m", target: 100 }, // ramp up: 0 â†’ 100 users
    { duration: "5m", target: 100 }, // steady: 100 users for 5 min
    { duration: "2m", target: 500 }, // spike: 100 â†’ 500 users
...
http_req_duration p(50) = 85ms    â†’ median response time
http_req_duration p(95) = 342ms   â†’ 95% of requests faster than this
http_req_duration p(99) = 891ms   â†’ 99% of requests faster than this
http_req_failed = 0.2%            â†’ error rate (should be < 1%)
http_reqs = 8,450/s               â†’ throughput (requests per second)

RED FLAGS:
  p99 >> p95 â†’ outliers present (GC pauses? DB lock waits?)
  Error rate climbing â†’ approaching breaking point
  Response time increasing under sustained load â†’ memory leak or connection pool exhaustion
  p95 OK but p99 spikes â†’ occasional slow queries (missing index? lock contention?)
```
...
Short load test (5 minutes): passes. p95 = 200ms. âœ“
...
Root cause: connection leak.
  ...
  Over 4 hours: 50,000 requests Ã— 0.1% = 50 leaked connections = pool exhausted.

Fix: try-with-resources / proper connection management.

Lesson: soak tests find what load tests miss â€” gradual resource exhaustion.
```
...
**Level 3:** Performance SLOs as test thresholds: p95 < 500ms, error rate < 0.1%, throughput > 1000 req/s. These become CI gates: if performance degrades beyond threshold â†’ fail the pipeline. Live metrics with Grafana: k6 can stream metrics to InfluxDB or Prometheus â†’ Grafana dashboard shows real-time percentiles, throughput, and error rates during the test.
...
| "Passing load test = ready for production"        | If staging â‰  production (different DB size, different caching), results don't transfer         |
| "1,000 VUs = 1,000 concurrent requests"           | VUs include think time (sleep); actual concurrent requests = VUs Ã— (request_time / cycle_time) |

### ðŸš¨ Failure Modes & Diagnosis

**1. Load Test Doesn't Represent Real Traffic**
Cause: All virtual users request the same endpoint with same data â†’ cache artificially inflates performance.
...
Cause: All VUs start simultaneously (`atOnceUsers(1000)`) â†’ 1000 cold-start connections at once.
Result: Database connection pool exhaustion at start, not under steady load.
Fix: Gradual ramp-up (`rampUsers(1000).during(60)`) â€” allows connection pool to fill gradually.
...
â”‚ WHAT         â”‚ Simulate N concurrent users; measure     â”‚
â”‚              â”‚ p95/p99 response time, error rate        â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ TEST TYPES   â”‚ Load (expected peak) / Stress (breaking  â”‚
â”‚              â”‚ point) / Soak (8h+ for leaks)            â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ k6 KEY       â”‚ stages (ramp) + thresholds (SLO gates)  â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ METRICS      â”‚ p50/p95/p99 latency, RPS throughput,     â”‚
â”‚              â”‚ error rate, active VUs over time         â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ ONE-LINER    â”‚ "Discover your capacity before Black     â”‚
â”‚              â”‚  Friday does â€” on your terms"            â”‚
...
**Q1.** Percentile-based performance metrics (p50, p95, p99) are the industry standard for characterizing response time distributions. Describe: (1) why averages are misleading for response time (a 50ms average can mask a 10% of requests taking 5 seconds â€” 1 in 10 users has a terrible experience), (2) the "long tail" problem in distributed systems â€” how independent service calls compound: if 3 services each have p99=200ms, the p99 of the combined call is NOT 200ms (it's the probability that at least one of the 3 is slow), (3) the Apdex score â€” a standardized satisfaction metric (T=target, F=frustration threshold; satisfied=response<T, tolerating=T<response<4T, frustrated=response>4T; Apdex=(satisfied+0.5Ã—tolerating)/total), and (4) why load testing with percentile thresholds at the p95 or p99 level is appropriate for SLO definition ("99% of users experience < 500ms" corresponds directly to p99 < 500ms).

**Q2.** k6's JavaScript runtime is NOT Node.js â€” it's Goja, a Go-based JavaScript engine. This has important implications for load test script writing: (1) you cannot use `node_modules` directly (only k6's built-in modules and xk6 extensions), (2) `async/await` is NOT the way k6 runs â€” k6 uses synchronous blocking HTTP calls (each `http.get()` blocks the VU until response arrives, which is intentional â€” each VU represents one user's sequential journey), (3) how k6's execution model differs from Node.js (k6 runs `default function()` for each VU in a goroutine, not a single event loop), (4) the initialization context (code outside `default function`) vs. VU context (code inside `default function`) â€” initialization runs once per VU; VU code runs for each iteration, and (5) k6 Cloud vs. self-hosted: when to use distributed k6 (when local machine can't generate enough load â€” typically > 10k RPS), cost considerations, and the test data privacy implications (k6 Cloud receives all HTTP request/response data).
