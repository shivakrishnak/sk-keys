...
# DBF-033 — Foreign Key / Referential Integrity
...
→ OR: app first moves orders to archive, then deletes customer
...
→ FK constraint re-enabled (or never re-enabled if forgotten)
→ 50,000 orphaned order_items with invalid product references
