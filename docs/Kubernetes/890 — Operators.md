...
parent: "Kubernetes"
nav_order: 890
permalink: /kubernetes/operators/
number: "0890"
category: "Kubernetes"
difficulty: "★★★"
depends_on:
...
Ongoing:
  → Primary crashes → operator promotes my-postgres-1 → updates status
  → User sets spec.replicas=5 → operator scales StatefulSet
  → CronJob triggers backup → operator records lastBackup in status
```
