...
Endpoints: [10.244.1.5:8080, 10.244.2.8:8080, 10.244.3.2:8080]
...
kube-proxy iptables rules (on every node):
  ...
  -A KUBE-SVC-MY-APP -m statistic --mode random --probability 0.333 \
    -j KUBE-SEP-POD1   # → DNAT to 10.244.1.5:8080
  -A KUBE-SVC-MY-APP -m statistic --mode random --probability 0.5 \
...
Pod "my-app-abc" fails readiness probe
  → Endpoints controller removes PodIP from Endpoints object
  → API Server: Endpoints updated
  → kube-proxy watch fires on all nodes
  → kube-proxy: regenerate iptables rules (remove KUBE-SEP-ABC)
