...
Limits:
  ...
  Deployment size:     50MB zip, 250MB unzipped, 10GB container
...
Cold start:
  First invocation (or after idle): download + init code = extra latency
...
public class OrderProcessorApplication {
    ...
    // Lambda initialization code (runs once per cold start, reused on warm invoke)
