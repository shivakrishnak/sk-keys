...
nav_order: 52
permalink: /technical-mastery/databases/materialized-view/
id: DBF-057
...
-- Partial refresh via view structure (
    only viable if query is designed for it):
-- Cannot refresh individual rows of a materialized view in PostgreSQL
-- → Must refresh the whole view or use incremental approaches (
    triggers/CDC)
```
...
→ Diffs new vs. existing (using unique index on
  month+category)
...
DECLARE mv TEXT;
BEGIN
    FOR mv IN SELECT matviewname FROM pg_matviews WHERE schemaname =
        'public'
    LOOP
