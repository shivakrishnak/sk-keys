...
nav_order: 26
permalink: /technical-mastery/databases/connection-pooling/
id: DBF-014
...
spring:
  datasource:
    hikari:
      ...
      # 30min: retire connections (before DB-side timeout)
...
Guideline: more connections ≠ more throughput
           Too many connections = CPU thrashing, context
             switching
...
→ Database: always sees pool_size connections (not
  per-request)
```
...
→ SQLException: Connection is not available, request timed
  out
