...
nav_order: 28
permalink: /technical-mastery/databases/read-replica/
id: DBF-028
...
# (or use sticky session / primary for the same user's reads after
# write)
def add_to_cart(user_id, product_id):
    primary_db.execute("INSERT INTO cart ...")  # write → primary
    return primary_db.execute("SELECT * FROM cart ...")
    # read same user's cart from primary
...
-- (hot_standby_feedback =
    on means queries on replica delay cleanup on primary)
