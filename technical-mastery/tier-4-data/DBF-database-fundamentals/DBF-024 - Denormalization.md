...
nav_order: 24
permalink: /technical-mastery/databases/denormalization/
id: DBF-007
...
```sql
-- Before: SELECT AVG(rating),
    COUNT(*) FROM reviews WHERE product_id=42
-- After: products(id, name, avg_rating, review_count)
-- Obligation: UPDATE products SET avg_rating,
    review_count on every review INSERT/UPDATE/DELETE
```
...
--             product_id, product_name, category_name, amount,
    order_date)
...
```sql
-- Table: org_weekly_active_users(org_id, week_start,
    active_user_count)
