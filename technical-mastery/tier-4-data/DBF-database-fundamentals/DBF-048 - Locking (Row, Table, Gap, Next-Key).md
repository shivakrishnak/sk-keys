...
nav_order: 48
permalink: /technical-mastery/databases/locking/
id: DBF-020
...
T2: SELECT ... WHERE status='ACTIVE' FOR UPDATE →
  overlapping gap lock
...
JOIN pg_catalog.pg_stat_activity blocked_query ON blocked_query.pid =
    blocked_locks.pid
...
├──────────────┼──────────────────────────────────────────┤
│ KEY PATTERN  │ SELECT ... FOR UPDATE for check-then-writ│
