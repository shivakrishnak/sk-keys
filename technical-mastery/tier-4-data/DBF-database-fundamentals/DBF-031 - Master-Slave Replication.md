...
nav_order: 31
permalink: /technical-mastery/databases/master-slave-replication/
id: DBF-051
...
(Reduces WAL sender load on primary; REPLICA 1 fans out to
  others)
```
...
binlog_format =
    ROW  -- recommended: row-level changes (most consistent)
...
```
Primary fails
→ Automated detection (Patroni heartbeat, MHA, AWS health
  check)
...
→ Old primary recovered:
   Rejoin as replica (PITR recovery to sync point +
     replicate forward)
```
...
→ Other replicas: primary_conninfo updated → follow new
  primary
→ Total: ~30-60 seconds
```
...
Check: etcd/ZooKeeper leader key - which node holds the
  leader lock?
