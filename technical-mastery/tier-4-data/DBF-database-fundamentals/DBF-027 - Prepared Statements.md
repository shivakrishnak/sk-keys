...
nav_order: 27
permalink: /technical-mastery/databases/prepared-statements/
id: DBF-027
...
// parameter 1: safe, not interpreted as SQL
...
→ [PREPARED STATEMENT ← YOU ARE HERE: separate SQL from
  data]
...
App queries a statement 6+ times → JDBC creates
  server-side prepared stmt
pgBouncer in transaction mode reassigns connection to
  different backend
New backend doesn't have the prepared statement
→ ERROR: prepared statement "S_1" does not exist
→ Application crashes with mysterious "prepared statement
  not found"
→ Fix: set prepareThreshold=-1 (disable server-side) OR
       use pgBouncer session mode OR
       use PgBouncer 1.21+ with protocol-aware prepared
         statement tracking
```
...
private static final Set<String> ALLOWED_SORT_COLS =
...
→ Check pgBouncer mode: pgbouncer.ini → pool_mode =
  transaction
→ Check JDBC URL: prepareThreshold not set (default 5) or
  set to >0
```
...
```
Option 1: Set JDBC prepareThreshold=0 (disable server-side
  prepared stmts)
          JDBC URL:
...
Option 2: Switch pgBouncer to session mode
          pool_mode = session
          → Preserves session state but reduces connection
            reuse efficiency

Option 3: Use PgBouncer 1.21+ (supports tracking prepared
  statements)
```
