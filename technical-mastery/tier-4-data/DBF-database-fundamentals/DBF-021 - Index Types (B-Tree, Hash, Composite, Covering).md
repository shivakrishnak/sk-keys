...
nav_order: 21
permalink: /technical-mastery/databases/index-types/
id: DBF-036
...
→ Composite covering index (customer_id, status) INCLUDE
  (amount)
...
→ Planner cannot efficiently use status-first index for
  customer_id=42
→ Falls back to sequential scan or inefficient bitmap scan
→ Fix: create index on (customer_id, status) - most
  selective first
```
...
-- e.g., idx on (customer_id) is redundant if (customer_id,
    status) exists
...
│              │ zero-scan indexes. Composite > many singl│
├──────────────┼──────────────────────────────────────────┤
