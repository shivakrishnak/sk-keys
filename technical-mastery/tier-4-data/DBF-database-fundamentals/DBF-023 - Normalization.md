...
nav_order: 23
permalink: /technical-mastery/databases/normalization/
id: DBF-006
...
```
order_id | product_id | product_name | quantity  ← PK:
  (order_id, product_id)
1        | 101        | Laptop       | 2
```
...
```
employee_id | department_id | department_name  ← PK:
  employee_id
1           | 10            | Engineering
```
...
student_id | student_name | course_id | course_name |
  professor_id | professor_name | room_id | room_capacity
```
...
courses(course_id, course_name, professor_id FK, room_id
  FK)
...
```
Denormalized schema in production (customer address in
  orders)
→ Customer address change requires UPDATE on all orders
→ Partial update (application bug) → some rows have old
  address
→ Data inconsistency: same customer has two different
  cities
