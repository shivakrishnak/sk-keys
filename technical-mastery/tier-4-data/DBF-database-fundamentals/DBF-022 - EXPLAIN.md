...
nav_order: 22
permalink: /technical-mastery/databases/explain/
id: DBF-037
...
EXPLAIN ANALYZE query            -- Execute + plan + timings +
    actual rows
EXPLAIN (ANALYZE, BUFFERS) query -- + cache hits/misses per node
EXPLAIN (ANALYZE,
    FORMAT JSON)   -- Machine-readable; use with explain.depesz.com
...
```
Index Scan using idx_customer on orders  (cost=0.43..12.45
  rows=5 width=64)
                                          (actual
                                            time=0.089..0.2
...
→ Seq Scan on large table → check selectivity → add index
  if <5%
→ Buffers: shared read high → cold data; check buffer pool
  size
→ Loops=N with high cost → inner loop is bottleneck; check
  inner table index
...
→ But developer sees "Seq Scan" and thinks it's still
  broken
...
--   -> Index Scan inner_table  (
    actual time=0.5..0.5 rows=1 loops=50000)
...
```
Hash Join (cost=2000..150000 rows=50000) (actual
  time=450..8900 rows=12 loops=1)
  Hash Cond: (a.id = b.a_id)
  Buffers: shared hit=200 read=15000
  -> Seq Scan on table_a (cost=0..8000 rows=400000)
    (actual rows=400000)
