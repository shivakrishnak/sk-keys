...
nav_order: 12
permalink: /technical-mastery/data-fundamentals/columnar-vs-row-storage/
id: DAT-009
...
department column: [ENG][HR][ENG]...               → 2 MB
  (dict-encod.)
```
...
Bytes used:    6 pairs × 8 bytes = 48 bytes vs 6 × 4 = 24
  raw
               → RLE wins only when runs are long; dict
                 better here

Dictionary encoding:
  Dict: {0: 78000, 1: 85000, 2: 92000}
  Values: [1, 1, 1, 2, 2, 0]
  → 6 bytes (1 byte each) + 3 × 4 byte dict = 18 bytes (vs
    24)
```
