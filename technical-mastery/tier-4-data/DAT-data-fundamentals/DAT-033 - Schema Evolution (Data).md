...
nav_order: 33
permalink: /technical-mastery/data-fundamentals/schema-evolution/
id: DAT-039
...
Reader schema (v2):
  {name: "order_id", type: "long"}
  {name: "amount",   type: "double"}
  {name: "currency", type: ["null","string"], default:
    null}
...
Result: currency=null (from default), order_id and amount
  decoded normally.
```
...
for f1, f2 in zip(schema_v1["fields"], schema_v2["fields"]):
    if f1["type"] != f2["type"]:
        print(
            f"TYPE CHANGE: {f1['name']}: {f1['type']} → {f2['type']}")
```
...
curl -X POST \
  ...
  -d '{"schema": "'"$(
      curl http://registry:8081/subjects/topic-value/versions/9)"'"}'
