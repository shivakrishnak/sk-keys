...
nav_order: 26
permalink: /technical-mastery/data-fundamentals/etl-vs-elt/
id: DAT-016
...
Source → Extract → [Transform: ETL Server/Spark/Python]
      → Load: only clean data → Destination
        (warehouse/store)
```
...
**ELT Architecture:**
```
Source → Extract → Load: raw data → Destination
  (lake/warehouse)
      → [Transform: SQL/dbt/Spark running inside
        destination]
