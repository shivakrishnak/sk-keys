...
nav_order: 19
permalink: /technical-mastery/data-fundamentals/star-schema/
id: DAT-023
...
```
fact_sales (1B rows)
  JOIN dim_product ON product_key    -- 500K rows (in
    memory)
  JOIN dim_customer ON customer_key  -- 10M rows (in
    memory)
  JOIN dim_date ON date_key          -- 3,652 rows (in
    memory)
...
```
┌─────────────────────────────────────────────────────────┐
│ WHAT IT IS   │ Central fact table + surrounding dimensio│
