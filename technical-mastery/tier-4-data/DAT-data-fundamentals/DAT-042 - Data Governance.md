...
nav_order: 42
permalink: /technical-mastery/data-fundamentals/data-governance/
id: DAT-020
...
**NORMAL FLOW:**
```
New dataset created → [GOVERNED REGISTRATION ← YOU ARE
  HERE]
→ catalog registration → classification → policy assignment
→ access control configured → audit log enabled
→ consumer requests access → owner approves → time-bounded
  grant
...
**FAILURE PATH:**
```
Auto-classification misses a PII column → column
  unprotected
...
│              │ automated controls for data trustworthine│
├──────────────┼──────────────────────────────────────────┤
