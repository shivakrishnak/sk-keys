...
nav_order: 34
permalink: /technical-mastery/data-fundamentals/data-vault/
id: DAT-040
...
```
link_order_customer:
  link_hash_key              | order_hash_key |
    customer_hash_key | load_date
  MD5(order+cust hash combo) | MD5("ORD-001") |
    MD5("CUST-001")   | 2024-01-15
```
...
```
sat_customer_crm:
  customer_hash_key | load_date   | end_date   | name  |
    loyalty_tier | record_source
  MD5("CUST-001")   | 2024-01-01  | 2024-06-01 | Alice |
    Silver       | CRM
  MD5("CUST-001")   | 2024-06-01  | NULL       | Alice |
    Gold         | CRM
```
...
FROM hub_customer h
JOIN sat_customer_crm s ON h.customer_hash_key =
    s.customer_hash_key AND s.end_date IS NULL
JOIN link_order_customer l ON h.customer_hash_key =
    l.customer_hash_key
JOIN hub_order ho ON l.order_hash_key = ho.order_hash_key
JOIN sat_order so ON ho.order_hash_key =
    so.order_hash_key AND so.end_date IS NULL
GROUP BY c.name;
