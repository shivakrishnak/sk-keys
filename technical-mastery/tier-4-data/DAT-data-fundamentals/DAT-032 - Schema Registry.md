...
nav_order: 32
permalink: /technical-mastery/data-fundamentals/schema-registry/
id: DAT-018
...
✅ Remove field:
   {order_id: long}
   → allowed: old consumers had amount, new don't;
     forward-compat breaks
   → BACKWARD only: old reader expects amount, but writer
     (v2) won't
     send it - old consumer gets default (0.0) for amount
       → OK
...
❌ Change field type:
   ...
   → REJECTED: order_id was long (8 bytes), now string →
     wire
     incompatible; old reader reads 8 bytes as long, sees
       garbage
```
...
│              │ Kafka topics with compatibility enforceme│
├──────────────┼──────────────────────────────────────────┤
