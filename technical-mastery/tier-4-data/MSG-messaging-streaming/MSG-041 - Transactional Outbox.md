...
nav_order: 41
permalink: /technical-mastery/messaging-streaming/transactional-outbox/
id: MSG-027
...
// curl -X POST http://kafka-connect:8083/connectors -H "Content-Type:
// application/json" -d @connector.json

{
  ...
  "config": {
    ...
    // PostgreSQL publication (created below)
    "slot.name": "order_service_slot",
    // replication slot name (unique)
    ...
    // ^ Routes to the topic value from the 'topic' column in
    // outbox_events

    // Tombstone deletion: delete outbox rows after publishing
    // (optional)
...
-- SELECT pg_create_logical_replication_slot('order_service_slot',
    'pgoutput');
```
...
public class OrderService {
    ...
    public Order placeOrder(CreateOrderRequest request) throws
        ...
        OutboxEvent event = OutboxEvent.builder()
            .topic("order-events")
            // Debezium routes to "order-events" Kafka topic
        ...
        // Debezium reads WAL → captures outbox INSERT → publishes to
        // "order-events" topic
        // Latency from DB commit to Kafka publish: ~1-10ms (WAL
        // capture latency)
...
CREATE TABLE outbox_events (
    ...
    aggregate_type VARCHAR(100) NOT NULL,  -- e.g.,
        "Order" (for routing context)
...
-- Kafka idempotent producer (
    enable.idempotence=true): deduplicates re-published events
```
...
WITHOUT Outbox Router SMT:
  Debezium publishes to: "order-db.public.outbox_events"
    (topic per table)
  Event format: {
    "before": null,
    "after": {"id":"abc", "topic":"order-events",
      "message_key":"123", "payload":"{...}"},
    "op": "c"  // c=create (INSERT)
  }
  Consumer: must parse the nested "after" → extract
    payload → find correct topic
  → messy, couples consumers to outbox schema

WITH Outbox Router SMT
  ("transforms.outbox.route.by.field": "topic"):
  Debezium publishes to: "order-events" (the value of the
    "topic" column)
  ...
  → transparent to consumer (they don't know about outbox
    table)

Example:
  INSERT INTO outbox_events (topic='order-events',
    message_key='123', payload='{"orderId":"123",...}')

  Debezium WAL capture → SMT → publish to Kafka topic
    "order-events":
    Key: "123"
  ...
  NotificationService
    ...
    receives: {"orderId":"123", "amount":50,
      "userId":"u456"}
...
Flow:
  ...
  2. DB transaction: INSERT orders + INSERT outbox_events
    [same TX]
  ...
  6. NotificationService (groupId=notifications):
     ...
     Idempotency check (processedMessages table,
       key=orderId)
     sendEmail() → ACK
...
Failure and recovery:
  ...
  Recovery: Debezium restarts → reads from last WAL offset
    in connect-offsets topic
  Re-captures and re-publishes → idempotent Kafka producer
    deduplicates
  Consumers: already-processed events skipped (idempotency
    check)

  Case: Kafka down during step 5
  Debezium: buffered in connect worker memory (or task
    retried)
  PostgreSQL WAL: retained in replication slot
  Debezium retries until Kafka available → publishes when
    Kafka recovers
...
-- DROP replication slot (
    will lose Debezium position → possible duplicate events):
