...
nav_order: 13
permalink: /technical-mastery/messaging-streaming/rabbitmq/
id: MSG-020
...
public class RabbitMQConfig {
    ...
    public Binding errorLogBinding() {
        return BindingBuilder.bind(errorLogsQueue())
            ...
            .with("logs.*.error");
            // matches: logs.app.error, logs.db.error
    }
    ...
    public Binding allLogBinding() {
        return BindingBuilder.bind(allLogsQueue())
            .to(logsTopicExchange())
            .with("logs.#");
            // matches: logs, logs.info, logs.app.info, logs.a.b.c
    }
    ...
    // SimpleRabbitListenerContainerFactory with manual ack +
    // prefetch:
    @Bean
    public SimpleRabbitListenerContainerFactory rabbitListenerContainerFactory(
        ...
        // prefetchCount=10: consumer fetches 10 unacked messages at a
        // time
        // Fair dispatch: don't send more until previous 10 are acked
        // prefetchCount=1: strictest fairness (send 1, wait for ack,
        // send next)
...
public class OrderProducer {
    ...
    public void publishLogEvent(String service, String level,
        String message) {
    ...
    public void publishNotification(String userId, String message) {
        // Fanout exchange: routing key ignored (broadcast to all
        // queues)
        Map<String, Object> notification = Map.of("userId", userId,
            "message", message);
...
public class OrderConsumer {
    ...
    public void processOrder(Order order, Channel channel,
        ...
        try {
            ...
            // false = ack only this message (not all unacked up to
            // this tag)
            ...
            // true = requeue
            // WARNING: if exception is permanent → infinite requeue
            // loop
            // Use DLX + retry count (x-death header) to limit retries

        } catch (PermanentException e) {
            // Don't requeue: message goes to DLX (dead letter
            // exchange)
...
prefetchCount=1:
  ...
  Consumer 2: gets msg2 → processes (fast, 0.1s) → gets
    msg3, msg4, msg5...
  Fast consumer gets more work: fair dispatch based on
    processing speed
...
3. OrderConsumer @RabbitListener (5 instances,
  prefetch=10):
   Instance 1: picks up order1, order6, order11...
     (round-robin)
...
4. Order3 fails (payment timeout):
   ...
   Dead letter handler: alert ops team, store in
     manual_review DB
...
For audit trail: save orders to DB in @RabbitListener
  before basicAck.
```
...
public void process(Order order, Channel channel,
    ...
    try {
        ...
        List<Map<String, Object>> xDeath = (List<Map<String,
            Object>>) headers.get("x-death");
        long retryCount = xDeath == null ? 0 :
            xDeath.stream().mapToLong(d -> (Long) d.get("count"))
                .sum();
