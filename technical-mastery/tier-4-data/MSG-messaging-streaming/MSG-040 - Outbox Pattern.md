...
nav_order: 40
permalink: /technical-mastery/messaging-streaming/outbox-pattern/
id: MSG-041
...
public Order placeOrder(CreateOrderRequest request) {
    ...
    kafkaTemplate.send("orders", order.getId(),
        new OrderEvent(order));  // Kafka: FAILS?
    ...
    // Scenario B: service crashes after DB commit, before Kafka
    // publish
    //   DB: order saved ✓
    ...
    // Result: ghost events trigger downstream actions for
    // non-existent orders

    return order;
}
// @Transactional wraps only the DB transaction - Kafka send is
// outside it
// Two-phase commit (XA transactions) could solve this but is slow and
// complex
...
CREATE TABLE outbox_events (
    ...
    INDEX idx_outbox_status_created (status,
        created_at)  -- relay query index
);
```
...
public interface OutboxEventRepository extends
    ...
    // FOR UPDATE
    // FOR UPDATE: prevents multiple relay instances from picking up
    // same events
}
...
public class OrderService {
    ...
    public Order placeOrder(CreateOrderRequest request) throws
        ...
        // SINGLE @Transactional: BOTH writes commit together OR both
        // rollback
        // If DB crashes: both rolled back → no inconsistency
        // Kafka not involved in this transaction (no dual-write
        // problem)
...
public class OutboxRelay {
    ...
    @Transactional
    public void relay() {
        // Fetch and lock pending events (pessimistic lock prevents
        ...
        for (OutboxEvent event : pendingEvents) {
            try {
                ...
                CompletableFuture<SendResult<String,
                    String>> future = kafkaTemplate.send(
                    event.getTopic(),
                    event.getMessageKey(),
                    event.getPayload()
                );
                ...
                if (event.getRetryCount() >= 5) {
                    ...
                    alertService.sendAlert("Outbox event stuck: " +
                        event.getId());
                }
            }
        }
...
// Problem: 3 relay instances all poll simultaneously → all pick same
// event → publish 3x

// SOLUTION 1: Pessimistic locking (shown above)
// SELECT FOR UPDATE: only one instance acquires lock → others wait or
// skip
//   Good for: low relay instance count, simple setup

// SOLUTION 2: Kafka Streams processing guarantee (Transactional
...
// Update: SET status='PROCESSING', locked_by=instanceId WHERE
// status='PENDING'
...
public void relay() {
    ...
    List<OutboxEvent> myEvents =
        outboxRepository.findByLockedBy(instanceId);
...
09:00:01 - POST /orders → OrderService.placeOrder()
  TRANSACTION:
    INSERT orders (id='123', status='PLACED',
      amount=50.00) ✓
    INSERT outbox_events (topic='order-events', key='123',
                          payload='{"orderId":"123","amount
                          status='PENDING') ✓
  COMMIT ✓
...
09:00:01.200 - OutboxRelay polls (every 500ms):
  ...
  Kafka ACK received ✓
  UPDATE outbox_events SET status='PUBLISHED' WHERE
    id='abc'
  COMMIT ✓
...
Crash scenario:
  Service crashes at 09:00:01.100 (after DB commit, relay
    not yet run)
  Service restarts
  OutboxRelay: finds PENDING event from before crash
  Publishes event → NotificationService processes
    (idempotency: already processed? NO)
