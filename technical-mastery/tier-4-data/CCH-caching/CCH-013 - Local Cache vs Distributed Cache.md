...
nav_order: 13
permalink: /technical-mastery/caching/local-vs-distributed/
id: CCH-033
category: Caching
difficulty: ★★☆
...
public class LocalCacheConfig {
    ...
    public Cache<String, Boolean> featureFlagCache() {
        return Caffeine.newBuilder()
            ...
            // Refresh every 30s (short TTL for config)
            .refreshAfterWrite(20, TimeUnit.SECONDS)
            // Stale-while-revalidate: serve stale,
                                                     // refresh
                                                     // asynchronously
                                                     // after 20s
            .buildAsync(key -> featureFlagRepository.load(key))
            // Async reloader
            .synchronous();
            // Return sync LoadingCache
    }
    ...
    public Cache<String, UserPreferences> userPreferencesCache() {
        return Caffeine.newBuilder()
            .maximumSize(1000)
            // Top 1000 users' preferences
            .expireAfterWrite(5, TimeUnit.MINUTES)
            // User preferences: 5-min stale OK
            .expireAfterAccess(1, TimeUnit.MINUTES)
            // Evict if not accessed in 1 min
            .recordStats()
            .build();
    }
}
...
public class FeatureFlagService {
    ...
    @Autowired @Qualifier("featureFlagCache") private Cache<String,
        Boolean> flagCache;

    public boolean isFeatureEnabled(String flag, String userId) {
        // In-process: no Redis, no network, ~0.01ms
        return flagCache.get(flag,
            k -> featureFlagRepository.load(k));
    }
}
// 100,000 flag checks/second: 100,000 × 0.01ms = 1 CPU second
// (manageable)
...
// Write to Redis from Instance 5 → all 20 instances can read fresh
...
public class UserService {
    ...
    public User getUser(String userId) {
        ...
        // If Instance 5 caches user:42 → all other instances get
    ...
    public User updateUser(String userId, UserUpdateRequest req) {
        // @CacheEvict: deletes from Redis (shared) → ALL 20 instances
        // now get MISS
...
// Any instance updates → DEL in shared Redis → all see fresh data
// next request.
...
// User B (routed to Instance 7): sees OLD email for up to 5 minutes
// (TTL)
// INCONSISTENCY: same user, different answers from different
// instances ✗

// ---- DISTRIBUTED CACHE ONLY ----
// Instance 5: updates DB + Redis DEL "user:42" (shared Redis)
// All instances: next GET "user:42" → Redis MISS → DB → Redis SET →
// cached
// User B (routed to any instance): sees NEW email on next request ✓
// CONSISTENT: all instances serve same cached data (from shared
// Redis)
...
// All instances: L1 = empty, L2 = empty → next read: L1 miss → L2
// miss → DB → fresh ✓
...
Start → "Is this a single-instance deployment?"
  YES → Local cache is fine (Caffeine, no consistency
    concern)
...
→ "Is latency critical (hundreds of thousands of
  reads/second)?"
  NO → Distributed cache (Redis) only - simplest,
    consistent
  YES → Hybrid L1+L2:
...
Examples:
  ...
  User profile:  distributed Redis ← consistent across 20
    instances
  Rate limits:   distributed Redis ONLY ← must be shared
  Product catalog (50M products, 80% top-1K): hybrid L1+L2
  Session tokens: distributed Redis ← must be revocable
    globally
```
...
public class RateLimiter {
    private final Cache<String,
        AtomicLong> counters = Caffeine.newBuilder()
        .expireAfterWrite(1, TimeUnit.MINUTES)
        .build();

    public boolean isAllowed(String userId) {
        AtomicLong count = counters.get(userId,
            k -> new AtomicLong(0));
        return count.incrementAndGet() <= 100;
        // Only counts on THIS instance!
    }
}
...
public class DistributedRateLimiter {
    ...
    public boolean isAllowed(String userId) {
        String key = "rate:" + userId + ":" + minuteBucket();
        // e.g., "rate:42:1234567"
        Long count = redis.opsForValue().increment("rate-limit:" +
            key);
        if (count == 1) {
        ...
        return count <= 100;
        // Shared counter across ALL 20 instances ✓
    }
