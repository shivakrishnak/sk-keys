...
nav_order: 29
permalink: /technical-mastery/caching/cache-warming/
id: CCH-045
category: Caching
difficulty: ★★★
...
// (fired AFTER the application is fully started, before accepting
// traffic
//  IF a readinessProbe is configured to delay traffic until ready)

@Component
public class CacheWarmer implements
    ...
    private void warmCache() {
        ...
        List<String> topProductIds =
            productRepository.findTopViewedProductIds(1000);
        ...
        log.info("Cache warming complete: {} entries in {}ms",
...
// periodSeconds: 5
//
// Pod does NOT receive traffic until readiness probe passes
// (warmed=true)
// Traffic is held by Kubernetes Service until this instance signals
// ready
```
...
```java
// Problem: top 1000 products have TTL=15min. At 15min mark: all
...
public class ScheduledCacheRefresher {
    ...
    public void refreshTopProducts() {
        ...
        List<Product> topProducts =
            productRepository.findTopViewedProducts(1000);

        for (Product product : topProducts) {
            String key = "product:" + product.getId();
            // This SETS the key even if it's still in cache
            // (overwrite = refresh)
            redisTemplate.opsForValue().set(key, product,
                Duration.ofMinutes(15));
        }

        log.info("Refreshed {} top products in cache",
            topProducts.size());
    }
}
// TTL=15min, refresh every 12min → cache always has 3min headroom
// before expiry
...
public void onProductChange(ProductChangeEvent event) {
    if (event.getType() == EventType.CREATED || event.getType(
        ) == EventType.UPDATED) {
...
// Result: any product updated via the write path is instantly
// available in cache
// READ path: almost never hits DB for recently modified products
// Proactive warming: write → cache immediately; not lazy (read → miss
// → DB → cache)
```
...
→ SELECT id FROM products WHERE in_flash_sale=true (→ 500
  IDs)
→ Batch load: SELECT * FROM products WHERE id IN
  (...500...)
...
private void warmCache() {
    ...
    List<String> hotKeys =
        productRepository.findTopViewedProductIds(1000);  // Not all
    ...
    Lists.partition(hotKeys, 100).forEach(batch -> {
        ...
        // Rate limit: 100 entries every 500ms = 200/second max
        // warm-up rate
        Thread.sleep(500);
    });
}
...
│ NEXT EXPLORE │ Write-Around → Cache Aside vs Read-Throug│
└─────────────────────────────────────────────────────────┘
```
