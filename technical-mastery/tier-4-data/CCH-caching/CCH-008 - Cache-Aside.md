...
nav_order: 8
permalink: /technical-mastery/caching/cache-aside/
id: CCH-022
category: Caching
difficulty: ★★☆
...
public class ProductService {
    ...
    public Product updateProduct(String productId,
        ...
        // Invalidate cache (delete, not update - avoids race
        // conditions)
...
public class ProductServiceAnnotated {
    ...
    public Product getProduct(String productId) {
        ...
        // Spring Cache: automatically check cache before calling
        // method body
    ...
    public Product updateProduct(String productId,
        ProductUpdateRequest req) {
        // @CacheEvict: automatically delete from cache after method
        // executes
        Product product =
            productRepository.findById(productId).orElseThrow();
        product.setPrice(req.getPrice());
...
Thread A: ...DB fetch returns OLD value → writes OLD value
  to cache
Result: stale cache entry (Thread B's update is not
  reflected)

Duration: seconds (between Thread B's write and Thread A's
  cache set)
Impact: low to medium (bounded by time between operations,
  not long-lived)

Fix 1: short TTL - stale entry expires quickly (accept
  brief inconsistency)
Fix 2: use "delete then re-fetch" on write (Thread B
  deletes; next GET re-fetches fresh)
Fix 3: distributed lock on cache key during re-population
  (prevents multiple simultaneous misses)

Most production systems accept the brief stale window
  (seconds) as a reasonable tradeoff
...
→ PostgreSQL: SELECT * FROM products WHERE id = 12345 →
  {id:12345, name:"Widget", price:9.99}
→ Redis: SET product:12345 '{"id":12345,...}' EX 600 (10
  min TTL)
...
→ PostgreSQL: UPDATE products SET price=12.99 WHERE
  id=12345 - COMMIT
...
if (Boolean.TRUE.equals(locked)) {
    ...
    // Loser: wait briefly and retry (cache should be populated by
    // winner)
    Thread.sleep(50);
...
│ FIX STALE   │ Short TTL + immediate invalidation on writ│
...
│ NEXT EXPLORE│ Read-Through → Write-Through → Write-Behin│
└─────────────────────────────────────────────────────────┘
```
