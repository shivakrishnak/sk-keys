...
nav_order: 27
permalink: /technical-mastery/big-data-streaming/apache-beam/
id: BIG-037
...
// </dependency>
// Runner: beam-runners-flink-1.16 OR
    ...
    public static void main(String[] args) {
        ...
        PCollection<String> formatted = wordCounts.apply(
            ...
            MapElements.into(TypeDescriptors.strings())
                .via((KV<String,
                    Long> wc) -> wc.getKey() + ": " + wc.getValue())
        );

        // Sink: write results
        formatted.apply("WriteResults",
            TextIO.write().to("gs://my-bucket/output/wordcount"));

        // Execute:
        pipeline.run().waitUntilFinish();
        // Change to DataflowRunner: run on Google Cloud (no code
        // change above)
    }
}
```
...
public class KafkaStreamingPipeline {
    ...
    public static void main(String[] args) {
        ...
        PCollection<KV<String,
            String>> kafkaMessages = pipeline.apply(
            "ReadFromKafka",
        ...
        PCollection<Order> orders = kafkaMessages.apply(
            ...
            ParDo.of(new DoFn<KV<String, String>, Order>() {
                ...
                public void processElement(@Element KV<String,
                    ...
                    try {
                        Order order =
                            objectMapper.readValue(kv.getValue(),
                                Order.class);
                        out.output(order);
                    } catch (JsonProcessingException e) {
                        // Dead-letter pattern: route bad records to
                        // side output
                        log.error("Failed to parse order: {}",
                            kv.getValue());
                    }
                }
            })
        ...
        // WINDOWING: assign event-time timestamps + 5-minute tumbling
        // windows
        PCollection<Order> windowedOrders = orders
            .apply("AssignTimestamps",
                WithTimestamps.of((Order order) -> new Instant(
        ...
        PCollection<KV<String,
            Double>> revenueByProduct = windowedOrders
            .apply("KeyByProduct",
        ...
        // Run on Dataflow: add DataflowPipelineOptions +
        // project/region/tempLocation
...
public class FraudDetectionDoFn extends DoFn<Order, FraudAlert> {
    ...
    // State (only available in Beam with state-capable runners:
    // Flink, Dataflow)
    ...
    private final TimerSpec cleanupTimer =
    ...
    public void processElement(
            @Element Order order,
            @StateId(
                "transaction-count") ValueState<Integer> countState,
            @TimerId("window-cleanup") Timer cleanupTimer,
            OutputReceiver<FraudAlert> out) {

        Integer count = MoreObjects.firstNonNull(countState.read(),
            0) + 1;
        countState.write(count);

        // Set timer to expire in 5 minutes (cleanup state)
        cleanupTimer.set(Instant.now()
            .plus(Duration.standardMinutes(5)));

        if (count > 5) {
            out.output(new FraudAlert(order.getUserId(), count,
                order));
        }
    }

    @OnTimer("window-cleanup")
    public void onTimer(@StateId(
        "transaction-count") ValueState<Integer> countState) {
        countState.clear();
...
1. Developer writes Beam pipeline (Java SDK):
   Read Kafka → Parse JSON → Filter → Aggregate (5-min
     windows) → Write BigQuery
...
3. Deploy to Google Dataflow:
   ...
   pipeline.run();
   → Beam SDK serializes pipeline graph → sends to
     Dataflow API
   → Dataflow: provisions workers (auto-scaled), runs
     pipeline
...
DoFn<...> fn = new DoFn<...>() {
    @ProcessElement
    public void process(...) { mapper.readValue(...); }
    // captures outer mapper
};
...
public class ParseDoFn extends DoFn<String, Order> {
    ...
    public void setup() {
        ...
        // created per-worker, not serialized
    }

    @ProcessElement
    public void process(@Element String json,
        OutputReceiver<Order> out) {
