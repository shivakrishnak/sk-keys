...
nav_order: 21
permalink: /technical-mastery/big-data-streaming/windowing/
id: BIG-031
...
// Aggregates: total orders and revenue per product per 5-minute
// window
orders
...
// Each 5-minute period: separate aggregation, results emitted when
// window closes

// 2. SLIDING WINDOW: 10-minute window, slide every 2 minutes
// Overlapping: event at T=4 appears in windows [0:00-10:00),
// [2:00-12:00), ..., [4:00-14:00)
...
orders
    ...
    .aggregate(new OrderAggregator())
    .print();
// Higher overhead: each event stored in (windowSize/slideSize) = 5
...
orders
    ...
    // keep window open 2 extra minutes for late events
    .sideOutputLateData(lateOutputTag)
    // events beyond 2min → side output (don't discard)
    .aggregate(new OrderAggregator());
// Window [00:00, 00:05):
...
orders
    ...
    .windowedBy(
        TimeWindows.ofSizeAndGrace(Duration.ofMinutes(10),
...
// ofSizeWithNoGrace: no late events allowed (emit immediately when
// watermark passes)
...
while (it.hasNext()) {
    ...
    System.out.println("Window time: " + next.key + ",
        count: " + next.value);
}
```
...
TUMBLING window (size=5min):
  Events: M1@0:01, M2@0:03, M3@0:04, M4@0:07, M5@0:09,
    M6@0:11
  Windows:
...
SLIDING window (size=5min, slide=2min):
  ...
  Windows:
    [0:00-0:05): M1, M2, M3 → count=3
    [0:02-0:07): M1*, M2, M3 → count=3  (* M1@0:01 NOT in
      [0:02-0:07), M2@0:03 IS)
    Actually: M2@0:03, M3@0:04 in [0:02-0:07): count=2
    [0:04-0:09): M3 in [0:04-0:09)? M3@0:04 is at
      boundary: count=1
  Each event: may be in MULTIPLE windows
    (windowSize/slideSize = 5/2 = ~2.5 windows avg)

SESSION window (gap=5min, per user):
  User-A events: M1@0:01, M2@0:02, M3@0:04, [gap],
    M4@0:15, M5@0:16
  Sessions for User-A:
    Session 1: [0:01 - 0:04+gap]: M1, M2, M3 (gap after M3
      > 5min) → closes ~0:09
    Session 2: [0:15 - 0:16+gap]: M4, M5 → closes ~0:21
  Window size: VARIABLE (determined by activity gaps, not
    fixed duration)
```
...
```
Fraud detection: > 3 transactions in 5-minute session for
  same user
...
Events for User-42:
  ...
  T=14:08:01 (watermark advances past 14:03:00 + gap=5min
    = 14:08:00)
  → Session window closes: [14:00:01, 14:03:00+5min)
  → Aggregate: User-42, 4 transactions, $1400 total, 3
    minutes
  → 4 > 3 threshold → FRAUD ALERT published to
    "fraud-alerts" topic
...
orders.assignTimestampsAndWatermarks(
    WatermarkStrategy.<Order>forBoundedOutOfOrderness(
        ...
        .withTimestampAssigner((order, ts) -> {
            ...
            if (timestamp <= 0) {
                log.error("INVALID TIMESTAMP: {} for order {}",
