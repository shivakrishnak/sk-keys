...
nav_order: 20
permalink: /technical-mastery/big-data-streaming/apache-flink/
id: BIG-016
...
Flink Cluster:
  ...
  [TaskManager 1] [TaskManager 2] ... [TaskManager N]
    ...
    - contains N task slots (each slot = 1 parallel
      pipeline thread)
    - hosts operator state (RocksDB or heap)
    - communicates with Kafka, HDFS, etc.

  Task slot: isolated unit of execution (memory partition
    within a TM)
  Parallelism: each operator runs on P tasks across P slots

  Example:
    4 TaskManagers × 8 slots = 32 available slots
    Job with parallelism=32: each operator has 32 parallel
      instances
    keyBy("user_id") → hash(user_id) % 32 → routes to 1 of
      32 instances
    Instance #5 handles all events for users in its key
      range
```
        ...
        if (lastTime == null || currentTime - lastTime > 60_000) {
            ...
            if (newCount > 5) {
                ...
                out.collect(new FraudAlert(
                    payment.getUserId(),
                    currentTime,
                    "HIGH_VELOCITY: " + newCount +
                        " transactions in 1 minute"
                ));
            }
        }

        // Register a timer: clean up state after 2 minutes of
        // inactivity
...
```
Flink Checkpoint Mechanism (simplified Chandy-Lamport
  barriers):

Normal flow:
  Kafka → [Source] → [Filter] → [keyBy] → [Process] →
    [Sink] → Delta Lake

Checkpoint triggered by JobManager:
  1. JobManager: inject CHECKPOINT BARRIER marker into
    each source stream
  2. Source operator: reads up to Kafka offset 15,237 →
    emits barrier

  3. Operators process events normally UNTIL they see the
    barrier
  ...
  5. When Process (keyed, stateful) receives barrier from
    ALL inputs:
     a. Wait until barriers from all parallel partitions
       arrive (align)
     b. Snapshot all keyed state (user_id → {count,
       lastTime} for all users)
     c. Async write state snapshot to S3/HDFS
     d. Emit barrier to Sink

  6. Sink: receives barrier → commits current transaction
    to Delta Lake
  7. JobManager: receives ACK from all operators →
    checkpoint N complete
  8. S3: now has snapshot of all operator states at Kafka
    offset 15,237

Recovery after failure:
  1. JobManager detects TaskManager failure (heartbeat
    timeout)
  2. Restart: restore all operators from last checkpoint N
     ...
     - Sink: state already committed (or will be replayed
       idempotently)
...
T=0ms: Payment {user=42, amount=500, ts=10:00:00.100}
  arrives in Kafka
T=1ms: Source task 5 reads event, assigns event timestamp,
  emits watermark
T=2ms: Filter task 5: amount > 10? Yes → passes through
T=3ms: keyBy(user_id): hash(42) % 32 = 10 → routed to
  Process task 10
T=4ms: Process task 10: lookup ValueState for user_id=42
  in RocksDB
...
T=4ms: Process task 10: count=5 → increment to 6 → FRAUD
  ALERT emitted

T=5ms: Sink: write alert to Kafka "fraud-alerts" topic via
  Kafka transaction
...
T=30s: Checkpoint triggered by JobManager:
  ...
  Checkpoint complete in ~2s (incremental: only changed
    RocksDB blocks)
```
