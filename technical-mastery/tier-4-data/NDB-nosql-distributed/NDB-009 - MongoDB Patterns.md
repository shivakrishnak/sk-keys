...
nav_order: 9
permalink: /technical-mastery/nosql/mongodb-patterns/
id: NDB-014
...
{ _id: ObjectId(), device: "sensor-42",
    ts: ISODate("2024-01-15T14:00:00"), temp: 25.1 }
{ _id: ObjectId(), device: "sensor-42",
    ts: ISODate("2024-01-15T14:00:01"), temp: 25.2 }
...
// 2. Pre-aggregated min/max/sum in document → fast aggregations
// without $group
...
db.sensor_data.aggregate([
  ...
  { $group: { _id: null, avgTemp: { $avg: "$avg_temp" } } }
  // use pre-computed avg
])
```
...
{ user_id: "user-celebrity", page: 1,
    followers: [ ... next 1000 ... ] }
{ user_id: "user-celebrity", page: 2,
    followers: [ ... next 1000 ... ] }
...
function getFollowers(userId) {
  ...
  if (user.has_extras) {
    const extras =
        db.user_followers_extras.find({user_id: userId}).toArray();
    extras.forEach(doc => followers.push(...doc.followers));
  }
  return followers;
}
// Regular users: single document read
// Celebrities: multiple reads (but only for celebrities, not for all
// users)
```
...
// NAIVE: sparse document with many null fields
{
  ...
  // laptop fields (null for headphones)
...
// Benefits: no sparse nulls; indexable; new attributes need no schema
// change
```
...
// Avoids JOIN (no separate car/motorcycle/truck collections to query
// + merge)
```
...
```javascript
{
  _id: orderId,
  user_id: "u42",
  user_snapshot: { name: "Alice", email: "alice@example.com" },
  ...
  items: [
    {
      ...
      // snapshot at order time (historical price)
      quantity: 1
    }
  ],
  total: 1299.99
...
→ db.carts.updateOne({user_id}, {
    $push: { items: {product_id, name_snapshot,
      price_snapshot, qty} },
    $inc: { total: price, item_count: 1 }
  })
→ [MONGODB PATTERNS ← YOU ARE HERE: extended ref +
  computed]
...
│              │  normalization - pre-compute what you rea│
