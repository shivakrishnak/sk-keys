...
nav_order: 10
permalink: /technical-mastery/nosql/redis-data-structures/
id: NDB-015
...
# atomic increment (integer as string)
GETEX user:1001:token EX 3600
# get + reset TTL (sliding expiry)
SETNX lock:payment:1001 "worker-3" EX 30
# set only if not exists (distributed lock basis)

# Atomic increment: no race condition
# Two requests simultaneously: INCR page:homepage:views
# Redis single-threaded → one command runs, then the other
# Both correctly get 1 then 2; NOT both getting 1 (race condition in
# naive DB logic)
```
...
BRPOP email:queue 5
# consumer: blocking pop, 5s timeout
...
# Better than: SET user:1001 '{"name":"Alice","email":"..."}'
# (serialized JSON)
# Why: individual field updates without
# deserialization/reserialization
# HINCRBY is atomic; JSON approach requires GET → decode → increment →
# encode → SET
```
...
SUNION tags:post:42 tags:post:99
# → {"redis","database","caching","nosql"}
...
# → ["player:bob", 102000, "player:alice", 98500, "player:charlie",
# 87200]
...
# 10 million users = 10MB bitmap (vs. Set with user IDs = hundreds of
# MB)
...
PFADD visitors:page:homepage "user-1001"
...
→ [Redis DATA STRUCTURES ← YOU ARE HERE: sorted sets for
  both]
All Redis operations: < 1ms, atomic, no application-level
  locking
```
