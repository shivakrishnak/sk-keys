...
nav_order: 23
permalink: /technical-mastery/nosql/graph-db/
id: NDB-026
...
Edges: have a type and properties (directed)
  (:Person {name: "Alice"})-[:WORKS_AT {since: 2020, role:
    "Engineer"}]->(:Company {name: "TechCorp"})
  (:Person {name: "Alice"})-[:FRIENDS_WITH {since:
    2019}]->(:Person {name: "Bob"})
...
MATCH p = shortestPath(
  (alice:Person {name: "Alice"})-[:FRIENDS_WITH*]-(
      bob:Person {name: "Bob"})
)
...
Neo4j native graph storage:
  ...
  "Index-free adjacency": each node stores pointers
    directly to its
  connected edges - no index lookup needed to find
    neighbors.
...
RETURN a.id, nodes(path) AS path,
       reduce(total=0,
           t IN relationships(path) | total + t.amount) AS total
...
FROM orders o1
JOIN orders o2 ON o1.user_id =
    o2.user_id AND o1.product_id != o2.product_id
...
→ Neo4j: CREATE (a:Account {id:"A"})-[:TRANSFER
  {amount:5000}]->(b:Account {id:"B"})
