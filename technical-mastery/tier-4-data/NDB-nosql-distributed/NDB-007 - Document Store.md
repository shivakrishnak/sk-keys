...
nav_order: 7
permalink: /technical-mastery/nosql/document-store/
id: NDB-012
...
EMBED (denormalize):
  ...
  { _id: "u1", name: "Alice", orders: [{orderId: "o1",
    amount: 100}, ...] }

  Pro: single read for user + all orders
  Con: document grows unbounded (orders accumulate); 16MB
    document size limit in MongoDB
...
REFERENCE (normalize):
  ...
  Con: two queries to get user + orders (application-level
    join)

RULE OF THUMB:
  Embed when: 1:few relationship; read together always;
    child rarely queried alone
  Reference when: 1:many (unbounded); child queried
    independently; large child data
```
...
```json
// posts collection
{ "_id": "post-1", "title": "...", "authorId": "author-1",
    "tagIds": ["tag-nosql"] }
// authors collection
{ "_id": "author-1", "name": "Alice", "bio": "Engineer" }
// comments collection
{ "_id": "c-1", "postId": "post-1", "user": "Bob",
    "text": "Great post!" }
```
...
│ NEXT EXPLORE │ MongoDB Patterns → Wide Column vs Documen│
└─────────────────────────────────────────────────────────┘
```
