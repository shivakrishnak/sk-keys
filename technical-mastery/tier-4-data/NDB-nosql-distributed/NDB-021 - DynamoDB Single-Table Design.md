...
nav_order: 21
permalink: /technical-mastery/nosql/dynamodb-single-table-design/
id: NDB-024
...
```
Pattern             PK           SK Condition      Index
  Returns
───────────────────────────────────────────────────────────
Get user by ID      USER#<id>    SK=PROFILE        -
  User
Get user orders     USER#<id>    SK begins_w ORDER# -
  Orders[]
Get order items     ORDER#<id>   SK begins_w ITEM#  -
  Items[]
Get pending orders  -            -                 GSI2
  Orders[]
Get user by email   -            -                 GSI1
  User
Get order by ID     ORDER#<id>  SK=METADATA        -
  Order
Get user+orders     USER#<id>    SK >= PROFILE      -
  User+Orders[]
```
