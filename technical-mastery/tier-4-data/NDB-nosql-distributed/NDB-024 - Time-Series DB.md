...
nav_order: 24
permalink: /technical-mastery/nosql/time-series-db/
id: NDB-027
...
```
Timestamp delta-delta encoding (Gorilla algorithm,
  Facebook):
  Raw:    [1592838450, 1592838460, 1592838470, 1592838480]
    (10-second intervals)
...
Float XOR compression:
  ...
  0.55, 0.56, 0.55, 0.57 → XOR with previous: mostly
    leading zeros
  XOR encoding: store only the differing bits
  Compression ratio: ~1.37 bits per value (vs. 64 bits for
    raw double)
```
...
InfluxDB v1 (line protocol):
  ...
  cpu_usage,host=web-01,region=us-east
    user=0.55,system=0.12 1592838450000000000
  cpu_usage,host=web-02,region=us-east
    user=0.43,system=0.08 1592838450000000000
  ...
  Anti-pattern: user_id as a tag → millions of tag values
...
-- Every day =
    one chunk (separate file; old chunks compressed/dropped easily)
...
```
# Prometheus: pull-based metrics; stores as time-series
  locally
...
# Prometheus storage: local TSDB (blocks on disk, ~1.5B
  samples/server)
# Long-term: Thanos or VictoriaMetrics for multi-node,
  long-retention
```
...
```
Service: instrumented with Micrometer (Java) /
...
Grafana:
→ PromQL query: avg(rate(http_requests_total[5m])) by
  (service)
→ Prometheus: prune to last 5 min → decode → rate() →
  result
→ Dashboard panel renders in < 100ms

Long-term (Thanos / VictoriaMetrics):
→ Prometheus remote_write to long-term store
→ Retention: 1 year at 15s resolution, 5 years at 1h
  resolution
```
...
influx query 'import "influxdata/influxdb" show series count(
    ) from bucket: "mydb"'
```
...
│ TOOLS        │ Prometheus, InfluxDB, TimescaleDB, VictMe│
