...
nav_order: 16
permalink: /technical-mastery/ai-foundations/inference/
id: AIF-023
...
```
Prefill: [all input tokens] → processed in parallel → KV
  cache
Decode:  [one new token]    → one forward pass       → one
  output
         [two tokens]       → one forward pass       → one
           output
