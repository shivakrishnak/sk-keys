...
nav_order: 36
permalink: /technical-mastery/ci-cd/tekton/
id: CCD-036
category: CI/CD
difficulty: ★★★
...
Webhook → Tekton Triggers EventListener Pod
  ...
  → TaskRun(maven-build): waits for git-clone [← YOU ARE
    HERE]
...
TaskRun(unit-test) fails: 3 tests fail
  ...
  → `tkn taskrun logs pipeline-run-abc-unit-test` shows
    output
  → Pod is automatically cleaned up (configurable
    retention)
```
