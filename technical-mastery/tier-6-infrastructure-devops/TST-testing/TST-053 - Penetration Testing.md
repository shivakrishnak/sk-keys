...
nav_order: 53
permalink: /technical-mastery/testing/penetration-testing/
id: TST-058
category: Testing
difficulty: ★★★
...
PHASE 1: PRE-ENGAGEMENT
  ...
  → Rules of Engagement: what's off-limits (production
    DBs?), timing
  → Emergency contacts: if tester accidentally causes an
    outage
  → Legal authorization: written permission (without this
    = criminal offense)
...
PHASE 3: THREAT MODELING
  → What are the high-value targets? (user PII, payment
    data, admin access)
...
PHASE 4: VULNERABILITY ANALYSIS
  ...
  → Manual analysis: test business logic, authentication
    flows
...
PHASE 5: EXPLOITATION
  ...
  → Move laterally (from one server to others in the
    network)
...
PHASE 6: POST-EXPLOITATION
  → Determine how long tester could have persisted
    undetected
...
3. Authentication Bypass:
   ...
   Server returns: token=abc123 in the response body (not
     just via email)
...
5. Exposed Admin Interfaces:
   ...
   /actuator/env exposes application config (Spring Boot
     default)
...
```
Tester finds: GET /api/v1/users/profile?id=10045 returns
  user profile
Sequential IDs → tries id=10044 → another user's profile
  (IDOR)
Profile contains: email, phone, partial address

Then: POST /api/v1/password-reset with {"email":
  "victim@victim.com"}
Response body: {"message": "reset email sent",
  "debug_token": "abc123"}
(Developer accidentally left debug mode on)

Password reset token returned in response → account
  takeover without email access

Chain: IDOR (email discovery) + token exposure (account
  takeover)
Neither vulnerability alone is critical; combined = full
  account takeover

Lesson: pen testers think in chains. Automated tools find
  individual
vulnerabilities; pen testers combine them into attack
  paths.
```
...
```bash
# OWASP ZAP - automated DAST (part of CI pipeline, not a full pen
# test)
...
Description:
  The API endpoint does not validate that the
    authenticated user owns the
  requested order. An attacker with a valid session can
    enumerate other users'
  order IDs and access their order data, including
    shipping address and items purchased.
...
Impact:
  Exposure of PII (name, address), order history for all
    users.
  ~50,000 user orders accessible.

Remediation:
  Add authorization check: verify order.userId ==
    authenticatedUser.id
  before returning order data. Return HTTP 403 if check
    fails.
