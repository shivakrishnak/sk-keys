...
nav_order: 20
permalink: /technical-mastery/testing/test-isolation/
id: TST-014
category: Testing
difficulty: ★★☆
...
Test 4: counts all users → expects 1 → gets 2 ✗ ←
  contaminated by Tests 1 and 3
```
...
2. INDEPENDENT TEARDOWN: each test cleans up after itself
   BAD:  Test 41 creates a User; no cleanup; Test 42 gets
     wrong count
   GOOD: @Transactional (Spring) rolls back after each test
         OR @BeforeEach / @AfterEach clears state
           explicitly

3. ORDER INDEPENDENCE: same result in any order
   Test: can pass in isolation if all isolation rules are
     followed

4. PARALLEL SAFETY: can run concurrently
   BAD:  two tests create User with email "alice@test.com"
     → unique constraint
   GOOD: each test uses unique data (random email,
     UUID-based)
```
...
@SpringBootTest
@Transactional
// entire test runs in a transaction that is rolled back
...
class CartServiceTest {
    ...
    // No shared state between tests - each test gets new mock
    // instances
    // mockitoExtension.beforeEach() creates them, afterEach() resets
    // them
}
```
...
Fix: Test C must either:
  a. Create its own state: confirm 0 admins in a fresh
    context, OR
  b. Clear admins in @BeforeEach, OR
  c. Create an admin and assert count is exactly +1 from
    baseline
```
...
class UserServiceIntegrationTest {
    ...
    @Test void createUser_incrementsCount() {
        long before = repo.count();  // baseline (0 in
          fresh transaction)
        service.createUser("alice@test.com");
        assertThat(repo.count()).isEqualTo(before + 1);
        // @Transactional rollback: no alice in DB for
          next test
    }
