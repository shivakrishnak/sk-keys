...
nav_order: 44
permalink: /technical-mastery/testing/wiremock/
id: TST-057
category: Testing
difficulty: ★★★
...
WireMock stub of POST /v1/charges:
  Tests ALL of the above:
    ...
    → Body: is the amount encoded as form data
      (amount=5000¤cy=usd)?
    → Error handling: what happens when WireMock returns
      429?
...
class StripePaymentServiceTest {
    ...
    void charge_successfulPayment_returnsTransactionId(
        ...
        PaymentResult result = stripeService.charge("tok_test",
            50.00, "USD");
    ...
    @Test
    void charge_rateLimited_retriesAndSucceeds(
        WireMockRuntimeInfo wm) {
        stubFor(post("/v1/charges")
        ...
        // Verify: service retries after 429 and succeeds on second
        // attempt
        PaymentResult result = stripeService.charge("tok_test",
            50.00, "USD");
...
```
Without WireMock: Mockito mocks StripeClient.charge(token,
  amount)
  Unit tests pass: charge() is called with correct
    arguments

Deploy to staging: payments fail
  Debug: Stripe returns 401 Unauthorized
  Root cause: Authorization header is "Bearer " + apiKey
    but apiKey was accidentally double-encoded →
      "Bearer%20sk_test_..."
    Stripe rejects the malformed header

This bug is INVISIBLE to Mockito (which mocks the Java
  method call)
WireMock would have caught it:
  verify(...).withHeader("Authorization", equalTo("Bearer
    sk_test_123"))
...
class WeatherServiceTest {
    ...
    @Test
    void getCurrentWeather_returnsTemperature(
        @WireMockRuntimeInfo wm) {
        stubFor(get(urlPathEqualTo("/weather/current"))
            ...
            .willReturn(aResponse()
                .withStatus(200)
                .withHeader("Content-Type", "application/json")
                .withBodyFile("weather-london.json")));
                // file in __files/
