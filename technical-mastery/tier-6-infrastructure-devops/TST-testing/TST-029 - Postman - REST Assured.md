...
nav_order: 29
permalink: /technical-mastery/testing/postman-rest-assured/
id: TST-027
category: Testing
difficulty: ★★☆
...
REST ASSURED:
  ...
  ✓ Can use Java test utilities (Testcontainers, Spring
    Test)
...
QA Engineer workflow:
  ...
  6. CI pipeline: newman run collection.json --environment
    ci.json
  7. Newman output: JUnit XML → CI picks up test results

Result: Manual Postman testing converted to automated CI
  pipeline
         with minimal developer overhead.

Limitation: Collection JSON in git → merge conflicts are
  painful
             → large teams often migrate to REST Assured
               as codebase matures
```
...
class OrderApiTest {
    ...
    void setUp() {
        ...
        authToken = obtainAuthToken("testuser@test.invalid",
            "password");
    }
