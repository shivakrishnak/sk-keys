...
nav_order: 41
permalink: /technical-mastery/testing/test-diamond/
id: TST-032
category: Testing
difficulty: ★★★
...
```
PYRAMID suits:
  ✓ Monolith with rich domain logic (pricing, rules,
    algorithms)
...
DIAMOND suits:
  ...
  ✓ When most bugs are at service boundaries (schema
    drift, API changes)
  ✓ When services evolve independently and must maintain
...
What a service test covers:
  ...
  ✓ Error handling (what HTTP status for various error
    conditions)
  ✓ Contract: does the response match what consumers
    expect?
...
Service B (provider) has contract test:
  Verify: GET /api/orders/{id} returns schema matching
    consumer expectation
  Tools: Pact (consumer-driven contract testing)

Result: if Service B changes its response schema, contract
  test fails immediately
  → Integration bug caught before deployment, not in
    production
```
...
Contract test (diamond middle layer): would catch this
  immediately.
  Consumer (Service A) defines: "I will send field
    'amount'"
  Provider (Service B) verifies: "I read field 'amount'" ←
    fails immediately
```
...
Service tests (30 tests - the diamond's wide layer):
  ✓ POST /orders → creates order in PostgreSQL
    (Testcontainers)
  ✓ GET /orders/{id} → returns correct JSON schema
  ✓ PUT /orders/{id}/cancel → validates state machine
    transitions
  ✓ All HTTP status codes (200, 201, 400, 404, 409)

Contract tests (10 tests - part of wide layer):
  ✓ Pact: order response matches Payment Service consumer
    pact
  ✓ Pact: order events match Notification Service consumer
    pact
...
Risk coverage: service contract, DB schema, HTTP layer,
  critical flows
```
...
```java
// Service test (widest diamond layer): real Spring context +
// Testcontainers
