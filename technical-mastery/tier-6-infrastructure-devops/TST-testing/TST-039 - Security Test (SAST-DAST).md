...
nav_order: 39
permalink: /technical-mastery/testing/security-test-sast-dast/
id: TST-055
category: Testing
difficulty: ★★★
...
```
A01: Broken Access Control  ← DAST (try accessing /admin
  without auth)
A02: Cryptographic Failures  ← SAST (detect weak cipher
  usage in code)
A03: Injection (SQL, LDAP, OS) ← SAST + DAST (pattern +
  runtime test)
A04: Insecure Design        ← Threat modeling (manual)
A05: Security Misconfiguration ← DAST (probe headers,
  defaults)
A06: Vulnerable Components  ← SCA (dependency CVE scan)
A07: Auth/Session Failures  ← DAST (session fixation,
  brute force)
A08: Software Integrity     ← SAST + SCA (CI pipeline
  integrity)
A09: Logging Failures        ← SAST (check for sensitive
  data in logs)
A10: SSRF                   ← DAST (probe internal
  endpoints)
```
...
```
Developer commit →
  [SAST: Semgrep/SonarQube] → scan on every PR, fail on
    HIGH severity
  [SCA: Snyk] → scan dependencies, fail on CVSS ≥ 7.0
  [Secrets scan: Gitleaks/truffleHog] → fail on any
    hardcoded credential
  ↓ (only if all pass)
Deploy to staging →
  [DAST: OWASP ZAP baseline scan] → passive scan, no
    active attack
  ↓ (on release branch)
Deploy to pre-prod →
...
SAST couldn't find this because the algorithm check was in
  a third-party library
  the library version had a known vulnerability that SAST
    didn't flag
```
...
Pre-prod:
  7. ZAP full active scan:
     → attempts 500+ attack payloads (XSS, SQL injection,
       etc.)
