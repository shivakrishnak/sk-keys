...
nav_order: 36
permalink: /technical-mastery/testing/contract-test/
id: TST-021
category: Testing
difficulty: ★★★
...
CONSUMER SIDE (Service A tests):
  1. Consumer test: mock the provider using Pact mock
    server
  ...
  2. Pact records the interaction: {request, response} →
    pact.json
...
CAN I DEPLOY? (Pact Broker):
  → Service A: all providers verified my contract? YES →
    deploy
  → Service B: all consumers' contracts verified? YES →
    deploy
...
Current contract (from consumer A's pact file):
  GET /users/123 → {userId: "123", email: "...", plan:
    "premium"}

Provider B's team removes 'plan' field (not in their DB
  anymore):
...
Without contract test:
  ...
  → In staging: A fails to deserialize,
    NullPointerException on plan
...
With contract test:
  ...
  → B either: keeps 'plan', coordinates deprecation, or
    contacts A
```
...
1. Consumer test (UserServiceConsumerTest.java):
   ...
   RequestResponsePact getUserPact(PactDslWithProvider
     builder) {
...
4. Provider test (AuthServiceProviderTest.java):
   ...
   class AuthServiceContractTest {
       @TestTarget public final HttpTarget target = new
         HttpTarget(port);

       @State("user 123 exists")
       void userExists() {
         testUserRepository.save(testUser("123")); }
   }
...
class UserClientContractTest {
    ...
    public RequestResponsePact getUserPact(
    ...
    void getUser_activeUser_parsedCorrectly(MockServer mockServer) {
        UserClient client = new UserClient("http://localhost:" +
            mockServer.getPort());
...
├──────────────┼──────────────────────────────────────────┤
│ NEXT EXPLORE │ Pact → Pact Broker → Spring Cloud Contrac│
└─────────────────────────────────────────────────────────┘
```
