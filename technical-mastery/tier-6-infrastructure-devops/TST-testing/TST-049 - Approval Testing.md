...
nav_order: 49
permalink: /technical-mastery/testing/approval-testing/
id: TST-034
category: Testing
difficulty: ★★★
...
FIRST RUN (no approved file exists):
  ...
  3. Diff tool opens: shows "received" side, "approved"
    side is empty
...
SUBSEQUENT RUNS (approved file exists):
  ...
  If code output changed:
    ...
    CASE B (intentional change): received ≠ approved
      Test FAILS → human reviews diff → approves: updates
        approved file
      Approved file updated → test passes

FILE STORAGE:
  approved files → committed in git alongside test code
  received files → generated at test time, ignored by
    .gitignore
...
DIFF TOOL INTEGRATION:
  ApprovalTests integrates with: DiffMerge, Beyond
    Compare, Araxis, IntelliJ Diff
...
Step 1: Write approval test capturing current behavior:
  @Test void taxCalculation() {
    ...
    BigDecimal tax =
      legacyCalculator.calculateTax(invoice);
    Approvals.verify("tax=" + tax);
    // First run: FAIL, generates received file with
      tax=127.50
    // Approve → approved file: "tax=127.50"
  }

Step 2: Refactor legacyCalculator
  → Run test → if tax is still 127.50 → test passes → safe
    to continue
  → If test fails → diff shows: "tax=127.50" vs
    "tax=130.00" → introduced a bug

Step 3: Once refactoring is complete, write proper unit
  tests
...
class InvoiceReportTest {
    ...
    void invoiceReport_complexFormat() {
        ...
        // First run: creates
        // InvoiceReportTest.invoiceReport_complexFormat.received.txt
        // After approval:
    ...
    void invoiceReport_asJson() {
        ...
        // Serializes invoice to JSON, approves the full JSON
        // structure
    }
}
```
...
// InvoiceReportTest.invoiceReport_complexFormat.approved.t
INVOICE
Customer: Acme Corp
--------------
Widget         x10    $99.90
