...
nav_order: 17
permalink: /technical-mastery/testing/stubbing/
id: TST-037
category: Testing
difficulty: ★★☆
...
when(repo.findByEmail("admin@company.com"))
...
Test: stub all three repos for fast, isolated unit test
  ...
  when(accountRepo.findByUserId(42L)).thenReturn(Optional.o
...
class WeatherAlertServiceTest {
    ...
    void sendAlert_whenTemperatureExceedsThreshold() {
        ...
        verify(notificationService).sendAlert(eq("London"),
            contains("Heat Alert"));
    }
