...
nav_order: 48
permalink: /technical-mastery/testing/test-parallelization/
id: TST-033
category: Testing
difficulty: ★★★
...
1. METHOD-LEVEL PARALLEL (within one class):
   ...
   junit.jupiter.execution.parallel.mode.classes.default=sa

   Requirement: test methods don't share mutable state
   Risk: shared @BeforeAll setup data (must be immutable
     or thread-safe)

2. CLASS-LEVEL PARALLEL:

   junit.jupiter.execution.parallel.mode.classes.default=co

   Each class runs on its own thread
   Requirement: classes don't share databases without
     isolation
   Common: each class gets own Testcontainers instance OR
...
3. PROCESS-LEVEL (Maven fork):
   ...
   Cost: JVM startup per fork (mitigated by
     reuseForks=true)
...
PROBLEM: Database conflicts
  ...
  Thread B: INSERT user (email='alice@test.com')  --
    UniqueConstraintException

SOLUTION: Thread-safe unique data generation
  String email = "test-" + Thread.currentThread().getId()
...
PROBLEM: Testcontainers performance (one container per
  class = N containers for N parallel classes)

SOLUTION: Static Testcontainers (shared across classes in
  same JVM, isolated via schema/keyspace)
  @Container
...
PROBLEM: Sequence generators (auto-increment IDs conflict)
  ...
  Thread B: insert gets ID=2; Thread A assertion fails if
    it assumed ID=1

SOLUTION: Never assert on auto-generated IDs; assert on
  behavior/content
```
...
class OrderServiceParallelTest {
    ...
    void placeOrder_success() {
        ...
        // @Transactional ensures rollback after test - no cleanup
        // needed
    }
}
```
