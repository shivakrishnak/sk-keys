...
nav_order: 6
permalink: /technical-mastery/testing/unit-test/
id: TST-006
category: Testing
difficulty: ★☆☆
...
Option B (right size): test each public method
  ...
  → Internal refactors don't break tests if behavior
    unchanged

Option C (too large): test entire service class with real
  DB
  → Slow (seconds), not a unit test
  → Failures could be caused by DB, network, data state

Rule: unit = the smallest piece of code with testable
  behavior
...
1. RED: Write failing test first
   ...
   void calculateDiscount_premiumUser_returns20Percent() {
       ...
       assertEquals(0.20, service.calculate(user));  //
         FAILS (method not implemented)
   }
...
3. REFACTOR: Improve code without breaking test
   public double calculate(User user) {
       return
         USER_TYPE_DISCOUNTS.getOrDefault(user.getType(),
           0.0);
   }
...
public class OrderPricingService {
    ...
    public BigDecimal calculateTotal(Order order) {
        BigDecimal discount =
            discountRepo.getDiscount(order.getUserId());
...
class OrderPricingServiceTest {
    ...
    void calculateTotal_withDiscount_appliesDiscountBeforeTax() {
        ...
        Order order = Order.builder()
            ...
            .region("US").build();
        when(discountRepo.getDiscount("user1"))
