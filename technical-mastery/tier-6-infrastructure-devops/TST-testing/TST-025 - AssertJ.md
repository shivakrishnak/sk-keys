...
nav_order: 25
permalink: /technical-mastery/testing/assertj/
id: TST-017
category: Testing
difficulty: ★★☆
...
// Which assertion? Why was it false? What was the actual
  value?
```
...
// WRONG ORDER! Error message backwards
...
// Extract a field from each element, then assert on the extracted
// values
assertThat(users)
    .extracting("name")
    // string field name (not type-safe)
...
public class OrderAssert extends AbstractAssert<OrderAssert, Order> {
    public OrderAssert(Order actual) { super(actual,
        OrderAssert.class); }
    public static OrderAssert assertThat(
        Order order) { return new OrderAssert(order); }
...
void userProfile_hasAllRequiredFields() {
    ...
    SoftAssertions.assertSoftly(soft -> {
        ...
        soft.assertThat(profile.getJoinDate())
...
├──────────────┼──────────────────────────────────────────┤
│ EXCEPTIONS   │ assertThatThrownBy(() -> ...).isInstanceO│
├──────────────┼──────────────────────────────────────────┤
