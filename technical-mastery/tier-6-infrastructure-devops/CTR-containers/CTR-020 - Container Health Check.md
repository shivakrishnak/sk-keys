...
difficulty: ★★☆
depends_on: CTR-024, CTR-025, CTR-027, CTR-028
used_by: K8S-001, CTR-027
related: CTR-028, CTR-027, CTR-038, K8S-001
tags:
  - containers
  - kubernetes
  - reliability
...
nav_order: 20
permalink: /technical-mastery/containers/container-health-check/
---
...
container enters deadlocked state (goroutine stuck)
  ...
  → kubelet: terminates container (SIGKILL after grace
    period)
  → container restarts
...
- name: spring-app
  ...
  startupProbe:
    httpGet:
      ...
      # Spring Actuator liveness group
      port: 8080
    failureThreshold: 30
    # 30 failures × 5s period = 150s max startup
    periodSeconds: 5
