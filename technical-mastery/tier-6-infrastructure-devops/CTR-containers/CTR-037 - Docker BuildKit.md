...
difficulty: ★★★
depends_on: CTR-025, CTR-007, CTR-006, CTR-009, CTR-014
used_by: CTR-029, CTR-036
related: CTR-014, CTR-007, CTR-022, CTR-041, CTR-020
tags:
  - containers
  - docker
  - build
...
nav_order: 37
permalink: /technical-mastery/containers/docker-buildkit/
---
...
RUN --mount=type=secret,id=pip-token \
    ...
    -r requirements.txt
# The secret file at /run/secrets/pip-token exists ONLY during this
# RUN
...
Both produce separate manifests → combined into OCI Image
  Index
```
...
**FAILURE PATH:**
```
Legacy Dockerfile without BuildKit (or with legacy
  builder):
...
RUN --mount=type=secret,id=pip-token \
    pip install \
      --index-url "https://$(
          cat /run/secrets/pip-token)@pypi.company.com/simple/" \
      -r requirements.txt
