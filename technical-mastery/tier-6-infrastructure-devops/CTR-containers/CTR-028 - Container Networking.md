...
difficulty: ★★★
depends_on: CTR-018, CTR-025, NET-001, CTR-024
used_by: CTR-015, CTR-035, K8S-001, CTR-043
related: CTR-043, CTR-018, CTR-035, CTR-015, K8S-001
tags:
  - containers
  - networking
  - docker
...
nav_order: 28
permalink: /technical-mastery/containers/container-networking/
---
...
HOST:
  ...
  veth-host-a (end on bridge)     ← Container A's
    host-side veth
  veth-host-b (end on bridge)     ← Container B's
    host-side veth

CONTAINER A (NET namespace A):
  eth0: 172.17.0.2/16             ← Container A's veth
    (inner end)

CONTAINER B (NET namespace B):
  eth0: 172.17.0.3/16             ← Container B's veth
    (inner end)
```
...
```
Container A (172.18.0.2) → DB service name → DNS
  (127.0.0.11)
...
→ if fails: iptables MASQUERADE rule missing (can happen
  post-reboot)
→ fix: docker restart or iptables rule re-applied by
  dockerd
