...
difficulty: ★★☆
depends_on: CTR-007, CTR-009, CTR-006, CTR-013
used_by: CTR-036, CTR-035, CTR-029, CTR-040
related: CTR-007, CTR-009, CTR-036, CTR-040
tags:
  - containers
  - docker
  - intermediate
...
nav_order: 14
permalink: /technical-mastery/containers/multi-stage-build/
---
...
COPY . .
RUN go build -o /app .
# Final image contains: Go compiler (700 MB) + Go libraries + source
# code + binary
# Total: ~900 MB
```
...
→ debug: docker build --target=builder to inspect builder
  stage
```
...
│ SOLVES       │ images bloat size and expand attack surfa│
├──────────────┼──────────────────────────────────────────┤
│ KEY INSIGHT  │ COPY --from=stage copies only what you   │
│              │ need; all build stage layers are discarde│
├──────────────┼──────────────────────────────────────────┤
