...
difficulty: ★★☆
depends_on: CTR-006, CTR-007, CTR-020, CTR-024
used_by: CTR-014, CTR-036, CTR-040, CTR-019
related: CTR-006, CTR-007, CTR-014, CTR-040, CTR-020
tags:
  - containers
  - docker
  - intermediate
...
nav_order: 12
permalink: /technical-mastery/containers/docker-layer/
---
...
**NORMAL FLOW:**
```
Dockerfile instruction → BuildKit executes → filesystem
  diff
→ [LAYER created ← YOU ARE HERE] → compressed +
  content-hashed
→ stored in local layer cache → referenced in image
  manifest
...
→ incomplete layer not stored (SHA256 verification
  prevents use)
→ pull retried automatically
→ observable: "layer already exists" on retry indicates
  resume
```
...
# Single layer: ~15 MB (apt cache removed within same OverlayFS
# snapshot)
```
