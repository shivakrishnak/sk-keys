...
nav_order: 55
permalink: /technical-mastery/observability-sre/chaos-engineering/
---
...
Rule: start in the bottom-left (high control, low impact)
      ...
      NEVER move diagonally more than one cell per
        experiment.
```
...
Application-level faults:
  ...
  - Queue depth injection (insert N messages in dead
    letter)
...
Architecture:
  ...
  Integration with SLO:
    ...
    Abort condition: SLI < abort_threshold → cancel
      experiment
...
GameDay is a structured chaos exercise (half/full day):
  ...
  Pre-GameDay (1 week before):
    - Define scenario: "Payment processor goes dark for
      10min"
...
What happened:
  ...
  T+30s: All retries exhausted, pod stays in
    ImagePullBackOff
  T+90s: Circuit breaker not configured - requests keep
    failing
