...
nav_order: 48
permalink: /technical-mastery/obs/formal-slo-theory/
---
...
Pattern 2: Availability × Latency Composite
  ...
  This covers: "service is up but slow" (latency budget
    burns)
               "service is down" (availability budget
                 burns)
...
Pattern 3: User-Journey SLI (synthetic canary)
  ...
  Advantage: captures failures invisible to per-service
    SLIs
  ...
  Challenge: canary transactions must be realistic but
             harmless; complex to implement for all
               journeys
...
WHY THIS SPECIFIC CALIBRATION:
  25% threshold = 10.8 minutes consumed on 43.2 minute
    budget
  ...
  Four incidents (100%) = chronic reliability problem →
    freeze

  The policy is derived from the budget math, not
    arbitrary.
```
...
print(time_to_budget_exhaustion_hours(0.5))
# infinite (not consuming)
```
