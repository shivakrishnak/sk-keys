...
nav_order: 19
permalink: /technical-mastery/obs/elk-efk-stack-log-management/
---
...
[Fluent Bit POST /logs-2024-04-15/_bulk]
  ...
  Routes to primary shard (based on _id hash or
    round-robin)
...
curl http://elasticsearch:9200/_cat/shards?h=index,shard,prirep,state,
    unassigned.reason&v \
  | grep UNASSIGNED
...
curl -XPOST http://elasticsearch:9200/_cluster/reroute \
  ...
  -d '{"commands":[{"allocate_replica":{"index":"logs-2024-04-15",
      "shard":0,"node":"node-2"}}]}'
```
...
curl http://elasticsearch:9200/_cat/indices/logs*?v&h=index,
...
├──────────────┼──────────────────────────────────────────┤
│ NEXT EXPLORE │ Jaeger (trace correlation) → Loki (cheape│
