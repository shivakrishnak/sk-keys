...
nav_order: 66
permalink: /technical-mastery/kubernetes/k8s-upgrade-strategy/
id: K8S-066
category: "Kubernetes"
difficulty: "★★★"
depends_on:
...
Upgrade order (CRITICAL - never out of order):
  ...
  3. kube-controller-manager, kube-scheduler (≤ 1 minor
    behind apiserver)
  4. kubelet (≤ 2 minors behind apiserver) - one node at a
    time
  5. kube-proxy (≤ 2 minors behind apiserver) - alongside
    kubelet
  6. kubectl (±1 minor of apiserver)

Node upgrade options:
  - In-place:   drain → upgrade kubelet → uncordon
  - Blue-green: new node group → drain old → delete old
    nodes
```
...
kubectl deprecations --k8s-version v1.29
# or use: pluto detect-all-in-cluster
...
for node in $(
    kubectl get nodes -l eks.amazonaws.com/nodegroup=workers-1-28 \
             -o name); do
...
# REPLACEMENT     DEPRECATED  REMOVED
# my-ingress        default      Ingress      extensions/v1beta1
# networking.k8s.io/v1  true    true
...
T-4 weeks: Planning
  ...
  - Run pluto: find deprecated API usage in cluster
    manifests
  - Identify add-ons + their compatible versions (VPC CNI,
    etc.)
  - Update runbook
...
T-0: Production upgrade
  - Schedule maintenance window (or upgrade with
    0-downtime)
  - Take etcd snapshot
  - Upgrade control plane (EKS: update-cluster-version or
    kubeadm)
