...
nav_order: 42
permalink: /technical-mastery/kubernetes/kubelet/
id: K8S-042
category: "Kubernetes"
difficulty: "★★★"
...
/etc/kubernetes/manifests/
  kube-apiserver.yaml     ← kubelet reads, starts API
    Server
...
```
Scheduler: binds Pod to Node-2 (sets
  pod.spec.nodeName=node-2)
  → kubelet on Node-2 watches API Server: "new Pod
    assigned to me!"
  ...
  → Start probes (startupProbe first, then
    liveness/readiness)
  → readinessProbe passes → kubelet updates Pod status:
    Ready=True
