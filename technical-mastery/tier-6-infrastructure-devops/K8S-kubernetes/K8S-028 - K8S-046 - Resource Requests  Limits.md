...
nav_order: 28
permalink: /technical-mastery/kubernetes/resource-requests-limits/
id: K8S-028
category: "Kubernetes"
difficulty: "★★☆"
...
Request (memory soft limit):
  ...
  Affects OOM score (more memory relative to request =
    higher OOM score)
...
Limit (memory.limit_in_bytes):
  ...
  kubectl get pod my-pod -o
    jsonpath='{.status.containerStatuses[0].lastState.termi
  → OOMKilled
```
...
```
kubectl apply -f pod.yaml (requests: cpu=250m,
  memory=256Mi)
  → Scheduler:
      - Filter: find nodes with allocatable-sum(requests)
        >= 250m CPU + 256Mi mem
      - Score: pick best (most available resources, or
        least-waste)
      - Bind: assign pod to node

  → Node:
      - kubelet creates cgroups with cpu.shares,
        memory.limit_in_bytes
      - Container runtime starts containers with cgroup
        constraints

  At runtime (CPU throttle scenario):
      - App spikes to 600m CPU (limit = 500m)
      - CFS throttles: app gets 500m out of every 100ms
        period
  ...
  At runtime (OOM scenario):
      ...
      - kubectl: "OOMKilled", restarts (CrashLoopBackOff
        if repeated)
```
