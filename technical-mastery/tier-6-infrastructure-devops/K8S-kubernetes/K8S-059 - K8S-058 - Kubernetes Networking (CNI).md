...
nav_order: 59
permalink: /technical-mastery/kubernetes/kubernetes-networking-cni/
id: K8S-059
category: "Kubernetes"
difficulty: "★★★"
...
Kubernetes Networking Model:
  ...
  2. Any Pod can communicate with any other Pod (without
    NAT)
...
Popular CNIs: Flannel (simple), Calico (BGP + policy),
  Cilium (eBPF + L7)
```
...
```
Pod A (10.244.1.2) → eth0 → veth → cni0 bridge → flannel.1
  (VTEP)
  → VXLAN encapsulate: original packet wrapped in UDP port
    4789
  → Node1 physical NIC → network → Node2 physical NIC
  → VXLAN decapsulate → flannel.1 → Pod B routing → eth0 →
    Pod B
```
...
Each node advertises its Pod CIDR via BGP:
  Node1: "I own 10.244.1.0/24" → BGP route to all
    nodes/routers
  Node2: "I own 10.244.2.0/24"

Pod A → eth0 → cali... (veth) → kernel routing:
  Destination 10.244.2.0/24 via Node2 (192.168.0.2) →
    direct IP routing
...
Cilium:
  ...
  - Good for: high-performance, security-conscious,
    observability
...
On Node 1:
  ...
  2. eBPF hook on veth ingress:
     - Check NetworkPolicy: is Pod A allowed to send to
       Pod B? ✅
     - Lookup: where is 10.244.2.10? → Node 2 (192.168.0.2)
  3. Cilium encapsulates (VXLAN) or routes natively
    (Wireguard tunnel)

On Node 2:
  1. eBPF hook on host network ingress:
     - Decapsulate
     - Check NetworkPolicy: is Pod A allowed to talk to
       Pod B? ✅
  2. Route to Pod B's network namespace
  3. Pod B receives packet

Zero iptables involved. O(1) eBPF map lookups vs O(n)
  iptables chains.
```
