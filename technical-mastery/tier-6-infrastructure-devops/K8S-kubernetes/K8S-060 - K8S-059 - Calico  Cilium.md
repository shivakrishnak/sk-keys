...
nav_order: 60
permalink: /technical-mastery/kubernetes/calico-cilium/
id: K8S-060
category: "Kubernetes"
difficulty: "★★★"
...
Benchmark (Cilium): 100Gbps line rate on 10GbE with < 10%
  CPU
```
...
```
frontend-pod: curl -X DELETE
  http://backend:8080/api/v1/users/123

Cilium agent on frontend's node:
  eBPF hook on frontend-pod veth egress:
    1. Identify source identity: frontend-pod (label
      app=frontend)
    2. Identify destination: backend (label app=backend),
      port 8080
    3. L3/L4 policy: frontend → backend:8080 allowed? ✅
    4. L7 policy check needed? YES (HTTP policy exists)
    5. Redirect to Envoy proxy (embedded in node, not
      sidecar)

Envoy proxy:
    6. Parse HTTP: method=DELETE, path=/api/v1/users/123
    7. Check CiliumNetworkPolicy: only GET /api/v1/.*
      allowed
