...
nav_order: 55
permalink: /technical-mastery/kubernetes/kubernetes-secrets-management/
id: K8S-055
category: "Kubernetes"
difficulty: "★★★"
...
AWS Secrets Manager:
  - Path: prod/my-app/db
  - Value: {"password": "s3cr3tP@ss", "host":
    "db.prod.example.com"}

ExternalSecret CRD applied:
  → External Secrets Operator (using IRSA SA) calls AWS SM
    API
...
Every 1 hour (refreshInterval):
  → ESO re-reads AWS SM
  → If value changed (password rotation), updates K8s
    Secret
  → Pod environment: Deployment must restart to pick up
    new env var
