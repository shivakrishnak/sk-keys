...
nav_order: 35
permalink: /technical-mastery/kubernetes/storageclass/
id: K8S-035
category: "Kubernetes"
difficulty: "★★★"
...
Immediate:            PV created when PVC is created
                      Volume in random AZ → Pod may land
                        in different AZ ❌

WaitForFirstConsumer: PV created when Pod using PVC is
  scheduled
...
Developer creates PVC (storageClassName=fast-ssd, 20Gi)
  ...
  → CSI driver controller (ebs.csi.aws.com):
      - Calls AWS API: CreateVolume(type=gp3, 20Gi,
        az=us-east-1a)
      - Volume ID: vol-0abc123
      - Creates PV object referencing vol-0abc123
      - PVC bound to PV
  → Pod scheduled to node in us-east-1a
  → kubelet: node plugin attaches EBS volume to EC2
    instance
