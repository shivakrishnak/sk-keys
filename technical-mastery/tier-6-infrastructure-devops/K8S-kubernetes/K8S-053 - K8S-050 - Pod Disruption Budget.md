...
nav_order: 53
permalink: /technical-mastery/kubernetes/pod-disruption-budget/
id: K8S-053
category: "Kubernetes"
difficulty: "★★★"
...
```
# Result: during drain, cluster will not evict pods if it
  would bring
...
spec:
  minAvailable: 1     # 1 must be available, but there's only 1!
                      # Any drain would violate this → permanently
                      # blocked
...
kubectl drain worker-2 --ignore-daemonsets
  ...
  → Eviction API for pod my-app-7d8f9b-yyyyy (second
    my-app pod on same node):
      1. Count currently healthy my-app pods: 2 (one just
        evicted, new one not ready yet)
  ...
  → All pods evicted, node drained safely, service always
    had ≥ 2 pods
```
