...
nav_order: 58
permalink: /technical-mastery/kubernetes/fluxcd/
id: K8S-058
category: "Kubernetes"
difficulty: "★★★"
...
```
                    [Git Repository]
                         ↓ (poll/webhook)
           [Source Controller]
              ├─ GitRepository (fetches git, stores
                artifact)
...
```
Developer pushes code → CI builds my-app:v1.3.0 → pushed
  to registry
...
Image Update Automation (every 30m or immediate):
  → Finds marker in Git: # {"$imagepolicy":
    "flux-system:my-app"}
    # In deployment.yaml: image: my-registry/my-app:v1.2.0
