...
nav_order: 54
permalink: /technical-mastery/kubernetes/service-account/
id: K8S-054
category: "Kubernetes"
difficulty: "★★★"
...
spec:
  ...
  # opt-out if no API access needed
```
...
```
Pod starts with serviceAccountName: s3-reader (annotated
  with IAM role ARN)
  → EKS OIDC provider: issue SA token with
...
Security:
  ...
  - OIDC validation ensures only that specific SA can
    assume that role
```
