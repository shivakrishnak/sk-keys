...
nav_order: 33
permalink: /technical-mastery/kubernetes/cluster-autoscaler/
id: K8S-033
category: "Kubernetes"
difficulty: "★★★"
depends_on:
...
Load drops:
  ...
  → CA: safely drain nodes (checks PDB, local storage,
    DaemonSets)
...
```
Every 10s:
  1. Find nodes where sum(Pod requests) / Node capacity <
    threshold
  2. Check scale-down-unneeded-time (must be unneeded for
    N min)
  3. Simulate: can all Pods on this node fit elsewhere?
  4. Check PodDisruptionBudgets - would removal violate
    any?
