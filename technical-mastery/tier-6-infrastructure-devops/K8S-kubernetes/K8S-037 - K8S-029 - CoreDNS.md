...
nav_order: 37
permalink: /technical-mastery/kubernetes/coredns/
id: K8S-037
category: "Kubernetes"
difficulty: "★★★"
...
Headless service (returns all Pod IPs):
  postgres.default.svc.cluster.local → [10.244.1.5,
    10.244.2.5, 10.244.3.5]
```
...
Query: "orders-service"  (2 dots = 0, less than 5)
  → Try: orders-service.default.svc.cluster.local ✅
    (match!)
  → Return ClusterIP
...
Query: "api.example.com."  (4 dots, but ends with dot =
  absolute)
...
```
Pod in namespace "orders" wants to call "payments-service"
  in "payments"

1. getaddrinfo("payments-service")
2. /etc/resolv.conf: ndots=5, search:
  orders.svc.cluster.local ...
3. Try: payments-service.orders.svc.cluster.local →
  NXDOMAIN (wrong NS)
