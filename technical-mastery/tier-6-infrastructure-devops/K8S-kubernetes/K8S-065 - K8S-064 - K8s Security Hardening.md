...
nav_order: 65
permalink: /technical-mastery/kubernetes/k8s-security-hardening/
id: K8S-065
category: "Kubernetes"
difficulty: "★★★"
depends_on:
...
4C Security Model:
  Cloud (IAM, VPC, node security groups)
    └── Cluster (RBAC, audit logging, API server flags)
          └── Container (securityContext, PSS, image
            scanning)
...
1. Run kube-bench:
   ...
   FAIL examples:
   [FAIL] 1.2.6 Ensure anonymous requests are not
     authorized
...
2. Remediation:
   ...
   For 4.2.6: Add seccompProfile: RuntimeDefault to all
     Pods

3. Re-run kube-bench: 60 PASS, 0 FAIL, 20 WARN
4. Schedule monthly compliance scans (Kubescape /
  kube-bench)
```
