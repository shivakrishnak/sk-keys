...
nav_order: 19
permalink: /technical-mastery/maven-build/maven-wrapper-mvnw/
id: MVN-024
...
my-project/
  .mvn/
    wrapper/
      maven-wrapper.properties       ← pinned Maven
        version + download URL
      maven-wrapper.jar              ← bootstrap
        downloader (in older setups)
...
Is apache-maven-3.9.5 in ~/.m2/wrapper/dists/?
   ...
   NO  → download from distributionUrl, extract, cache
       │
       ▼
Execute: ~/.m2/wrapper/dists/apache-maven-3.9.5/bin/mvn
  clean install
```
...
mvn wrapper:wrapper
# uses current installed Maven version
...
# Make mvnw executable (Unix - required after git clone on some
# systems)
chmod +x mvnw
...
# In modern Maven wrapper this is not needed (pure shell bootstrap)
```
...
# set in git index (persists in repo)
