...
nav_order: 27
permalink: /technical-mastery/maven-build/gradle-build-script/
id: MVN-031
...
Phase 2: CONFIGURATION
  ...
  - ALL build script code runs here (even if task won't
    execute)
...
```kotlin
// WRONG: This print runs EVERY time - during configuration, not just
// when 'build' runs
tasks.named("build") {
    println("Building...")
    // configuration phase - runs even for 'gradle tasks'
}
...
repositories {
    mavenCentral()
    maven { url =
        uri("https://nexus.mycompany.com/repository/maven-public/") }
}
...
dependencies {
    ...
    // compile + runtime (not test)
    runtimeOnly("org.postgresql:postgresql")
    // runtime only
    compileOnly("org.projectlombok:lombok")
    // compile only (like Maven 'provided')
...
tasks.named<Jar>("jar") {
    ...
    // Spring Boot plugin creates fat jar; this avoids conflict
}
...
# show dependency graph for all configurations
...
./gradlew build --scan
# generate build scan at scans.gradle.com
./gradlew help --task compileJava
# describe a specific task's inputs/outputs
```
...
dependencyResolutionManagement {
    ...
    versionCatalogs {
        create("libs") {
            ...
            library("spring-boot-web", "org.springframework.boot",
...
├──────────────┼──────────────────────────────────────────┤
│ SCOPE MAP    │ implementation=compile, runtimeOnly=runti│
├──────────────┼──────────────────────────────────────────┤
