...
nav_order: 18
permalink: /technical-mastery/maven-build/snapshot-vs-release/
id: MVN-023
...
Build request for my-lib:2.0.0-SNAPSHOT:
  ...
  2. Update policy = daily → last checked > 24h ago →
    re-fetch
...
Next build today:
  ...
  2. Update policy = daily → last checked < 24h ago → use
    cached
```
...
# Build offline: use only local cache (fail if SNAPSHOT is
# missing/stale)
mvn clean install -o
