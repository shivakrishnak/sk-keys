...
nav_order: 28
permalink: /technical-mastery/maven-build/gradle-tasks/
id: MVN-032
...
Task execution states:
  ...
  UP-TO-DATE     → inputs/outputs unchanged since last
    execution; skipped
  FROM-CACHE     → outputs restored from build cache;
    skipped
  SKIPPED        → task was explicitly skipped (e.g.,
    predicate false, --exclude-task)
  NO-SOURCE      → task has no source files to process
    (e.g., no test files found)
```
...
tasks.register<Exec>("generateProto") {
    commandLine("protoc", "--java_out=build/generated",
        "src/proto/*.proto")
}
...
tasks.register("generateCode") {
    inputs.file("src/schema/model.json")
    // declare: re-run if this changes
