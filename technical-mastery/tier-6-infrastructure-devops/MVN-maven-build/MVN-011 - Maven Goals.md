...
nav_order: 11
permalink: /technical-mastery/maven-build/maven-goals/
id: MVN-014
...
```
mvn [phase]            → runs lifecycle up to and
  including phase
mvn [plugin]:[goal]    → runs a specific goal directly (no
  lifecycle)
mvn [phase] [plugin]:[goal]  → runs lifecycle to phase,
  then goal
```
