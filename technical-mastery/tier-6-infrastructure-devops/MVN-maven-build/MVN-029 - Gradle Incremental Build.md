...
nav_order: 29
permalink: /technical-mastery/maven-build/gradle-incremental-build/
id: MVN-033
...
```
Previous execution:
  inputs: {src/Foo.java: SHA-256-abc, config.yml:
    SHA-256-xyz}
...
Current execution request for 'compileJava':
  current inputs: {src/Foo.java: SHA-256-abc, config.yml:
    SHA-256-xyz}
...
abstract class ProcessTemplates : DefaultTask() {
    ...
    fun processTemplates(inputChanges: InputChanges) {
        if (inputChanges.isIncremental) {
            ...
            inputChanges.getFileChanges(templateFiles)
                .forEach { change ->
                when (change.changeType) {
                    ChangeType.ADDED,
                        ChangeType.MODIFIED -> processFile(
                            change.file)
...
Only Foo.java changed:
  ...
  2. Analyse: which classes depend on Foo? (classpath
    analysis)
...
fun generate(inputChanges: InputChanges) {
    inputChanges.getFileChanges(sourceFiles).forEach { change ->
        if (change.changeType != ChangeType.REMOVED) {
            generateDocFor(change.file)
            // only regenerate this file's doc
        }
    }
}
```
...
./gradlew compileJava --info 2>&1 | grep -E "(
    UP-TO-DATE|not up-to-date|Input property)"
...
├──────────────┼──────────────────────────────────────────┤
│ HOW TRACKED  │ Content fingerprints (SHA), not timestamp│
├──────────────┼──────────────────────────────────────────┤
