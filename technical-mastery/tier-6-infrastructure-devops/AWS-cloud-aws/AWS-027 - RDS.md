...
nav_order: 27
permalink: /technical-mastery/cloud-aws/rds/
id: AWS-041
category: "Cloud - AWS"
difficulty: "★★☆"
depends_on:
...
```
RDS instance:
  Instance class:  db.t3.micro (dev) → db.r7g.32xlarge
    (prod)
  Storage:         gp3 or io1/io2 EBS; autoscaling
    available
  Endpoint:        DB hostname (DNS round-robin Multi-AZ)

Multi-AZ:
  Primary → synchronous replication → Standby (different
    AZ)
...
resource "aws_db_instance" "main" {
  ...
  # RDS manages in Secrets Manager
  username                    = "dbadmin"
  ...
  final_snapshot_identifier =
      "main-postgres-final-${formatdate("YYYYMMDD", timestamp())}"
...
resource "aws_db_parameter_group" "postgres" {
  ...
  parameter {
    name  = "shared_buffers"
    # 25% of RAM for db.r7g.large (16GB)
    value = "4000000"
    # in 8KB blocks = ~32GB; capped to instance RAM
...
resource "aws_db_proxy" "main" {
  ...
  vpc_security_group_ids = [aws_security_group.rds.id]
  vpc_subnet_ids         = [aws_subnet.private_a.id,
      aws_subnet.private_b.id]

  auth {
    auth_scheme = "SECRETS"
    iam_auth    = "REQUIRED"
    # require IAM auth (no password in Lambda)
...
aws rds describe-db-instances \
  --query 'DBInstances[].{Id:DBInstanceIdentifier,Engine:Engine,
      Class:DBInstanceClass,Status:DBInstanceStatus}'
