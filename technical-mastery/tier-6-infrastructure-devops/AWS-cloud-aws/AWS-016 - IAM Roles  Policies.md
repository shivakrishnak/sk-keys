...
nav_order: 16
permalink: /technical-mastery/cloud-aws/iam-roles-policies/
id: AWS-025
category: "Cloud - AWS"
difficulty: "★★☆"
...
Role trust policy:  WHO can assume this role
  "Principal": {"Service": "lambda.amazonaws.com"}  →
    Lambda can assume
  "Principal": {"AWS": "arn:aws:iam::ACCOUNT:role/X"} →
    role X can assume
...
// → Can only manage EC2 instances tagged Team=payments +
// Environment=prod
```
...
4. App uses AWS SDK (auto-discovers credentials from
  metadata)

5. SDK call: dynamodb.getItem(...)
   → Signs request with temporary credentials
   → DynamoDB validates signature
   → CloudTrail: "who: payment-task-role, action: GetItem,
     table: payments"

6. Credentials expire: SDK auto-refreshes via metadata
  endpoint
```
