...
nav_order: 58
permalink: /technical-mastery/cloud-aws/eks/
id: AWS-052
category: "Cloud - AWS"
difficulty: "★★★"
depends_on:
...
EKS components:
  ...
  Worker nodes: EC2 Node Groups or Fargate profiles (you
    pay for EC2/Fargate)

Node Group options:
  Managed Node Group: AWS-managed EC2 ASG with K8s
    labels/taints
  Self-managed Node Group: you manage ASG
  Fargate Profile: serverless pods (no EC2 management)
  Karpenter: node provisioner (faster than Cluster
    Autoscaler)

Key AWS integrations:
  IRSA: IAM Roles for Service Accounts (pod-level AWS
    permissions)
...
module "eks" {
  ...
  # for control plane ENIs

  # Cluster endpoint access
  cluster_endpoint_public_access  = true
  # for kubectl from outside VPC
  cluster_endpoint_private_access = true
  cluster_endpoint_public_access_cidrs = ["10.0.0.0/8"]
  # restrict to corp IPs
    ...
    spec:
      ...
      containers:
        - name: app
          ...
          # AWS SDK auto-discovers credentials from projected service
          # account token
```
...
aws eks describe-cluster \
  --name my-cluster \
  --query 'cluster.{Status:status,K8sVersion:version,
      Endpoint:endpoint}'

# Check add-on versions
aws eks describe-addon-versions \
  --kubernetes-version 1.31 \
  --query 'addons[].{Name:addonName,
...
aws eks describe-nodegroup \
  ...
  --query 'nodegroup.{Status:status,Desired:scalingConfig.desiredSize,
      Min:scalingConfig.minSize,Max:scalingConfig.maxSize}'
