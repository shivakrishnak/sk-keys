...
nav_order: 62
permalink: /technical-mastery/cloud-aws/aws-cost-optimization/
id: AWS-066
category: "Cloud - AWS"
difficulty: "★★★"
...
Quick wins (implement first):
  ...
  - VPC Endpoints for S3/DynamoDB = eliminate NAT Gateway
    traffic
```
...
aws ec2 describe-addresses \
  --query 'Addresses[?AssociationId==null].{AllocationId:AllocationId,
      IP:PublicIp}'

# 3. Old, unneeded snapshots (running script to find candidates)
aws ec2 describe-snapshots \
  --owner-ids self \
  --query 'Snapshots[?StartTime<`2023-01-01`].{Id:SnapshotId,
      Date:StartTime,Size:VolumeSize}' \
  --output table
...
aws ec2 describe-instances \
  ...
  --query 'Reservations[].Instances[].{Id:InstanceId,
      Type:InstanceType,Stopped:StateTransitionReason}'
...
# 7. Find Lambda functions with over-provisioned memory (use Lambda
# Power Tuning)
...
resource "aws_savingsplans_savings_plan" "compute" {
  ...
  # Get offer IDs via: aws savingsplans
...
```hcl
# Replace NAT Gateway traffic for S3/DynamoDB with VPC Endpoints
# (FREE)
...
aws ce get-cost-and-usage \
  ...
  --query 'ResultsByTime[0].Groups[].{Service:Keys[0],
...
aws compute-optimizer get-ec2-instance-recommendations \
  --query 'instanceRecommendations[?findingReasonCodes[?contains(@,
      `OVERPROVISIONED`)]].[instanceArn,finding]'

# Set up budget alert
aws budgets create-budget \
  --account-id 123456789 \
  --budget '{"BudgetName":"Monthly","BudgetLimit":{"Amount":"5000",
      "Unit":"USD"},"TimeUnit":"MONTHLY","BudgetType":"COST"}' \
