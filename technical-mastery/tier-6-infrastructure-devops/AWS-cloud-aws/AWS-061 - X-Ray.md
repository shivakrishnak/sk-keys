...
nav_order: 61
permalink: /technical-mastery/cloud-aws/x-ray/
id: AWS-065
category: "Cloud - AWS"
difficulty: "★★★"
...
Core concepts:
  ...
  Segment:  work done by one service/component (has
    subsegments)
  Subsegment: work within a segment (DB call, HTTP
    request, function call)
  Sampling: % of requests traced (default: 5% + 1 req/sec
    = cost control)

Trace header: X-Amzn-Trace-Id propagated across service
  calls
...
public class OrderService {
    ...
    public Order createOrder(CreateOrderRequest request) {
        // X-Ray automatically captures HTTP requests and AWS SDK
        // calls
        ...
        return AWSXRay.createSubsegment("validateOrder", () -> {
            ...
            AWSXRay.createSubsegment("persistOrder", () -> {
                ...
                AWSXRay.getCurrentSegment().putAnnotation("userId",
                    request.getUserId());
                AWSXRay.getCurrentSegment().putAnnotation("orderType",
                     request.getOrderType());

                // Add metadata (not indexed, for debugging)
                AWSXRay.getCurrentSegment()
                    .putMetadata("orderDetails",
    ...
    public RestTemplate restTemplate() {
        ...
        // X-Ray auto-instruments if using AWSXRay
    }
}
```
