...
nav_order: 32
permalink: /technical-mastery/cloud-aws/api-gateway-aws/
id: AWS-057
category: "Cloud - AWS"
difficulty: "★★☆"
...
```
API Types:
  HTTP API:   $1.00/million requests; lower latency; JWT
    auth; simpler
  REST API:   $3.50/million requests; full features
    (caching, validation)
  WebSocket:  $1.00/million msgs + $0.25/million
    connection-minutes

Integration types:
  Lambda Proxy:      raw request to Lambda, Lambda formats
    response
  HTTP Integration:  proxy to HTTP endpoint (ECS, EC2, any
    URL)
  AWS Service:       integrate directly with SQS,
    DynamoDB, etc.
  Mock:              return static response (useful for
    CORS preflight)
...
resource "aws_apigatewayv2_route" "get_user" {
  ...
  target             =
      "integrations/${aws_apigatewayv2_integration.users.id}"
...
public class UserHandler implements
    ...
    private APIGatewayV2HTTPResponse response(int statusCode,
        String body) {
