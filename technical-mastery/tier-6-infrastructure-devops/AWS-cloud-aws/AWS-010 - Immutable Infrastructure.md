...
nav_order: 10
permalink: /technical-mastery/cloud-aws/immutable-infrastructure/
---

# AWS-014 - Immutable Infrastructure
...
```
Mutable (bad):
  Server v1 --> patch --> patch --> config change -->
    "unique snowflake"
  Cannot reproduce. Cannot roll back. Cannot trust.

Immutable (good):
  Image v1 → deploy → run
  Need change? → build Image v2 → deploy → replace v1 →
    terminate v1
  Every instance is traceable to a specific known image
    version.
```
...
Immutable server approach:
  ...
  Only break glass access for emergency diagnostic
    (read-only).
```
      ...
      containers:
        - name: myapp
          ...
          securityContext:
            ...
            # enforce immutability at container level
...
```
┌─────────────────────────────────────────────────────────┐
│ KEY IDEA     │ Never modify running infrastructure - bui│
│              │ new, replace old, terminate old          │
├─────────────────────────────────────────────────────────┤
│ USE WHEN     │ Cloud-native, containerized, IaC-driven  │
│              │ environments                             │
├─────────────────────────────────────────────────────────┤
│ AVOID WHEN   │ Legacy systems with state on disk that ca│
│              │ yet be externalised                      │
├─────────────────────────────────────────────────────────┤
│ ONE-LINER    │ "Build, don't patch - every running insta│
│              │  is a known, version-controlled artifact"│
├─────────────────────────────────────────────────────────┤
│ NEXT EXPLORE │ Docker --> IaC --> GitOps --> Twelve-Fact│
└─────────────────────────────────────────────────────────┘
```
