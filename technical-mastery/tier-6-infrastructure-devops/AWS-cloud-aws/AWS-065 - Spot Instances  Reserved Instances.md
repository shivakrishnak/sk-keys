...
nav_order: 65
permalink: /technical-mastery/cloud-aws/spot-instances-reserved-instances/
id: AWS-014
category: "Cloud - AWS"
difficulty: "★★★"
...
Spot Instances:
  ...
  - Risk: 2-minute interruption notice when AWS needs
    capacity
  - Best for: batch processing, CI/CD workers, ECS/EKS
    worker nodes
  - Key pattern: Spot + On-Demand fallback; diversify
    instance types

Reserved Instances:
  - Commitment: 1 or 3 years, specific instance type +
    region
  - Savings: 36% (1yr no-upfront) to 75% (3yr all-upfront)
  - Types: Standard (fixed, cannot change) / Convertible
    (flexible)
...
Savings Plans (preferred over RIs):
  ...
  - Flexibility: automatically applies to any instance
    family/region
```
    ...
    public void checkForInterruptionNotice() {
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                // save progress
                checkpointService.gracefulShutdown();
  ...
  mixed_instances_policy {
    ...
    launch_template {
      ...
      override {
        ...
        # same specs, different CPU vendor
      }
      override {
...
aws ec2 describe-spot-price-history \
  ...
  --query 'SpotPriceHistory[].{Type:InstanceType,Price:SpotPrice,
      AZ:AvailabilityZone,Time:Timestamp}'
```
...
# Check Spot interruption rate by instance type (AWS Spot Advisor =
# manual)
...
aws savingsplans describe-savings-plans \
  --states active \
  --query 'savingsPlans[].{Arn:savingsPlanArn,Commitment:commitment,
      Savings:savingsPlanType}'
