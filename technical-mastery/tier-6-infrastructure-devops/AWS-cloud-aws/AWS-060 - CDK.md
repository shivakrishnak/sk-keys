...
nav_order: 60
permalink: /technical-mastery/cloud-aws/cdk/
id: AWS-064
category: "Cloud - AWS"
difficulty: "★★★"
...
CDK workflow:
  ...
  3. cdk diff       → shows infrastructure changes (like
    terraform plan)
  4. cdk deploy     → deploys via CloudFormation
  5. cdk destroy    → deletes stack

Construct levels:
  L1 (CfnBucket):   Direct CloudFormation resource, all
    properties
  L2 (Bucket):      Opinionated with smart defaults +
    helper methods
  L3 (StaticWebsite): Complete pattern (S3 + CloudFront +
    Route53)
...
export class VpcStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // L2 VPC construct: creates VPC + public/private subnets + NAT GW
    // + route tables
...
export class AppStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: AppStackProps) {
    ...
    const fargateService = new ecs.FargateService(this,
        "AppService", {
      cluster,
    ...
    const processorFunction = new lambda.Function(this,
        "OrderProcessor", {
    ...
    ordersQueue.grantConsumeMessages(processorFunction);
    // SQS consumer
  ...
  private createTaskDefinition(
      vpc: ec2.Vpc): ecs.FargateTaskDefinition {
