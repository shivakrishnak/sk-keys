...
nav_order: 53
permalink: /technical-mastery/cloud-aws/transit-gateway/
id: AWS-051
category: "Cloud - AWS"
difficulty: "★★★"
...
resource "aws_ec2_transit_gateway_vpc_attachment" "prod_vpc" {
  ...
  subnet_ids         = [aws_subnet.prod_tgw_a.id,
...
resource "aws_ec2_transit_gateway_route_table_association" "prod_vpc" {
  transit_gateway_attachment_id  =
      aws_ec2_transit_gateway_vpc_attachment.prod_vpc.id
...
Centralized Egress with TGW (cheaper at scale):
  ...
  All private VPCs → TGW → egress-vpc → NAT Gateway →
    internet
...
But also: single routing policy point for internet access
  controls
```
