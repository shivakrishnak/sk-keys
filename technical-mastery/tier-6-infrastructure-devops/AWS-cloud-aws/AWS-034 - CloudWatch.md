...
nav_order: 34
permalink: /technical-mastery/cloud-aws/cloudwatch/
id: AWS-031
category: "Cloud - AWS"
difficulty: "★★☆"
depends_on:
...
CloudWatch Metrics:
  ...
  Resolution: standard (1min), high-resolution (1s, extra
    cost)
  Retention: 3hr (1-sec), 15days (1-min), 63days (5-min),
    455days (1-hr)

CloudWatch Logs:
  Log Group: collection of log streams (e.g.,
...
public class OrderMetrics {
    ...
    public void recordOrderProcessingTime(String orderId,
        ...
        List<MetricDatum> metrics = List.of(
            MetricDatum.builder()
                ...
                .dimensions(
                    Dimension.builder().name("Service").value(appName)
                        .build(),
                    Dimension.builder().name("Success")
                        .value(String.valueOf(success)).build()
                )
...
public class OrderService {
    ...
    public void processOrder(Order order) {
        ...
        try {
            ...
            // Structured log with metrics embedded (EMF format for
            // Lambda)
            logger.info("""
                {"_aws":{"Timestamp":{},
                    "CloudWatchMetrics":[{"Namespace":"MyApp/Orders",
                        "Dimensions":[["Service"]],
                            "Metrics":[{"Name":"OrderDuration",
                                "Unit":"Milliseconds"},
                                    {"Name":"OrderSuccess",
                                        "Unit":"Count"}]}]},
...
resource "aws_cloudwatch_composite_alarm" "critical" {
  ...
  # only page if BOTH alarms trigger
}
...
| stats count(@initDuration) as coldStarts,
  avg(@initDuration) as avgInitMs by bin(1h)

# P99/P95/P50 latency over time
fields @timestamp, @duration
| filter @type = "REPORT"
| stats pct(@duration, 50) as p50, pct(@duration, 95) as
  p95, pct(@duration, 99) as p99 by bin(5m)
...
aws logs start-query \
  ...
  --query-string 'fields @timestamp,
      @message | filter @message like /ERROR/ | limit 20'
