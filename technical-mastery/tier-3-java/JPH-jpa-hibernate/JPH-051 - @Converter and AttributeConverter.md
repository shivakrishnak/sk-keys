...
nav_order: 51
permalink: /technical-mastery/jpa-hibernate/attribute-converter/
---
...
// 6 months later: requirement adds "ON_HOLD" between PENDING and
// PROCESSING:
// enum OrderStatus { PENDING, ON_HOLD, PROCESSING, ... }
// NOW: all existing DB rows with status=1 (PROCESSING) are misread as
// ON_HOLD
...
LOAD (SELECT):
  ...
  -> JPA calls:
    converter.convertToEntityAttribute("100.00:USD")
...
JPQL/Criteria query on converted field:
  ...
  NOTE: The converter is applied to parameters, not just
    entities.
```
...
├──────────────┼──────────────────────────────────────────┤
│ REGISTER     │ @Converter(autoApply=true) -> all X field│
