...
nav_order: 52
permalink: /technical-mastery/jpa-hibernate/dirty-checking-flush/
---
...
At em.find(Product.class, 42):
  ...
  3. Store hydrated state (snapshot) in SessionImpl:
     entityEntries map: {id=42} -> Object[]{42, "Widget",
       100.00}
...
At em.flush() (or before JPQL query in AUTO mode):
  1. For each managed entity in session:
     ...
     d. Generate: UPDATE products SET price=90.00 WHERE
       id=42
...
```
Scenario: 50 JPQL queries in one transaction, 100 managed
  entities

FlushModeType.AUTO:
  - Each JPQL query checks: "do pending changes affect
    this query?"
  - If any dirty entities match the query's entity type:
    FLUSH FIRST
  - Potential: up to 50 flush cycles (worst case: every
    query triggers flush)
  - Dirty check: 100 entities * 50 flush opportunities =
    5,000 comparisons

FlushModeType.COMMIT:
  - No automatic flush before queries
  - One dirty check at commit time: 100 entities * 1 flush
    = 100 comparisons
  - Risk: JPQL queries may not see pending in-memory
    changes
  - Use only when you know queries don't depend on pending
    changes
...
em.createQuery("SELECT p FROM Product p WHERE
  p.active=true"):
...
@Transactional method returns:
  ...
  -> Flush (final):
     ...
     - No other dirty entities (all cleaned after earlier
       flush)
  -> Transaction commits
...
```
┌─────────────────────────────────────────────────────────┐
│ DIRTY CHECK  │ entity state != snapshot -> UPDATE at flu│
