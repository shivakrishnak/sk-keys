...
nav_order: 60
permalink: /technical-mastery/jpa-hibernate/hibernate-6-and-jakarta-persistence-3-migration/
---
...
BREAKING CHANGE 2: Hibernate 6 type system overhaul
  ...
  Hibernate 6 equivalents:
    ...
    // For custom types: implement UserType<T> (H6
      interface)
...
BREAKING CHANGE 3: HQL/JPQL query changes
  ...
  Removed: implicit join path from SELECT without FROM
    entry
    H5 (worked): SELECT e.department.name FROM Employee e
    H6 (works):  SELECT d.name FROM Employee e JOIN
      e.department d
...
Wrong migration order (common mistake):
  ...
  2. Compile -> 500 errors (all javax.persistence.* not
    found)
...
Right migration order:
  ...
  4. Then: upgrade pom.xml boot version 2.7 -> 3.0
     Fix remaining compilation errors (from OpenRewrite
       misses)
  5. Run all tests; fix failures
  6. Deploy to staging; verify behavior

Each step is independently releasable.
The javax -> jakarta rename is ~95% automated by
  OpenRewrite.
The Hibernate type system changes (5%) require manual
  review.
```
...
Breaking changes from H5 -> H6 SQM:
  ...
  - Query plan cache key changed: warm-up needed post
    deploy
```
...
Pre-migration:
  ...
  [ ] All tests passing
  [ ] Baseline metrics captured (response times, DB query
    counts)
...
Step 2 - Manual fixes:
  ...
  [ ] Update hypersistence-utils: hibernate-55 ->
    hibernate-60
  ...
  [ ] Review and fix HQL queries (if implicit join
    warnings)
...
Step 3 - Test and validate:
  ...
  [ ] Check N+1 behavior (H6 may change lazy loading
    behavior)
