...
nav_order: 44
permalink: /technical-mastery/jpa-hibernate/hibernate-validator/
---
...
Level 2: Service (@Validated on class, constraints on
  params)
  -> ConstraintViolationException (unchecked)
  -> AOP proxy intercepts; Spring does NOT auto-map to
    HTTP 400
  -> Must catch and convert to HTTP 400 in
    @ControllerAdvice

Level 3: JPA (before INSERT/UPDATE)
  -> Hibernate calls Validator.validate(entity) before
    flush
  -> ConstraintViolationException wraps javax.validation
    violations
  -> Can catch in @Transactional method or propagates to
    controller
...
public User createUser(@Valid @RequestBody UserDto dto) {
    ...
    // -> Spring: HTTP 400 with {"field": "email", "message": "must
    // not be null"}
