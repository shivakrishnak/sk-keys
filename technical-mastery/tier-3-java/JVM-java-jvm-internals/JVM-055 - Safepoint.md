...
nav_order: 55
permalink: /technical-mastery/java/safepoint/
id: JVM-059
...
[GC detects Eden full / explicit request]
    ...
    → [Time To Safepoint: varies 50µs – 50ms]  ← YOU ARE
      HERE
    → [All threads stopped]
...
[One thread has tight counted loop (no safepoint poll)]
    ...
    → [Fix: -XX:+UseCountedLoopSafepoints or add method
      call]
```
...
├──────────────┼──────────────────────────────────────────┤
│ USE WHEN     │ Automatic; enable UseCountedLoopSafepoint│
...
├──────────────┼──────────────────────────────────────────┤
│ NEXT EXPLORE │ Card Table → Write Barrier → Remembered S│
└─────────────────────────────────────────────────────────┘
```
