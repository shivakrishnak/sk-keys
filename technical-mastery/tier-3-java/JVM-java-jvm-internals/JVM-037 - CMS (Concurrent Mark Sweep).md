...
nav_order: 37
permalink: /technical-mastery/java/cms-concurrent-mark-sweep/
id: JVM-021
...
Key: most work (concurrent mark + sweep) happens while app
  runs
Young Gen still uses same Minor GC (STW) as Parallel GC
No compaction → fragmentation over time → can trigger
...
Phase 2: Concurrent Mark (no STW, ~seconds)
  - GC thread traverses object graph from Initial Mark
    results
  - App threads continue mutating heap concurrently
  - Write barriers capture reference changes during this
    phase
  - Tri-color marking: white (unvisited), gray
    (in-progress), black (done)
...
Result: "concurrent mode failure"
  ...
  → Falls back to SERIAL single-threaded Full GC (STW,
    minutes!)
  
Prevention:
  -XX:CMSInitiatingOccupancyFraction=70  (start CMS when
    70% full)
  -XX:+UseCMSInitiatingOccupancyOnly     (don't auto-tune
    this value)
  → Start CMS early enough that it finishes before Old Gen
    fills
```
...
╚══════╩══╩════════╩══╩══════════════╩══╝
         ▲   free    ▲   free             ← many small
           holes

Large object allocation (e.g., 1 MB array):
- Total free: 5 MB, but no contiguous 1 MB block
- Allocation fails → concurrent mode failure → serial Full
  GC

Mitigation: -XX:+CMSFullGCsBeforeCompaction=N
(Compact after N Full GCs - but Full GC is still a long
  STW)
```
...
```
Problem: What if app thread changes reference DURING
  concurrent mark?
  Object A (black/done) → points to B → app changes A to
    point to C
  Now C was never visited by GC → C incorrectly reclaimed
    as garbage!
...
Solution: Card Table + Write Barrier
  ...
  - Final Remark re-scans all dirty cards to catch these
    changes
  
Performance cost:
  - Every reference write has a small overhead (write
    barrier)
...
# Start CMS when Old Gen is 70% full
-XX:+UseCMSInitiatingOccupancyOnly
# Don't override occupancy fraction
...
❌ Not setting CMSInitiatingOccupancyFraction
   → JVM auto-tunes; may start CMS too late → concurrent
     mode failure
...
❌ Expecting CMS to compact Old Gen
   → CMS never compacts; only Full GC compacts (as
     fallback)

❌ Heavy use of soft/weak references with CMS
   → Reference processing in Final Remark can extend STW
     significantly
```
...
```
🚫 Anti-Pattern: Using CMS in Java 9+ without explicit
  enablement
...
🚫 Anti-Pattern: Ignoring fragmentation metrics
   Problem:  CMS runs fine for weeks, then sudden long
     Full GC pauses
   Fix:      Monitor Old Gen fragmentation; plan GC
     migration
...
└── concept seeds ───► Concurrent marking → G1, ZGC,
  Shenandoah
```
