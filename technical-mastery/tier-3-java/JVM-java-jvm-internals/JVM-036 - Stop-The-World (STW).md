...
nav_order: 36
permalink: /technical-mastery/java/stop-the-world-stw/
id: JVM-041
...
TTSP problem:
- If Thread A is in a native sleep(10000), all other
  threads
...
All GC phases are STW:
  STW: [Mark all live objects] → [Sweep garbage] →
    [Compact heap]
...
❌ Ignoring time-to-safepoint in GC logs
   → Long TTSP looks like long GC pause but root cause is
     JNI code
...
❌ Long-running JNI native calls
   → Delays all threads reaching safepoint → apparent STW
     inflation

❌ Using Thread.sleep() in tight loops
   → Thread may not yield safepoint quickly (context
     dependent)
...
🚫 Anti-Pattern: Choosing GC algorithm without heap size
  consideration
   Problem:  Serial GC on 32 GB heap = minutes of STW
   Fix:      Match GC algorithm to heap size and latency
     requirements

🚫 Anti-Pattern: Ignoring STW from non-GC sources in
  latency analysis
