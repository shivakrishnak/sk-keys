...
nav_order: 48
permalink: /technical-mastery/java/method-inlining/
id: JVM-052
...
After Inlining:
  caller IR: ... IF x == null THROW; result = x.length > 0
    → use result ...
```
...
[Inlined type guard fails at runtime]
    ...
    → [Bimorphic inline: both types inlined with if/else
      check]
```
...
// could be ArrayList, LinkedList, etc.
for (int i = 0; i < 1_000_000; i++) {
