...
nav_order: 29
permalink: /technical-mastery/java-language/string-pool-string-interning/
id: JLG-068
...
intern() algorithm:
  ...
  3. Walk bucket's chain: compare each entry by value
    (.equals())
...
[Java class loads with literal "Content-Type"]
    ...
    → [JVM: String Table lookup for "Content-Type"]  ← YOU
      ARE HERE
    → [Found: return existing reference]
    → [All classes with "Content-Type" literal: same
      reference]
...
[Excessive runtime intern() calls]
    ...
    → [Fix: reduce intern() usage, use explicit caches
      instead]
```
...
// null-safe if status might be null
// Or:
...
│ SOLVES       │ repeated string values (literals, constan│
├──────────────┼──────────────────────────────────────────┤
│ KEY INSIGHT  │ String literals are ALWAYS pooled; runtim│
│              │ Strings are NOT unless explicitly intern(│
│              │ → NEVER use == for String value compariso│
├──────────────┼──────────────────────────────────────────┤
