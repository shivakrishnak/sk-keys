...
nav_order: 37
permalink: /technical-mastery/java-language/functional-interfaces/
id: JLG-036
...
**NORMAL FLOW:**
```
[Developer writes: users.stream().filter(u ->
  u.isActive())]
    → [Lambda: u -> u.isActive()]
    → [Compiler infers target type: Predicate<User>]  ←
      YOU ARE HERE
...
**FAILURE PATH:**
```
[Developer uses Function<String,String> where Consumer
  needed]
    → [Compiler: incompatible types - return value not
      void]
...
```
┌─────────────────────────────────────────────────────────┐
│ WHAT IT IS   │ Interface with exactly one abstract metho│
...
│              │ Java - pass them, store them, compose the│
├──────────────┼──────────────────────────────────────────┤
