...
nav_order: 39
permalink: /technical-mastery/java-language/lambda-expressions/
id: JLG-038
...
count++;
// COMPILE ERROR if this line exists: count must be effectively final
```
...
[Source: list.stream().filter(u -> u.isActive())]
    → [Compiler: lambda body → private static method]  ←
      YOU ARE HERE
...
List<Order> bigOrders = orders.stream()
    ...
    // ^^^ threshold captured - new lambda per call if threshold
    // varies
    .collect(toList());
```
...
class OrderService {
    ...
    Runnable createTask(Long userId) {
        // Captures 'this' (OrderService) implicitly to reach
        // userService
