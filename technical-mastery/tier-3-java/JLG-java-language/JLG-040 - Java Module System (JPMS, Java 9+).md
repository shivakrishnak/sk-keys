...
nav_order: 40
permalink: /technical-mastery/java-language/java-module-system-jpms/
---
...
module com.example.app {
    ...
    // transitive: re-exports to dependents
...
lib-impl.jar: com.example.impl.FastService (public,
  internal)

User code: new com.example.impl.FastService() // works!
  Unintended coupling
```
...
module lib-impl {
    requires lib-api;
    exports com.example.impl to lib-api; // ONLY to
      lib-api internally
    provides com.example.api.Service with
      com.example.impl.FastService;
}
// User code: new com.example.impl.FastService()
// -> InaccessibleObjectException at compile time AND
  runtime
```
...
3. For each requires X: add X to graph, transitively
  resolve
4. Verify no missing modules (all requires satisfied)
5. Verify no split packages (same package in two modules -
  illegal)
6. Install readability: A reads B means A can see B's
  exports
7. Accessibility: enforced at Class.forName / field access
  time
```
...
**NORMAL FLOW:**
```
Compile: javac --module-path libs -d out
  ...
  Compiler enforces exports (compile-time access check)
       |
Runtime: java --module-path libs:out -m
  myapp/com.example.Main
       |    <- YOU ARE HERE
...
-> X is on classpath (jar) not module path (missing
  module-info)
...
module com.example.lib {
    requires java.base;
    // implicitly required; explicit is fine
    ...
    // allow Jackson/Hibernate reflection
}
```
