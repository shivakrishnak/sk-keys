...
nav_order: 35
permalink: /technical-mastery/java-language/optional/
id: JLG-072
...
Optional<String> city = findUser(id)
    ...
    .map(Address::getCity);
    // getCity returns String, not Optional
```
...
[Caller: optional.get() on empty Optional]
    ...
    → [Better: use .orElse(), .orElseThrow(), or
      functional API]
```
...
│              │ functional API safety vs verbose imperati│
├──────────────┼──────────────────────────────────────────┤
