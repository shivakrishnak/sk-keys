...
nav_order: 4
permalink: /technical-mastery/jcc/concurrency-vs-parallelism-in-java/
---
...
```bash
# Check carrier thread count (should equal CPU cores for CPU-bound
# work)
jstack <pid> | grep "ForkJoinPool\|carrier"
# If virtual thread count >> carrier thread count for CPU work, it's
# over-threaded
```
