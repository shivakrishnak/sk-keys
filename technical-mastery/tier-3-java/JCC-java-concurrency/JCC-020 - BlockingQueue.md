...
nav_order: 20
permalink: /technical-mastery/java-concurrency/jcc-039-blockingqueue/
id: JCC-044
...
┌──────────────────────┬───────────┬──────────────┬────────
│ Operation            │ Full queue│ Empty queue  │ Return│
├──────────────────────┼───────────┼──────────────┼───────┤
│ put(e)               │ BLOCKS    │ N/A (insert) │ void  │
│ offer(e)             │ false     │ N/A          │ boolea│
│ offer(e, time, unit) │ BLOCKS    │ N/A          │ boolea│
│ take()               │ N/A       │ BLOCKS       │ elemen│
│ poll()               │ N/A       │ null         │ elemen│
│ poll(time, unit)     │ N/A       │ BLOCKS       │ elemen│
└──────────────────────┴───────────┴──────────────┴────────
```
...
Put flow: lock → check count==capacity → if full:
  notFull.await()
         → insert at putIndex → count++ → signal notEmpty
           → unlock

Take flow: lock → check count==0 → if empty:
  notEmpty.await()
          → remove at takeIndex → count-- → signal notFull
            → unlock
```
...
```
NORMAL FLOW (ThreadPoolExecutor uses BlockingQueue
  internally):
...
→ Or: CallerRunsPolicy → calling thread executes task
  itself
