...
nav_order: 58
permalink: /technical-mastery/java-concurrency/structured-concurrency/
id: JCC-021
...
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
Invariant: scope.close() called AFTER all forked tasks
  finish
```
...
→ join() returns → throwIfFailed() (no-op) → results
  collected
...
→ join() returns → throwIfFailed() throws
  ExecutionException
...
exhausting OS threads. At scale, the
  parent-outlives-children
guarantee prevents thundering-herd thread leaks when
  requests
