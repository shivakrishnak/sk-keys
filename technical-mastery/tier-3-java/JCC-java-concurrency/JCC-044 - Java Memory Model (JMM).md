...
nav_order: 44
permalink: /technical-mastery/java-concurrency/java-memory-model/
id: JCC-062
...
// Because: write(data) HB(prog order) write(flag) HB(volatile)
// read(flag) HB(prog order) read(data)
```
...
[T1: write(x=42) then write(volatile flag=true)]
    → [JMM: write(x) HB write(flag) by program order]  ←
      YOU ARE HERE
    → [JMM: write(flag=true) HB read(flag=true) by
      volatile rule]
    → [T2: read(volatile flag) = true]
    → [JMM: read(flag) HB read(x) by program order]
    → [T2: read(x) = 42 - GUARANTEED visible]
    → [HB chain: T1.write(x) → T1.write(flag) →
      T2.read(flag) → T2.read(x)]
```
...
[T1: write(x=42) then write(flag=true) - both non-volatile]
    ...
    → [T2: read(flag=true) but read(x=0) - valid JMM
      behavior]
