...
difficulty: ★★★
depends_on: JCC-078, JCC-079, JCC-077
used_by:
related: JCC-060, JCC-064, JCC-016
tags:
  - java
  - concurrency
  - observability
...
nav_order: 80
permalink: /technical-mastery/java-concurrency/memory-visibility-diagnostics-jstack-jfr/
---
...
| Field      | Value                                              |
|------------|----------------------------------------------------|
| Depends on | JCC-078 JMM Happens-Before, JCC-079 Lock-Free Data Structures, JCC-077 Thread Pinning |
| Used by    | (advanced diagnostics endpoint)                    |
| Related    | JCC-060 Atomic Classes, JCC-064 Thread Dump Analysis, JCC-016 Java Memory Model |
...
3. JFR dump shows: 10 second pause for JavaMonitorBlocked
  on
   UserService$sessionCache (same timestamp)
4. JFR shows GCBefore: GC pause = 0ms (not the cause)
5. Root cause: UserService.getUser() calls an external
  HTTP API
...
- [[JCC-078 - JMM Happens-Before - Deep Rules]] - what you're
  diagnosing at the memory level
- [[JCC-079 - Lock-Free Data Structures]] - what lock-free
  monitoring reveals about CAS contention
- [[JCC-064 - Thread Dump Analysis]] - pattern recognition in
  jstack output
