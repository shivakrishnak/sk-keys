...
nav_order: 62
permalink: /technical-mastery/java-concurrency/lock-striping/
id: JCC-025
...
16-stripe lock:
  ...
  Thread 2 ──get("b")──► LOCK[stripe 11]──get──►
    UNLOCK[11]   (parallel!)
  Thread 3 ──put("c")──► LOCK[stripe 7] ──put──► UNLOCK[7]
    (parallel!)
...
JAVA 7 ConcurrentHashMap (Segment-based striping):
  ...
  3. Segment[7].lock()                   ← only segment 7
    locked
  4. Segment[7].put("alice", value)      ← write into
    segment 7
  5. Segment[7].unlock()

  put("bob", value):                     ← concurrent,
    hash → segment 3
  1. Segment[3].lock()                   ← different
    segment
  2. Segment[3].put("bob", value)        ← parallel with
    step 4 above!
  3. Segment[3].unlock()
...
JAVA 8 ConcurrentHashMap (per-bucket CAS + synchronized):
  ...
  2. If bucket empty: CAS to insert first node (no lock
    needed)
  3. If bucket non-empty: synchronized(bucket.headNode) {
    insert }
...
Thread 2 acquires lock on bucket 11 - PARALLEL (no
  conflict)
...
parallelism. Beyond that, per-entry locking (Java 8 CHM
  style)
or lock-free CAS is needed. At extreme scale (millions of
concurrent operations), striping overhead - 16 lock
  objects -
