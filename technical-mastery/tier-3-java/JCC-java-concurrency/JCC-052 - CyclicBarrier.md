...
nav_order: 52
permalink: /technical-mastery/java-concurrency/jcc-037-cyclicbarrier/
id: JCC-070
...
State: count = parties
  ...
  → count == 0: signal all + run barrier action + reset
    count
...
→ After timeout: BrokenBarrierException thrown in T1, T2,
  T4
→ Observable: BrokenBarrierException in logs for all
  awaiting threads
→ Fix: wrap work in try-catch; call barrier.reset() or
  cancel
