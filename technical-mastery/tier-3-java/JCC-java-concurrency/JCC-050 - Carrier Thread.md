...
nav_order: 50
permalink: /technical-mastery/java-concurrency/carrier-thread/
id: JCC-068
...
T=10ms: VT "order-2" calls Socket.read() → unmount
          mounting VT "order-1" → continues after
            Socket.read()
...
```
...
synchronized (sharedLock) {         // pin begins
    data = socket.read();
    // carrier blocked - cannot release
    process(data);
}                                   // pin ends
...
[VT "order-1" ready to run]
    → [FJPool: schedule VT on available carrier]     ← YOU
      ARE HERE
