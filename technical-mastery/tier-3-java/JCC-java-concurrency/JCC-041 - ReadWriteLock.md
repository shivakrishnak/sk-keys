...
nav_order: 41
permalink: /technical-mastery/java-concurrency/read-write-lock/
id: JCC-059
...
```java
// Downgrade: acquire read lock while holding write, then release
// write
writeLock.lock();
try {
...
[Non-fair mode, continuous readers]
    ...
    → [Fix: use fair=true, or StampedLock with write
      priority]
```
...
```
┌─────────────────────────────────────────────────────────┐
│ WHAT IT IS   │ Read lock (shared) + write lock (exclusiv│
│              │ - parallel reads, exclusive writes       │
├──────────────┼──────────────────────────────────────────┤
│ PROBLEM IT   │ synchronized serializes readers needlessl│
│ SOLVES       │ reads are safe to run in parallel        │
├──────────────┼──────────────────────────────────────────┤
│ KEY INSIGHT  │ Faster only when reads >> writes. For    │
│              │ balanced workloads, synchronized is faste│
...
├──────────────┼──────────────────────────────────────────┤
│ ONE-LINER    │ "Many can read, one can write - never bot│
├──────────────┼──────────────────────────────────────────┤
