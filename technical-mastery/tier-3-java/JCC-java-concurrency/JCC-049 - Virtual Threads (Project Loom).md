...
nav_order: 49
permalink: /technical-mastery/java-concurrency/virtual-threads/
id: JCC-067
...
```
10 threads → max 10 DB calls in parallel → 1000 batches ×
  10ms = 10s total
...
[Virtual thread VT1: httpClient.send(request)]
    → [JDK: async socket registered with NIO selector]  ←
      YOU ARE HERE
    ...
    → [VT1: continuation restored, httpClient.send()
      returns]
