...
nav_order: 19
permalink: /technical-mastery/java-concurrency/count-down-latch/
id: JCC-043
...
Usage pattern 1: one waits for many
  ...
  worker3: work() → latch.countDown()  // count: 1→0 →
    RELEASE
...
[CountDownLatch(3) created: count=3]
    ...
    → [main thread: latch.await()]              ← YOU ARE
      HERE
...
├──────────────┼──────────────────────────────────────────┤
│ ONE-LINER    │ "Wait for N things - fire once, stays ope│
├──────────────┼──────────────────────────────────────────┤
│ NEXT EXPLORE │ CyclicBarrier → Phaser → CompletableFutur│
└─────────────────────────────────────────────────────────┘
```
