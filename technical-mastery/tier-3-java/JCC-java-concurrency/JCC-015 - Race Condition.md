...
nav_order: 15
permalink: /technical-mastery/java-concurrency/race-condition/
id: JCC-039
...
class Registry {
    ...
    Object get() {
        if (instance == null) {          // check
            instance = new Object();
            // act - two threads can both create!
        }
        return instance;
    }
    ...
    // Or: Class holder pattern (thread-safe lazy init via class
    // loading)
}
```
...
[T1: balance = 1000, amount = 700]
    ...
    → [T2: check: 1000 >= 700 → true (T1 hasn't deducted
      yet!)]
...
│              │ timing - different ordering = wrong resul│
├──────────────┼──────────────────────────────────────────┤
