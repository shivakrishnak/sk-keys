...
nav_order: 48
permalink: /technical-mastery/java-concurrency/fork-join-pool/
id: JCC-066
...
When T1 calls join(subtask) and subtask is not done:
  → T1's thread tries to execute pending tasks from its
    deque
...
[pool.invoke(SumTask(0, 1M))]
    → [Worker T1: compute() - range > threshold, fork]  ←
      YOU ARE HERE
