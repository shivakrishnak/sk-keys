...
nav_order: 55
permalink: /technical-mastery/java-concurrency/jcc-041-copyonwritearraylist/
id: JCC-073
...
WRITE SEQUENCE (add):
  ...
  try {
    ...
    Object[] newArr = Arrays.copyOf(current,   // COPY
                       current.length + 1);
    newArr[current.length] = element;          // modify
      copy
    setArray(newArr); // volatile write: publish atomically
  } finally { lock.unlock(); }

READ SEQUENCE (iterator):
  Object[] snapshot = getArray(); // volatile read: take
    snapshot
...
Thread R3:                             [reading snapshot
  v2]

R1 and R2 see list without new element (their snapshot is
  v1)
...
ApplicationEvent fired → EventMulticaster iterates
  listeners
...
→ add() executes: copy array v42 → v43, insert, volatile
  write
→ Existing iterators unaffected (still on v42)
→ Next event dispatch: gets snapshot v43 → sees new
  listener
...
├──────────────┼──────────────────────────────────────────┤
│ NEXT EXPLORE │ ReadWriteLock → Persistent Data Structure│
