...
nav_order: 39
permalink: /technical-mastery/java-concurrency/wait-notify-notifyall/
id: JCC-057
...
```java
while (buffer.isEmpty()) { /* spin */ }
// consumes 100% CPU doing nothing
```
...
[Consumer: checks isEmpty() → true outside synchronized!]
    → [Producer: adds item, calls notify() - NO ONE
      WAITING]
    → [Consumer: lock.wait() - waits forever]
    → [Missed notification: state check before entering
      monitor]
