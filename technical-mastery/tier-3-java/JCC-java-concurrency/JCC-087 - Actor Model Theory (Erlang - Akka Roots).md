...
nav_order: 87
permalink: /technical-mastery/java-concurrency/actor-model-theory-erlang-akka-roots/
---
...
// balance accessed by exactly one thread at a time (actor processes
// one msg)
...
Behavior<AccountMsg> accountBehavior(int initialBalance) {
    return Behaviors.receive(AccountMsg.class)
        .onMessage(Deposit.class, (state, msg) -> {
            ...
            return accountBehavior(newBalance);
            // new behaviour = new state
        })
...
InventoryActor throws RuntimeException during GetStock
  ...
  -> No shared state corrupted - inventory state = clean
    init
```
...
public class AccountActor {
    ...
    record Debit(int amount,
        ActorRef<Response> replyTo) implements Cmd {}
