...
nav_order: 51
permalink: /technical-mastery/java-concurrency/continuation/
id: JCC-069
...
StackChunk (heap object):
  ...
  - data: actual stack frame bytes (local vars, return
    addresses)
...
[VT "order-5" calls socket.read() - blocking op]
    → [JVM: detect blocking → initiate yield]     ← YOU
      ARE HERE
    → [Continuation: copy stack frames to StackChunks on
      heap]
    ...
    → [Continuation: copy StackChunks back to carrier
      stack]
...
// In application code, you use virtual threads; continuations are
// internal
