...
nav_order: 47
permalink: /technical-mastery/java-concurrency/thread-pool-executor/
id: JCC-065
...
executor.setRejectedExecutionHandler((r, pool) -> {
    ...
    log.warn("Task rejected - pool full, max={}",
...
[submit(task) when corePoolSize reached and queue empty]
    → [queue.offer(task) → queued]             ← YOU ARE
      HERE
