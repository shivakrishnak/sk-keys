...
nav_order: 61
permalink: /technical-mastery/java-concurrency/jcc-047-deadlock-detection/
id: JCC-024
...
Strategy 1: LOCK ORDERING (most reliable)
  ...
  Thread 1: acquire lockA (order 1) → acquire lockB (order
    2)
  Thread 2: acquire lockA (order 1) → acquire lockB (order
    2)
  → Both threads request in same order → no cycle → no
    deadlock
...
public class DeadlockMonitor {
    ...
    public void checkForDeadlocks() {
        ...
        if (deadlockedThreadIds != null) {
            // Get full thread info including stack traces and
            // monitors
...
JVM returns array of deadlocked thread IDs
  ...
  → Root cause: lock ordering violation in
    ServiceA.method()
...
More services = more shared locks = higher deadlock
  probability.
At microservice scale: distributed deadlocks (service A
  waits for
service B which waits for service A) are not detectable by
  JVM
MXBean - require distributed tracing and timeout-based
  detection.
```
