...
nav_order: 56
permalink: /technical-mastery/java-concurrency/jcc-042-atomic-classes/
id: JCC-074
...
│                      │ soft-deletion in lock-free struct│
├──────────────────────┼──────────────────────────────────┤
...
Thread B: removes nodeX, adds nodeY, removes nodeY,
  re-adds nodeX
Thread A: CAS(nodeX, newNode) SUCCEEDS (sees nodeX) but
          nodeX's state is now different than when A read
            it
