...
nav_order: 59
permalink: /technical-mastery/java-concurrency/scoped-values/
id: JCC-022
...
USER.get() at Level 3: walks chain → finds charlie (level
  3)
USER.get() at Level 2: walks chain → finds bob (level 2)
TRACE_ID.get() at Level 3: walks chain → finds abc123
  (level 1)
LOCALE.get() at Level 1: walks chain → not found →
    ...
    try (var scope = new
      StructuredTaskScope.ShutdownOnFailure()) {
        scope.fork(() -> {
            ...
            USER.get() == alice ✓
            // Can nest: ScopedValue.where(USER,
              bob).run(...)
            // → creates local override without affecting
              parent
        });
        scope.join();
    }
...
→ Handler fans out to parallel DB calls via
  StructuredTaskScope
