...
difficulty: ★★★
depends_on: JCC-062, JCC-084, JCC-025
used_on:
related: JCC-059, JCC-019, JCC-022
tags:
  - java
  - concurrency
  - advanced
...
nav_order: 85
permalink: /technical-mastery/java-concurrency/structured-concurrency-theory/
---
...
| Field      | Value                                              |
|------------|----------------------------------------------------|
| Depends on | JCC-062 Structured Concurrency, JCC-084 Project Loom Design Rationale, JCC-025 Thread Interruption |
| Related    | JCC-059 CopyOnWriteArrayList, JCC-019 Virtual Threads, JCC-022 CompletableFuture Composition |
...
join():
  ...
  |
[ShutdownOnFailure]: task fails -> shutdown() -> interrupt
  others
[ShutdownOnSuccess]: task succeeds -> shutdown() ->
  interrupt others
  |
...
void allTasksCompleteOrNoneLeaks() throws Exception {
    ...
    try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
        scope.fork(() -> { running.incrementAndGet();
            Thread.sleep(100); running.decrementAndGet(
                ); return 1; });
        scope.fork(() -> { running.incrementAndGet();
            Thread.sleep(50);
...
- [[JCC-062 - Structured Concurrency]] - the Java API entry point
- [[JCC-084 - Project Loom Design Rationale]] - why virtual threads
  make structured concurrency practical
- [[JCC-025 - Thread Interruption and Cancellation]] - how
...
- [[JCC-022 - CompletableFuture Composition Patterns]] - no lifetime
  guarantee but stable API
- [[JCC-026 - CompletionService]] - result collection without
  lifetime management
