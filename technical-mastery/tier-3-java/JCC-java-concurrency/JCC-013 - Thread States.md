...
nav_order: 13
permalink: /technical-mastery/java-concurrency/thread-states/
id: JCC-014
...
BLOCKED     → Lock contention   → "waiting to lock <XX>
  (held by thread YY)"
WAITING     → wait() / park()   → "waiting on <object>" or
  "parking to wait"
TIMED_WAIT  → sleep / wait(ms)  → "sleeping" or "waiting
  on <object> timeout"
RUNNABLE    → running or I/O    → stack trace at execution
  point
