...
nav_order: 60
permalink: /technical-mastery/java-concurrency/jcc-046-thread-dump-analysis/
id: JCC-023
...
"http-nio-8080-exec-5" #44 daemon prio=5 os_prio=0
   java.lang.Thread.State: BLOCKED (on object monitor)
    at com.example.OrderService.getOrder(OrderService.java:
    - waiting to lock <0x000000076b5e4f68>
         (a com.example.OrderService)        ← wants THIS
           lock
    at com.example.PaymentService.pay(PaymentService.java:5
    - locked <0x000000076b5e5a20>
         (a com.example.PaymentService)      ← HOLDS THIS
           lock

"http-nio-8080-exec-3" #42 daemon prio=5 os_prio=0
   java.lang.Thread.State: BLOCKED (on object monitor)
    at com.example.PaymentService.refund(PaymentService.jav
    - waiting to lock <0x000000076b5e5a20>  ← wants what
      exec-5 holds
    at com.example.OrderService.cancel(OrderService.java:10
    - locked <0x000000076b5e4f68>           ← holds what
      exec-5 wants
...
Thread pool: 100 threads, all states:
  90 threads: BLOCKED "waiting to lock <0x...> (a
    DbConnectionPool)"
  10 threads: WAITING (on object monitor) in
    DbConnectionPool.getConnection
  → All 100 threads are waiting for DB connections!

"db-conn-pool-cleaner-1": RUNNABLE
  at com.example.pool.DbConnectionPool.cleanup(DbConnection
  - locked <0x...> (a DbConnectionPool)  ← HOLDS the pool
    lock
  → cleanup thread holds pool lock for 5+ minutes
    (cleaning stale connections)

Root cause: DbConnectionPool.cleanup() acquires pool lock
  for entire cleanup
duration. With all 100 request threads blocked, no
...
Thread X: RUNNABLE at MyClass.process:42 (same across 3
  dumps)
...
FAILURE PATH:
Concurrent lock bug only in ReentrantLock (not
  synchronized)
...
millions of virtual thread entries. Regular thread dump
  tools
...
void dumpThreadsOnHang() {
    ...
    if (deadlocked != null) {
        ThreadInfo[] infos = tmx.getThreadInfo(deadlocked, true,
            true);
...
watchdog.scheduleAtFixedRate(() -> {
    ...
    if (blockedCount > ALERT_THRESHOLD) {
        log.warn("High thread count: {} - dumping threads",
            blockedCount);
        dumpThreadsOnHang();
    }
...
│              │ not a substitute for continuous monitorin│
├──────────────┼──────────────────────────────────────────┤
│ TRADE-OFF    │ Zero code changes needed vs point-in-time│
│              │ only (JFR is continuous but more setup)  │
├──────────────┼──────────────────────────────────────────┤
│ ONE-LINER    │ "An X-ray of your running JVM: shows ever│
