...
nav_order: 37
permalink: /technical-mastery/java-concurrency/completable-future/
id: JCC-055
...
CompletableFuture.supplyAsync(() -> fetchData())
    ...
    // skipped if fetchData failed
    .exceptionally(ex -> defaultData())
    // handles failure, returns default
    .thenApply(d -> serialize(d))
    // runs with real or default data
    .handle((result, ex) -> {
    // always runs; both available
...
[supplyAsync(() -> fetchUser(id), pool)]  ← YOU ARE HERE
    ...
    → [thenApply callback: T1 or new thread
      transform(user)]
    → [thenCombine: waits for both futures, fires when
      ready]
...
List.of(f1, f2, f3).forEach(f -> {
    if (f.isCompletedExceptionally()) {
        f.exceptionally(ex -> { log.error("Failed",
            ex); return null; })
         .join();
    }
});
