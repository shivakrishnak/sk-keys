...
nav_order: 83
permalink: /technical-mastery/java-concurrency/jsr-133-java-memory-model-specification/
---
...
```
1. Program Order: A <HB B if A comes before B in same
  thread
2. Monitor Lock: unlock(m) <HB lock(m) for any later lock
3. Volatile:     write(v) <HB read(v) for any later
  volatile read
...
Thread 2: read volatile flag -> sees true
  ...
  -> HB edge consumed
Thread 2: read x
  -> HB chain: write(x) <HB write(flag) <HB read(flag) <HB
    read(x)
  -> x is guaranteed 42
```
...
Thread 2: read x
-> No HB edge -> JMM allows Thread 2 to see x = 0 (old
  value)
...
@Outcome(id = "1, 1", expect = ACCEPTABLE,
      desc = "Both threads see 1")
@Outcome(id = "0, 0", expect = FORBIDDEN,
       desc = "Should not see 0,0")
@State
