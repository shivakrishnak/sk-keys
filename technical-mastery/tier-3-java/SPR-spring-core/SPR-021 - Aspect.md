...
nav_order: 21
permalink: /technical-mastery/spring/aspect/
id: SPR-013
...
public class PerformanceAspect {
    ...
    public Object logSlowCalls(ProceedingJoinPoint pjp) throws
        Throwable {
...
```
AnnotationAwareAspectJAutoProxyCreator.postProcessAfterInit
  ↓
findEligibleAdvisors(beanClass):
  ...
  ↓
  Filter: AopUtils.findAdvisorsThatCanApply(advisors,
    beanClass)
...
UserService bean created
    ↓
AnnotationAwareAspectJAutoProxyCreator checks: any aspects
  match?
    YES ← YOU ARE HERE (Aspect applied to UserService)
    ↓
Proxy wrapping UserService with [SecurityAspect,
  LoggingAspect]
    ↓
...
public class ServiceAuditAspect {
    ...
    private static final Logger log =
        LoggerFactory.getLogger(ServiceAuditAspect.class);
    ...
    @AfterReturning(pointcut = "publicServiceMethod()",
        returning = "result")
    ...
    @AfterThrowing(pointcut = "publicServiceMethod()",
        throwing = "ex")
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                log.warn("Attempt {}/{} failed", attempt,
                    maxAttempts);
                if (attempt < maxAttempts) Thread.sleep(
                    100L * attempt);
            }
        }
        throw lastException;
...
│              │ Advice (WHAT) for one cross-cutting conce│
├──────────────┼──────────────────────────────────────────┤
