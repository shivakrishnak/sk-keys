...
nav_order: 25
permalink: /technical-mastery/spring/dispatcherservlet/
id: SPR-016
...
DispatcherServlet.doDispatch():
  1. getHandler() → HandlerMapping.getHandler() →
  ...
  6. processDispatchResult() → ViewResolver /
...
HTTP POST /users with JSON body
    ↓
Servlet filter chain (Spring Security auth filter, CORS
  filter)
    ↓
DispatcherServlet.doDispatch()
    ↓
HandlerMapping: POST /users → UserController.createUser()
    ↓ ← YOU ARE HERE (DispatcherServlet routes to
      controller)
...
# disable default static resource handler
```
...
│              │ → Response                               │
├──────────────┼──────────────────────────────────────────┤
│ SPRING BOOT  │ Auto-configured - no XML/annotation neede│
├──────────────┼──────────────────────────────────────────┤
