...
nav_order: 67
permalink: /technical-mastery/spring/transaction-propagation/
id: SPR-085
...
placeOrder() begins TX_1
    ...
    → TX_1 commits or rolls back based on placeOrder's
      logic
```
...
placeOrder() begins TX_1
    → logAction() begins TX_2 (suspends TX_1)
    → logAction() completes → COMMIT TX_2 (audit entry
      saved!)
    → TX_1 resumed
    → placeOrder() throws RuntimeException → ROLLBACK TX_1
    → Order rolled back. But audit entry is COMMITTED
      independently.
```
...
public class OrderService {
    ...
    public Order placeOrder(OrderRequest req) {
        ...
        // REQUIRES_NEW
        // If auditService fails, its TX rolls back but THIS TX
        // continues
        return order;
    }
}
...
public class InternalTransactionHelper {
    ...
    public void doSomethingThatRequiresTransaction() {
        // If called without active transaction: throws
...
public class NotificationService {
    ...
    public void sendEmailViaHttpApi(String to, String body) {
        httpClient.post("https://api.email.com/send",
            new EmailPayload(to, body));
...
public void importUsers(List<UserDto> users) {
    for (UserDto dto : users) {
        try {
            ...
            log.warn("Failed to import user {}: {}", dto.getEmail(),
                e.getMessage());
            // Inner rollback to savepoint - this user's changes
            // undone
...
public void importSingleUser(UserDto dto) {
    ...
    // If this throws: rolls back to savepoint, outer transaction
    // unaffected
}
```
...
// Option 2: Use REQUIRES_NEW for reserve() if it should be
// independent
// Option 3: Understand that in REQUIRED, inner exception = outer tx
