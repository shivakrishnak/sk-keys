...
nav_order: 27
permalink: /technical-mastery/spring/transactional/
id: SPR-061
...
userService.createUser(user) called
    ↓
CGLIB proxy intercepts ← @Transactional proxy wraps the
  call
    ↓
...
```
method throws DataIntegrityViolationException
  (RuntimeException)
    ↓
...
public class OrderService {
    ...
    public void saveAuditLog(String action) {
        ...
        // Always committed independently - even if outer tx rolls
        // back
    }
    ...
    public void createAndNotify(User user) throws MailException {
        ...
        // checked MailException triggers rollback
    }
}
```
...
```java
@Transactional(rollbackFor = Exception.class)
// or specific: MailException.class
...
// Option 2: Extend transaction scope (Open Session in View -
// discouraged)
// Option 3: Use DTOs with all required data loaded within the
// transaction
```
