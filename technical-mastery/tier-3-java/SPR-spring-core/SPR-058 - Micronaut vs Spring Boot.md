...
nav_order: 58
permalink: /technical-mastery/spring/micronaut-vs-spring-boot/
id: SPR-032
...
```
STARTUP COMPARISON (same service: 1 controller, 1 service,
  DB)
───────────────────────────────────────────────────────────
...
New service requirement
  ...
  │     │                         if team knows it)
  │     └─ Batch/scheduled job → Spring Batch has no MN
    equiv
  │
  ├─► Team knowledge?              ← YOU ARE HERE
  │     ├─ Spring expert team → Spring Boot
  │     ├─ Mixed/greenfield → evaluate both
  │     └─ Kotlin-first team → Micronaut (excellent Kotlin
    support)
  │
  ├─► Ecosystem needs?
  │     ├─ Spring Security OAuth2 → Spring Boot
  │     ├─ Spring Data JPA → Spring Boot (Micronaut Data ≠
    JPA)
...
application.properties/.yml  application.yml (same
  structure)
```
