...
difficulty: ★★★
depends_on: SPR-003, SPR-067, SPR-068
used_by:
related: SPR-077, SPR-080, SPR-085
tags:
  - spring
  - java
  - advanced
...
nav_order: 79
permalink: /technical-mastery/spr/spring-boot-configuration-strategy/
---
...
| Field          | Value                                                                                                                                                  |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Depends on** | [[SPR-003 - Why Spring Boot Changed Java Development]], [[SPR-067 - Auto-Configuration]], [[SPR-068 - Spring Boot Actuator]]                           |
| **Used by**    | -                                                                                                                                                      |
| **Related**    | [[SPR-077 - Spring Architecture at Scale]], [[SPR-080 - Spring Security Architecture Design]], [[SPR-085 - Spring Specification and Extension Points]] |
...
- [[SPR-067 - Auto-Configuration]] - how auto-config reads properties
- [[SPR-068 - Spring Boot Actuator]] - `/actuator/env` for config debugging

**Builds On This (learn these next):**

- [[SPR-080 - Spring Security Architecture Design]] - applying config strategy to security values
- [[SPR-052 - Spring Cloud Config]] - centralised external configuration
- [[SPR-077 - Spring Architecture at Scale]] - config at multi-team scale
...
```
+----------------------------------------------------------
| WHAT IT IS    | Decisions about where config lives &
  wins |
| PROBLEM       | Secrets in Git; wrong env in prod; no
  type|
...
| TRADE-OFF     | Simplicity vs security + flexibility
  |
| ONE-LINER     | Defaults in yml, env-specifics in
  profiles,|
|               | secrets in env vars / Vault
  |
| NEXT EXPLORE  | SPR-080 (Security Config), SPR-052
  |
|               | (Spring Cloud Config)
  |
+----------------------------------------------------------
...
_Hint:_ Look at Spring Cloud Config's Git backend, Vault's dynamic secret generation in [[SPR-052 - Spring Cloud Config]], and how `@RefreshScope` + actuator `/refresh` endpoint can propagate the change.
