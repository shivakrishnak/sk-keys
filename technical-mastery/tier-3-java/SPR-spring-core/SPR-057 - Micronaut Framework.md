...
nav_order: 57
permalink: /technical-mastery/spring/micronaut-framework/
id: SPR-076
...
gradle build
  ...
  ├─► Generated: *Definition.class,
    *DefinitionReference.class
  │
java -jar app.jar
  │
  ├─► ApplicationContext.start()                 ← YOU ARE
    HERE
  │     ├─► Load all DefinitionReference classes (fast
    index)
  │     ├─► Resolve bean graph (precomputed)
  │     ├─► Instantiate singletons in order
  │     └─► Register Netty HTTP routes from @Controller
    classes
  │
