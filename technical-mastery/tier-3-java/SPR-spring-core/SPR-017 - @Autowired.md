...
nav_order: 17
permalink: /technical-mastery/spring/autowired/
id: SPR-058
...
Bean class: UserService
  Constructor(UserRepository repo) ← @Autowired (or
    implicit)
  @Autowired UserEventPublisher events  ← field injection

During doCreateBean():
  1. Determine injection points:
     AutowiredAnnotationBeanPostProcessor.determineCandidat
  ...
  4. postProcessBeforeInitialization():
     AutowiredFieldElement.inject():
       → resolvedValue =
         beanFactory.resolveDependency(UserEventPublisher)
