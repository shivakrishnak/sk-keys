...
nav_order: 18
permalink: /technical-mastery/spring/qualifier-primary/
id: SPR-059
...
"expected single matching bean but found 2 primary
  candidates"
    ↓
...
```java
// Remove @Primary from one; it should apply to exactly one impl per
// type
@Component
@Primary
public class StripeGateway implements PaymentGateway { }
...
├──────────────┼──────────────────────────────────────────┤
│ USE WHEN     │ Multiple implementations of same interfac│
│              │ need to coexist as separate Spring beans │
├──────────────┼──────────────────────────────────────────┤
│ AVOID WHEN   │ Avoid two @Primary beans of same type;   │
│              │ avoid string @Qualifier in large codebase│
├──────────────┼──────────────────────────────────────────┤
...
├──────────────┼──────────────────────────────────────────┤
│ NEXT EXPLORE │ @Configuration/@Bean → Circular Dependenc│
└─────────────────────────────────────────────────────────┘
```
