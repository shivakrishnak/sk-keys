...
nav_order: 62
permalink: /technical-mastery/spring/circular-dependency/
id: SPR-080
...
1. Create UserService:
   - singletonFactories["userService"] = factory (produces
     raw UserService instance)
...
3. OrderService needs UserService:
   ...
   - Check singletonFactories: FOUND → call factory → get
     raw UserService
   - Move raw UserService to earlySingletonObjects
   - Inject raw UserService into OrderService (partially
     initialized)
    ...
    if (bean == null && isSingletonCurrentlyInCreation(beanName)) {
        ...
        if (bean == null) {
            ObjectFactory<?> factory =
                singletonFactories.get(beanName);  // level 3
            if (factory != null) {
                bean = factory.getObject();
                // calls getEarlyBeanReference()
...
(AOP wrap)                      ↓
    ↓                       getEarlyBeanReference() → AOP
      proxy
singletonObjects            inject AOP proxy into circular
  partner
                                ↓
...
│              │ cycles; ArchUnit for pre-startup detectio│
├──────────────┼──────────────────────────────────────────┤
