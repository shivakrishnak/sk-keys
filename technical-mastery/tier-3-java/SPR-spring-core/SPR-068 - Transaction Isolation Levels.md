...
nav_order: 68
permalink: /technical-mastery/spring/transaction-isolation-levels/
id: SPR-086
...
```
TX_A: UPDATE accounts SET balance = 0 WHERE id = 1   (not
  committed)
TX_B: SELECT balance FROM accounts WHERE id = 1      →
  reads 0 (dirty!)
TX_A: ROLLBACK  → balance is actually still the original
  value
...
TX_B: UPDATE accounts SET balance = 2000 WHERE id = 1;
  COMMIT
TX_A: SELECT balance FROM accounts WHERE id = 1      →
  2000 (different!)
```
...
```
TX_A: SELECT * FROM accounts WHERE balance > 1000    →
  returns 5 rows
...
TX_A: UPDATE accounts SET balance = 1500 WHERE id = 1;
  COMMIT
TX_B: UPDATE accounts SET balance = 1500 WHERE id = 1;
  COMMIT  ← TX_A's +500 lost!
```
...
READ_COMMITTED snapshot per STATEMENT:
    ...
    T=3: TX_A reads row X → sees TX_B's committed change
      (new snapshot)
    T=4: TX_A reads row X again → SAME result (TX_B
      already committed at T=2)
         (Non-repeatable read is possible if TX_B
           committed between TX_A's two reads)

REPEATABLE_READ snapshot per TRANSACTION:
    T=1: TX_A starts → takes snapshot S1
    T=2: TX_B starts, modifies row X, commits
    T=3: TX_A reads row X → sees S1's version (ignores
      TX_B's change)
    T=4: TX_A reads row X again → SAME S1 version
      (repeatable!)
```
...
TX_B: sets Bob off_call (1 remaining based on snapshot!) →
  COMMIT
Result: 0 doctors on call. INVARIANT VIOLATED.
Fix: SERIALIZABLE isolation (PostgreSQL SSI prevents this)
...
CORRECT TRANSFER with READ_COMMITTED:
  TX_1: BEGIN
  TX_1: SELECT balance FROM accounts WHERE id = 'A'  →
    $1000

  [CONCURRENT: TX_2 reads balance A → also $1000
    (committed value)]

  TX_1: UPDATE accounts SET balance = $500 WHERE id = 'A'
    (not committed)

  [TX_2 reads balance A with READ_COMMITTED → still sees
    $1000 committed version]

  TX_1: UPDATE accounts SET balance = $1000 WHERE id = 'B'
  TX_1: COMMIT

  [Now TX_2 reads balance A with READ_COMMITTED → sees
    $500 (committed)]
  → READ_COMMITTED is consistent for simple bank transfer
  → But: two tx simultaneously doing this without
    row-level locks = Lost Update!

SOLUTION: SELECT ... FOR UPDATE (pessimistic) or @Version
  (optimistic)
```
...
public class AccountService {
    ...
    public BalanceSnapshot getBalanceSnapshot(Long accountId) {
        ...
        // ... some processing ...
        BigDecimal stillCurrent = accountRepo.getBalance(accountId);
        // same value!
    ...
    public void criticalTransfer(Long fromId, Long toId,
        BigDecimal amount) {
...
@Transactional
public void transferPessimistic(Long fromId, Long toId,
    BigDecimal amount) {
    // Locks the row - other transactions wait
    Account from =
        accountRepo.findByIdForUpdate(fromId).orElseThrow();
...
public MonthlyReport generateReport(YearMonth month) {
    ...
    // consistent
    BigDecimal total = orderRepo.sumByMonth(month);
    // consistent
...
```
┌─────────────────────────────────────────────────────────┐
│ LEVEL               │ DIRTY │ NON-REPEAT │ PHANTOM │ PER│
│─────────────────────│───────│────────────│─────────│────│
│ READ_UNCOMMITTED    │ Yes   │ Yes        │ Yes     │ Fas│
│ READ_COMMITTED      │ No    │ Yes        │ Yes     │ Fas│
│ REPEATABLE_READ     │ No    │ No         │ Yes*    │ Med│
│ SERIALIZABLE        │ No    │ No         │ No      │ Slo│
├─────────────────────┴───────┴────────────┴─────────┴────┤
│ DEFAULT: database default (usually READ_COMMITTED)      │
│ USE: Optimistic locking (@Version) over SERIALIZABLE    │
│ DANGER: SERIALIZABLE → SerializationFailureException ris│
└─────────────────────────────────────────────────────────┘
*MySQL InnoDB REPEATABLE_READ also prevents phantom reads
  via MVCC/gap locks
```
