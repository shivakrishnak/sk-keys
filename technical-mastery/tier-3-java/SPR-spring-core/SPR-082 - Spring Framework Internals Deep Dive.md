...
difficulty: ★★★
depends_on: SPR-006, SPR-007, SPR-013, SPR-014, SPR-015, SPR-060, SPR-061
used_by:
related: SPR-085, SPR-083, SPR-009
tags:
  - spring
  - java
  - advanced
...
nav_order: 82
permalink: /technical-mastery/spr/spring-framework-internals-deep-dive/
---
...
| Field          | Value                                                                                                                                                                                                                                                 |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Depends on** | [[SPR-006 - IoC (Inversion of Control)]], [[SPR-007 - DI (Dependency Injection)]], [[SPR-013 - ApplicationContext]], [[SPR-014 - BeanFactory]], [[SPR-015 - Bean Lifecycle]], [[SPR-060 - BeanPostProcessor]], [[SPR-061 - BeanFactoryPostProcessor]] |
| **Used by**    | -                                                                                                                                                                                                                                                     |
| **Related**    | [[SPR-085 - Spring Specification and Extension Points]], [[SPR-083 - Spring Reactive Model (Project Reactor Internals)]], [[SPR-009 - Spring Context Refresh (AbstractApplicationContext)]]                                                           |
...
```
1. prepareRefresh()
   └─ Set start time, active flag, validate required
     properties
...
4. postProcessBeanFactory(beanFactory)
   └─ Subclass hook (e.g., web app adds web-specific
     scopes)

5. invokeBeanFactoryPostProcessors(beanFactory)
   └─ Run BeanDefinitionRegistryPostProcessor first
       (ConfigurationClassPostProcessor reads
         @Configuration)
...
- [[SPR-006 - IoC (Inversion of Control)]] - the principle the container implements
- [[SPR-013 - ApplicationContext]] - the container being described
- [[SPR-060 - BeanPostProcessor]] - the Phase 2 extension point
- [[SPR-061 - BeanFactoryPostProcessor]] - the Phase 1 extension point

**Builds On This (learn these next):**

- [[SPR-085 - Spring Specification and Extension Points]] - applying this knowledge to write extensions
- [[SPR-009 - Spring Context Refresh (AbstractApplicationContext)]] - the `refresh()` call in detail
- [[SPR-033 - Spring Boot AOT Compilation]] - shifting Phase 1 to build time
...
| WHAT IT IS    | Spring IoC two-phase container internals
  |
| PROBLEM       | Debugging "magic" failures without
  knowing|
|               | how the container actually works
  |
| KEY INSIGHT   | Phase 1 = BeanDefinitions; Phase 2 =
  |
|               | instances; extension points at both
  phases|
| USE WHEN      | Debugging AOP, transactions, circular
  deps;|
...
|               | correctly at all - no avoidance
  |
| TRADE-OFF     | Runtime flexibility vs startup
  complexity |
| ONE-LINER     | BeanFactoryPostProcessor: definitions;
  |
|               | BeanPostProcessor: instances
  |
| NEXT EXPLORE  | SPR-085 (Extension Points), SPR-009
  |
|               | (Context Refresh)
  |
+----------------------------------------------------------
