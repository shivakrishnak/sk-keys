...
nav_order: 20
permalink: /technical-mastery/spring/aop/
id: SPR-007
...
public User save(User user) {
    ...
    log.info("save returned: {} in {}ms", result, ms);
    // cross-cutting
    return result;
}
...
public class ExecutionLoggingAspect {
    ...
    public Object log(ProceedingJoinPoint pjp) throws Throwable {
        log.info("{} called with: {}", pjp.getSignature(),
            pjp.getArgs());
        long start = System.currentTimeMillis();
        Object result = pjp.proceed();
        long ms = System.currentTimeMillis() - start;
        log.info("{} returned: {} in {}ms", pjp.getSignature(),
            result, ms);
        return result;
    }
}
...
ApplicationContext.refresh()
    ↓
AnnotationAwareAspectJAutoProxyCreator registered as
  BeanPostProcessor
    ↓
@Aspect beans (LoggingAspect, TransactionAspect) created
  and registered
    ↓
...
ReflectiveMethodInvocation.proceed():
  ...
  2. @Around before-part (ProceedingJoinPoint before
    pjp.proceed())
  3. @Before advice
  4. proceed()
     ↓
...
Intercepted by CGLIB/JDK proxy
    ↓
Advisor chain: [LoggingAdvisor, TransactionAdvisor,
  CachingAdvisor]
...
TransactionAdvisor @Around: commit transaction
    ↓
LoggingAdvisor @AfterReturning: log("save returned %s",
  result)
    ↓
...
public class ExecutionLoggingAspect {
    ...
    @Around("execution(* com.example.service.*.*(..))")
    public Object logExecution(ProceedingJoinPoint pjp) throws
        Throwable {
