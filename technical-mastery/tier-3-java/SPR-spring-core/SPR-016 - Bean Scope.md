...
nav_order: 16
permalink: /technical-mastery/spring/bean-scope/
id: SPR-057
...
Proxy internally calls:
    ...
    ↓
If not present: create new UserContext, store in request
  attributes
...
public class ReportService {
    ...
    public Report generateReport(ReportRequest req) {
        ...
        ReportBuilder builder =
            beanFactory.getBean(ReportBuilder.class);
...
# Look for multiple threads inside the same method accessing instance
# fields
```
...
├──────────────┼──────────────────────────────────────────┤
│ KEY INSIGHT  │ Singleton is safe ONLY for stateless bean│
...
│              │ safety risk; Prototype: safe vs no cleanu│
├──────────────┼──────────────────────────────────────────┤
