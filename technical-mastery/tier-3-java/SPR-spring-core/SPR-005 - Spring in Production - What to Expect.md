...
nav_order: 5
permalink: /technical-mastery/spr/spring-in-production-what-to-expect/
---
...
```
+----------------------------------------------------------
| WHAT IT IS    | Spring Boot production readiness
  checklist|
| PROBLEM       | Gap between "runs locally" and "runs
  prod"|
| KEY INSIGHT   | The framework provides the tools; you
  must |
|               | configure them before first deployment
  |
| USE WHEN      | Any Spring Boot app before production
  ship |
| AVOID WHEN    | -
  |
| TRADE-OFF     | Config complexity vs operational
  resilience|
