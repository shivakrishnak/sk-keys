...
nav_order: 69
permalink: /technical-mastery/spring/n-plus-1-problem/
id: SPR-087
...
for (User u : users) {
    ...
    // Query 2...N+1 each iteration
    // Hibernate: SELECT * FROM orders WHERE user_id = ?  ← repeated N
    // times
}
```
...
// Trade-off: Cartesian product can cause duplicate users (use
// DISTINCT or Set)
```
...
Loop iteration 1: user.getOrders().size()
    ...
    → Hibernate: SELECT o.* FROM orders o WHERE o.user_id
      = 1
    → PersistentBag populated
    → .size() = 5

Loop iteration 2: user.getOrders().size()
    → PersistentBag.isInitialized? NO (different User
      object)
    → Hibernate: SELECT o.* FROM orders o WHERE o.user_id
      = 2
    → PersistentBag populated
...
Result set (3 users, user 1 has 2 orders, user 2 has 1
  order, user 3 has 0):
    Row 1: user_id=1, user_name=Alice, order_id=101
    Row 2: user_id=1, user_name=Alice, order_id=102  ←
      duplicate user row!
    Row 3: user_id=2, user_name=Bob,   order_id=103
    Row 4: user_id=3, user_name=Carol, order_id=null ←
      LEFT JOIN: no orders
...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    // Dynamically applies eager fetch for 'orders' for this specific
    // method
...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    // 1 query, no entity loading, no lazy associations, no N+1
    // possible
}
```
...
class N1DetectionTest {
    ...
    void detectN1() {
        ...
        userRepo.findAll().forEach(u -> u.getOrders().size());
        // trigger N+1
...
// Option 3: Separate queries (EntityGraphs don't solve MultipleBag
// either)
...
```
┌─────────────────────────────────────────────────────────┐
│ WHAT IT IS  │ 1 query for N entities + N queries for the│
