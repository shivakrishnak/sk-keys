...
nav_order: 3
permalink: /technical-mastery/spr/why-spring-boot-changed-java-development/
---
...
```
+----------------------------------------------------------
| WHAT IT IS    | Opinionated Spring assembler +
  auto-config|
| PROBLEM       | XML ceremony; manual dependency wiring
  |
| KEY INSIGHT   | Defaults active; user overrides always
  win |
...
|               | compile-time DI required (use Micronaut)
  |
| TRADE-OFF     | Startup "magic" vs zero-setup
  productivity|
| ONE-LINER     | start.spring.io → @SpringBootApplication
  |
|               | → java -jar
  |
| NEXT EXPLORE  | SPR-067 (Auto-Config), SPR-069
  (Lifecycle) |
+----------------------------------------------------------
```
