...
nav_order: 64
permalink: /technical-mastery/syd/what-is-a-cache/
---
...
Pros: simple, flexible, only caches what is actually
  needed.
Cons: first request for each item = cache miss (slow).
      On cache restart: all requests miss until cache
        warms.
      ("cold start problem")
```
...
Default recommendation: LRU (Redis default with
