...
nav_order: 48
permalink: /technical-mastery/syd/chat-system-design/
---
...
HTTP Long Polling:
  ...
  Server: hold connection until message arrives, then
    respond
...
WebSocket:
  ...
  Bidirectional: server can push anytime; client can send
    anytime
  
  Latency: < 50ms (persistent connection, minimal framing
    overhead)
...
Solution: Presence store (Redis)
  On connect: SET user:{user_id}:server = "ws-server-3" EX
    300
  On disconnect: DEL user:{user_id}:server
  On heartbeat: EXPIRE user:{user_id}:server 300
    (refreshed)
...
│             │ chat → "read". 3-state: sent/delivered/rea│
├─────────────┼──────────────────────────────────────────┤
  │
