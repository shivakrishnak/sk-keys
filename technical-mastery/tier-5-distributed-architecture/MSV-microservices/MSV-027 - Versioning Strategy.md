...
nav_order: 27
permalink: /technical-mastery/microservices/versioning-strategy/
---
...
URL PATH VERSIONING (most common in REST):
  ...
  Pros:
    ...
    - Cache-friendly (different URLs are different
      resources)
    - API Gateway routing: trivial (path prefix match)
  Cons:
    - Violates REST purity (version is not a resource
      property)
...
HEADER VERSIONING (REST-pure approach):
  ...
  Pros:
    - URL remains stable
    - Correct REST semantics (same resource, different
      representation)
  ...
  Cons:
    ...
    - Consumers often forget the header (silent v1
      fallback)
...
GRPC / PROTOBUF VERSIONING:
  ...
  Protobuf backward compatibility: preferred over
    versioning
```
...
Option B: Version at endpoint level
  ...
  Pros: minimal disruption (only affected endpoints
    versioned)
...
routing): route /v1/_ to the v1 controller, /v2/_ to v2.
