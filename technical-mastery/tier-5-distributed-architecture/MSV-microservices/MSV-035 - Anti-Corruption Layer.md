...
nav_order: 35
permalink: /technical-mastery/microservices/anti-corruption-layer/
---
...
SCENARIO 1 - LEGACY SYSTEM INTEGRATION:
  ...
  - QTY_ON_HAND - QTY_RESERVED calculation ->
...
SCENARIO 3 - POOR UPSTREAM MICROSERVICE:
  ...
  - Map payment-service's "amount_in_cents" to Money value
    object
  - Map payment-service's status codes to your domain
    statuses
```
...
Legacy ERP has:
  ...
  ErpItemDto {
    int ITEM_ID;
    String ITEM_TYPE;    // P=physical, V=virtual,
      S=service
    int QTY_ON_HAND;
    int QTY_RESERVED;
...
PHASE 2: New inventory-service replaces ERP inventory
  ...
  ACL updated: now calls inventory-service API instead of
    ERP
