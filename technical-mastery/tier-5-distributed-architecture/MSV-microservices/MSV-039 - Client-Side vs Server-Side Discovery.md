...
nav_order: 39
permalink: /technical-mastery/microservices/client-side-vs-server-side-discovery/
---
...
```
  OrderService  LoadBalancer/Gateway  ServiceRegistry
    PaymentService
       |               |                    |
         (3 instances)
       |  1. POST       |                    |
...
In Kubernetes:
  ...
  kube-proxy: iptables rules route 10.96.0.10 -> one of 3
    pods
...
│              │  transparent LB. K8s: built-in server-sid│
└─────────────────────────────────────────────────────────┘
```
