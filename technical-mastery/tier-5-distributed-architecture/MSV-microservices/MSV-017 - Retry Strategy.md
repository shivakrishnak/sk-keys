...
nav_order: 17
permalink: /technical-mastery/microservices/retry-strategy/
---
...
EXPONENTIAL BACKOFF: retry delay doubles each attempt
  ...
  Problem: if all callers start at same time (e.g.,
    deploy),
...
```
┌─────────────────────────────────────────────────────────┐
│ RETRY ONLY   │ Transient errors: timeout, 503, conn rese│
...
│              │ CB opens after N total failures incl. ret│
├──────────────┼──────────────────────────────────────────┤
│ IDEMPOTENCY  │ POST/mutations: add idempotency key      │
│              │ Server deduplicates: safe to retry       │
├──────────────┼──────────────────────────────────────────┤
│ ONE-LINER    │ "Retry transient failures with exponentia│
