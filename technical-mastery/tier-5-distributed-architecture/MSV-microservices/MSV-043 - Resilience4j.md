...
nav_order: 43
permalink: /technical-mastery/microservices/resilience4j/
---
...
CLOSED ---------------------------------> OPEN
  |                                         |
  | all calls pass through                  | all calls
    fail fast
  |                                         | (no upstream
    call)
...
HALF_OPEN: Allow N probe calls.
           Success rate >= threshold: -> CLOSED
             (recovered).
           Failure rate >= threshold: -> OPEN (still
             broken).
```
