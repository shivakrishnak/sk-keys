...
nav_order: 7
permalink: /technical-mastery/microservices/service-discovery/
---
...
Pros: caller is unaware of discovery, any
  language/framework
...
Ribbon Load Balancer (local to caller)
  ...
  │ 2. If stale: GET
    http://eureka/eureka/apps/INVENTORY-SERVICE
...
HTTP GET http://inventory-service/inventory/SKU-123
  │ kube-dns resolves "inventory-service" to ClusterIP
    10.96.0.5
  │
  ▼
...
Envoy Sidecar (intercepted locally)
  ...
  │ Applies routing rules (virtual service, destination
    rule)
