...
nav_order: 42
permalink: /technical-mastery/leadership/blameless-culture/
id: BHV-024
...
HUMAN ERROR:
  ...
  Context: the UI was ambiguous; the confirm dialog was
    unclear
...
AT-RISK BEHAVIOUR:
  ...
  Context: team norm allowed informal skips; low perceived
    risk
...
1. INCIDENT RESOLVED
   ...
   → Assign post-mortem facilitator (not the IC; fresh
     eyes)
    ↓
...
3. CONTRIBUTING FACTORS ANALYSIS
   ...
   → Categories: tooling, process, communication,
     monitoring,
...
| ---------------------------------------- | ----- | ---------- | ------ |
| Add pool utilisation alert (
    > 80%)       | Alice | YYYY-MM-DD | Open   |
...
```
DIAGNOSIS:
  → Are near-misses being reported? (If no: fear is
    present)
  → Are action items completed? (If not: post-mortems
    aren't real)
  → Do engineers volunteer to write post-mortems for their
    mistakes?
     (If never: culture is blame-based despite the label)

REPAIR:
1. LEADERSHIP MODELS FIRST:
   → Engineering leadership post-mortems their own
     mistakes PUBLICLY
   → "I pushed a config change without review and it
     caused X.
...
3. ACTION ITEM COMPLETION AS METRIC:
   → Track: what % of post-mortem action items close on
     time?
   → Review in engineering all-hands
   → Incomplete action items signal: "post-mortems don't
     matter"

4. MONTHLY REVIEW:
   → Engineering all-hands: "here are the incidents we had;
     here are the systemic fixes we made; here's what
       changed."
