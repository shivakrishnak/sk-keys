...
nav_order: 46
permalink: /technical-mastery/leadership/project-leadership/
id: BHV-050
...
PHASE 1: DEFINITION (before work begins)
  ...
  GOAL ALIGNMENT:
    ...
    "If we shipped this tomorrow, what metric would
      change?"
...
PHASE 2: EXECUTION (while work is in progress)
  ...
  STATUS COMMUNICATION (the format that works):
    ...
    Red: off track; here's what we need to get back on
      track
  ...
  BLOCKER RESOLUTION:
    Define a blocker: "Something that prevents progress on
      a
...
PHASE 3: DELIVERY + CLOSE
  ...
  ACCEPTANCE CRITERIA CHECK:
    "Do we meet the success criteria we defined in Phase
      1?"
  ...
  When a new request arrives mid-project:
    ...
    If not in scope:
      "This is valuable. Two options:
       Option A: Add it to scope - here's the timeline
         impact
...
DELIVERY:
  ...
  Retrospective
    ↓
POST-DELIVERY:
  Did we achieve the goal? (not: did we complete the
    scope?)
  What would we do differently?
```
...
AFTER (leadership communication):
  "Week 6: Auth module 70% complete. 🟡 Yellow.
   ...
   Options:
   A. Escalate to VP level for Team X prioritisation (my
     ask)
   B. Implement against v1 API and migrate later (2-week
     effort)
   C. Descope the auth integration for Q3 (discuss with
     product)

   Recommendation: Option A. I'll send the escalation
     today if
...
THE DIFFERENCE:
  ...
  Project leader:  here is the problem, here are the
    options,
                   here is my recommendation, here is what
                     I need
```
