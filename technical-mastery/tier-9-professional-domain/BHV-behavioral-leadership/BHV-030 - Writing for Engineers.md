...
nav_order: 30
permalink: /technical-mastery/leadership/writing-for-engineers/
id: BHV-023
...
```
PATTERN 1: BLUF STRUCTURE (for recommendations and
  proposals)
...
BAD (bottom-up):
  ...
  Result: reader who stopped at page 3 doesn't know what
    to do.

GOOD (BLUF):
  Recommendation: We should adopt Option B (use DynamoDB
    for
  session storage).
  ...
  Result: reader who reads one sentence knows the
    recommendation.
  Reader who reads the full document has the complete
    argument.

PATTERN 2: THE EXECUTIVE SUMMARY (for status updates,
  project docs)
...
PATTERN 3: RFC STRUCTURE
  ...
  Problem Statement:
    What is the problem? Why does it matter? What happens
      if
    we don't solve it?
  ...
  Implementation Plan:
    How would this be implemented? What does the rollout
      look like?

  Open Questions:
    What remains unresolved? What feedback are we
...
PRINCIPLE 1: ONE IDEA PER SENTENCE
  BAD: "The service is experiencing high latency which is
    causing
       user-visible failures, which in turn is increasing
         support
       ticket volume, leading to engineer burnout on the
         support team."
  GOOD: "The service has high latency. This causes
    user-visible failures.
         Support ticket volume has increased 40% this
           week."

  Short sentences are readable. Long sentences bury
    information.
...
PRINCIPLE 4: REMOVE THE THROAT-CLEARING
  BAD: "In this document, we will be discussing the
    background of
       the problem and then presenting our analysis of the
         various
       options we considered before arriving at our final
         recommendation."
  ...
  First sentences should contain information, not
    announcements.

PRINCIPLE 5: THE READER'S QUESTION IS "SO WHAT?"
  Every paragraph: ask "so what?" If there's no clear "so
    what":
  either state it explicitly or cut the paragraph.

  BAD: "The current session store uses Redis on a single
    node
       with 16GB of RAM."
  GOOD: "The current session store uses Redis on a single
    node (16GB),
        which will reach capacity in Q2 at current growth
          - this is
...
PASS 2: CLARITY
  ...
  Is the logic explicit? (No "therefore" that isn't
    obvious)
...
RULE: The first draft is for you (to think through the
  problem).
      The final draft is for the reader (to act on the
        information).
...
STEP 1: AUDIENCE + PURPOSE
  Who will read this? (Technical peer? VP?
...
STEP 5: READER TEST
  ...
  Ask: "What would you do after reading the first
    paragraph?"
...
STEP 4: Add an options table after the recommendation:
  | Option | Pros | Cons | Decision |
  This gives reviewers the comparison without reading
    prose.

STEP 5: Move all context and analysis to "Background"
  section
...
Each reader self-selects their depth. Decision time: 1
  week.
```
