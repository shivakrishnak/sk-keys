...
nav_order: 18
permalink: /technical-mastery/leadership/prioritization/
id: BHV-018
...
SHOULD HAVE (S):
  ...
  "Users would be significantly inconvenienced without
    this,
   but they could complete the task another way."
  Target: ~20% of total scope

COULD HAVE (C):
  Desirable; lower pain if absent; natural candidates for
    cuts
...
CONFIDENCE:
  ...
  80% = solid evidence; 50% = educated guess; 20% =
    untested
  Example: 80%
...
MoSCoW traps:
  ...
  MoSCoW as one-person decision
    → Needs stakeholder negotiation, not unilateral
      assignment
...
3. Retrospect on overrides: "We overrode RICE to choose X.
   Did X deliver the expected value? Was the override
     correct?"
```
...
│              │ For: comparing initiatives; quarterly pla│
├──────────────┼──────────────────────────────────────────┤
