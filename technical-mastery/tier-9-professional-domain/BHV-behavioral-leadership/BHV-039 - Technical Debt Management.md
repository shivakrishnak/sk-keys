...
nav_order: 39
permalink: /technical-mastery/leadership/technical-debt-management/
id: BHV-007
...
INADVERTENT DEBT (unknown until discovered):
  Definition: "We didn't know this was wrong when we did
    it"
  Examples:
    Patterns that seemed right but proved brittle
    Technology choices that didn't scale
    Architectural decisions made without full system
      context
...
LOCAL DEBT (contained):
  ...
  Risk: medium (isolated; impact on contributors to that
    area)
...
STRUCTURAL DEBT (systemic):
  ...
  Risk: high (impacts all development; hard to fix
    incrementally)
  Examples: wrong database for access patterns;
            monolith coupling blocking independent
              deployment
  Management: multi-quarter programme with dedicated
    investment
```
...
Measuring interest:
  - Developer surveys: "how much of your time is lost to
    X?"
  - DORA metrics: deployment frequency, change failure rate
  - Incident correlation: which services generate most
    incidents?
