...
nav_order: 36
permalink: /technical-mastery/react/usecallback-hook/
---
...
React render:
  ...
  handleClick = () => ...  // new function object each
    render
  React.memo compares props: prevProps.onClick !==
    nextProps.onClick
...
useCallback:
  ...
  Same JavaScript object reference → React.memo comparison
    passes
  → Re-render skipped
```
...
// With useCallback: stable reference, effect runs only when userId
// changes
...
function ProductList({ products, onProductUpdate }) {
  ...
  const filtered = useMemo(
    () =>
      filter === "all" ? products : products.filter((
          p) => p.type === filter),
    [products, filter],
  );
...
// React.memo: only re-renders when product, onDelete, or onToggle
// change
...
// For even better perf: use a ref for query and keep handleSubmit
// stable
```
