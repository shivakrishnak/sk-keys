...
nav_order: 23
permalink: /technical-mastery/react/useref-hook/
---
...
function Counter() {
  let timerId = null;           // reset to null every
    render
  ...
  const stop = () => {
    clearInterval(timerId);    // timerId is null (reset
      on render)
    // stop does not work
  };
}
...
function Counter() {
  const timerRef = useRef(null);  // same object every
    render
