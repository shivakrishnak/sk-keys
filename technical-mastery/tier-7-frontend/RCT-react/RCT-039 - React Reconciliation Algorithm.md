...
nav_order: 39
permalink: /technical-mastery/react/react-reconciliation-algorithm/
---
...
```
React traverses both trees simultaneously (old tree, new
  tree).
...
If types differ: React REPLACES the entire subtree.
  <div> → <span>
  Old div and ALL its children: UNMOUNT
...
Example bug:
  function Parent() {
    ...
    // On isLoading=false: Spinner unmounts, UserProfile
      mounts fresh
    // This is CORRECT - you want a fresh UserProfile
  }

  function Parent() {
    // BAD: ternary changes the element at the same tree
      position
    return (
      <div>
        {isEditing ? <TextInput /> : <TextDisplay />}
        // When isEditing changes: TextInput state is
          DESTROYED
        // Including typed text, focus, cursor position
      </div>
    );
    // Fix: keep both in the tree, use CSS display or
...
Without key (or index key):
  ...
  Updates: A gets D's data, B gets A's data, C gets B's
    data, new C
  WRONG: all items updated, state of A preserved on
    position 0 (now D)
...
Commit phase processes these in three passes:
  Pass 1 (before mutation): componentWillUnmount for
    deletions
  Pass 2 (mutation):        actual DOM operations
  Pass 3 (after mutation):  componentDidMount/Update,
    useLayoutEffect
...
// pos 0: Update task-1 with task-4 data (task-1 state preserved,
// wrong data)
...
// State (checkboxes, expanded state) is NOT matched to the correct
// item
```
