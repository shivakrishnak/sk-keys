...
nav_order: 49
permalink: /technical-mastery/react/react-performance-profiling/
---
...
5. Examine the flame chart:
   ...
   - Click a bar = see why it rendered
     ("Props changed", "State changed", "Context changed",
      "Parent rendered" - the last one is often the
        culprit)
...
7. Apply targeted fixes:
   - Slow component: useMemo, useCallback, computation
     extraction
...
1. Commit duration - total time React spent rendering
   this commit (paint update). >16ms = potential frame
     drop.
...
4. Render reason - WHY did this component render?
   "Props changed" → which prop changed? Is it a new
     object/array/function reference? (unstable reference
       bug)
...
function onRenderCallback(
  ...
  if (actualDuration > 16) {
    console.warn(`Slow render: ${id} took ${actualDuration.toFixed(
        2)}ms`);
...
function UserCard({ user, onEdit, onDelete }) {
  ...
  const handleEdit = useCallback(() => onEdit(user.id), [onEdit,
      user.id]);
      // Is this component actually memoised with React.memo?
...
│ PROP FN   │ Function prop new ref → useCallback in paren│
