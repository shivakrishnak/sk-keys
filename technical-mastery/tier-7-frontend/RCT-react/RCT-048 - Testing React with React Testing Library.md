...
nav_order: 48
permalink: /technical-mastery/react/testing-react-with-react-testing-library/
---
...
3. getByPlaceholderText - input placeholder (lower: not
  always accessible)
...
8. getByTestId       - data-testid attribute (LAST RESORT)
   ...
   Signals missing accessibility - consider fixing the
     a11y.
...
```
getBy*       → element must exist; throws if not found or
  multiple found
queryBy*     → element may not exist; returns null if not
  found
findBy*      → async; waits for element to appear; returns
  promise
getAllBy*    → multiple elements must exist; throws if
  none found
...
test("shows loading indicator while submitting", async () => {
  ...
  expect(screen.getByRole("status",
      { name: /loading/i })).toBeInTheDocument();
